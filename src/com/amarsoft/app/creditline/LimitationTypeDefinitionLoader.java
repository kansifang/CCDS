package com.amarsoft.app.creditline;

import com.amarsoft.are.sql.Transaction;
import com.amarsoft.are.util.ASValuePool;
import com.amarsoft.web.config.ASConfigure;
import com.amarsoft.web.config.loader.IConfigLoader;

public class LimitationTypeDefinitionLoader implements IConfigLoader{

	/* (non-Javadoc)
	 * @see com.amarsoft.config.loader.IConfigLoader#loadConfig(com.amarsoft.are.sql.Transaction)
	 */
	public ASValuePool loadConfig(Transaction Sqlca) throws Exception {
        ASValuePool vpLimitationTypeDef = null;
        String sLimitationTypeKey = null;
        LimitationType tmpType= null;
        
        vpLimitationTypeDef = new ASValuePool();
        String[] sKeys = {"TypeID",
        		"TypeName",
        		"CheckerClass",
        		"LimitationExpr",
        		"CompileCheckExpr",
        		"ControlType",
        		"ObjectType",
        		"CrossUsageEnabled",
        		"LimitationComp",
        		"LimitationWizard"
        		};
        
        StringBuffer sbSelect = new StringBuffer("");
        sbSelect.append("select ");
        for(int i=0;i<sKeys.length;i++) sbSelect.append(sKeys[i]+",");
        sbSelect.deleteCharAt(sbSelect.length()-1);
        sbSelect.append(" from CL_LIMITATION_TYPE ");
        
        String[][] sValueMatrix = Sqlca.getStringMatrix(sbSelect.toString());
        for(int i=0;i<sValueMatrix.length;i++){
        	sLimitationTypeKey = sValueMatrix[i][0];
        	tmpType = new LimitationType();
        	for(int j=0;j<sValueMatrix[i].length;j++){
            	tmpType.setAttribute(sKeys[j],sValueMatrix[i][j]);
            }
            vpLimitationTypeDef.setAttribute(sLimitationTypeKey,tmpType,false);
        }

        ASConfigure.setSysConfig("SYSCONF_LIMITATIONTYPE",vpLimitationTypeDef);
        return vpLimitationTypeDef;		

	}
}
