...
public class GetExposureBalance extends Bizlet {
	...
	public Object run(Transaction Sqlca) throws Exception {
        ...
        if("balance".equals(sFlag)){
        	sSql = " Select nvl(Balance,0)*getERate(BusinessCurrency,'"+sCurrency+"',ERateDate) as Balance,nvl(BailSum,0)*getERate(BailCurrency,'"+sCurrency+"','') as BailSum "+
        		   " from "+sTable+" where SerialNo='"+sObjectNo+"' and (PigeonholeDate !=' ' and PigeonholeDate is not null) ";
        }else{
        	sSql = " Select nvl(BusinessSum,0)*getERate(BusinessCurrency,'"+sCurrency+"',ERateDate) as Balance,nvl(BailSum,0)*getERate(BailCurrency,'"+sCurrency+"','') as BailSum "+
