...
public class DefaultCreditLineKeeper extends CreditLineKeeper
{
    ...
    public double calcBalance(Transaction Sqlca, String sBalanceID)throws Exception{
        ...
        Any aReturn = null;
        try{
	        String sExpression = (String)this.getCreditLine().getType(Sqlca).getAttribute(sBalanceID);
    ...
    public Vector checkLine(Transaction Sqlca,String sObjectType,String sObjectNo) throws Exception{
    	...
    	String sExpression = (String) this.getCreditLine().getType(Sqlca).getAttribute("CheckExpr");
		if (sExpression == null)
			throw new ASException("没有取到额度合法性检查公式。额度编号："
					+ this.getCreditLine().id()
					+ ",字段：CheckExpr。请检查额度类型定义（CL_TYPE）");
		try {
