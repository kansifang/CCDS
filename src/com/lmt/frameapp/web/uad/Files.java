...
public class Files {
	...
	protected void addFile(File newFile) {
		if (newFile == null) {
			throw new IllegalArgumentException(
					"[Amarsoft Upload]newFile cannot be null.");
		} else {
	...
	public File getFile(int index) {
		if (index < 0)
			throw new IllegalArgumentException(
					"[Amarsoft Upload]File's index cannot be a negative value (1210).");
		File retval = (File) m_files.get(new Integer(index));
		if (retval == null)
			throw new IllegalArgumentException(
					"[Amarsoft Upload]Files' name is invalid or does not exist (1205).");
		else
			return retval;
	}
