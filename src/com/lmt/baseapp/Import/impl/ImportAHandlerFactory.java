...
public class ImportAHandlerFactory{
	...
	public static void handle(String sFiles,String sFileType,String HandlerFlag,String sConfigNo,String sOneKey,ASUser CurUser,Transaction Sqlca) throws Exception{
		ImportAHandlerFactory.beforeHandle(sFiles, sFileType,HandlerFlag, sConfigNo, sOneKey, CurUser, Sqlca);
		if("Customer".toUpperCase().equals(HandlerFlag)){
			ImportAHandlerFactory.customerHandle(sConfigNo, sOneKey, Sqlca);
		}else if("Contract".toUpperCase().equals(HandlerFlag)){
			ImportAHandlerFactory.contractHandle(sConfigNo, sOneKey, Sqlca);
		}else if("Duebill".toUpperCase().equals(HandlerFlag)){
			ImportAHandlerFactory.dueBillHandle(sConfigNo, sOneKey, Sqlca);
		}else if("OperationReport".toUpperCase().equals(HandlerFlag)){
			ImportAHandlerFactory.operationReportHandle(HandlerFlag,sConfigNo, sOneKey, Sqlca);
		}
	}
	/**
	 * 客户信息导入后处理
	...
	private static void contractHandle(String sConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		...
		Sqlca.executeSQL("Delete from Batch_Import_Process where ConfigNo='"+sConfigNo+"' and OneKey='"+sOneKey+"'");
	 	String groupBy="case when ~s合同明细@其他担保方式e~ like '%保证%' and ~s合同明细@其他担保方式e~ like '%软抵押%' then '保证+软抵押' "+
	 			...
	 			"when ~s合同明细@其他担保方式e~ like '%抵押%' and ~s合同明细@其他担保方式e~ like '%质押%' then '抵押+质押' "+
	 			"else '其他担保' end";
	 	AIContractHandler.process(sConfigNo, sOneKey, Sqlca,"混合担保方式",groupBy);
	 	//4、加工后，进行合计，横向纵向分析
	 	AIContractHandler.afterProcess(sConfigNo, sOneKey, Sqlca);
	}
	/**
	 * 借据导入后处理
	 * @param sheet
	...
	private static void dueBillHandle(String sConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		...
		AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"归属条线","~s借据明细@归属条线e~");
 		String groupBy="case "+
				...
				"when ~s借据明细@经营类型(新)e~ like '%有色冶炼%' then '有色冶炼' "+
	 			"else '其他' end";
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"经营类型(新)",groupBy);
	 	...
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"期限业务品种",groupBy);
	 	...
	 	groupBy="case when ~s借据明细@主要担保方式e~ like '保证-%' then '保证' "+
	 			...
	 			"else '其他' end";
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"单一担保方式",groupBy);
	 	...
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"企业规模","~s借据明细@企业规模e~");
	 	
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"业务品种","~s借据明细@业务品种e~");
	 	...
	 	groupBy="case when ~s借据明细@国家地区e~ like '%太原市%' then '太原市' "+
	 			...
	 			"else '其他地区' end";
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"地区分类",groupBy);
	 	
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"机构分类","~s借据明细@直属行名称e~");
	 	//贷款金额区间
	 	groupBy="case when ~s借据明细@余额(元)e~ >=500000000 then '5亿以上（含5亿）' "+
	 			...
	 			"else '5000万以下' end";
	 	AIDuebillHandler.process(sConfigNo, sOneKey, Sqlca,"贷款余额区间",groupBy);
	 	
	 	
	 	//4、加工后，进行合计，横向纵向分析
	 	AIDuebillHandler.afterProcess(sConfigNo, sOneKey, Sqlca);
	}
	/**
	 * 月度经营报告处理
	 * @param sheet
