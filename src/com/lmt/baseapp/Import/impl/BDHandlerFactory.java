...
public class BDHandlerFactory{
	...
	private static void dueBillHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		...
		DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"归属条线","~s借据明细@归属条线e~","");
 		...
 		String groupBy="case "+
				...
				"when ~s借据明细@经营类型(新)e~ like '%有色冶炼%' then '有色冶炼' "+
	 			"else '其他' end";
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"经营类型(新)",groupBy,"");
	 	...
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"期限业务品种",groupBy,"");
	 	...
	 	groupBy="case when ~s借据明细@主要担保方式e~ like '保证-%' then '保证' "+
	 			...
	 			"else '其他' end";
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"单一担保方式",groupBy,"");
	 	...
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"企业规模","~s借据明细@企业规模e~","");
	 	
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"业务品种","~s借据明细@业务品种e~","");
	 	...
	 	groupBy="case when ~s借据明细@国家地区e~ like '%太原市%' then '太原市' "+
	 			...
	 			"else '其他地区' end";
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"地区分类",groupBy,"");
	 	
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"机构分类","~s借据明细@直属行名称e~","");
	 	//贷款金额区间
	 	groupBy="case when ~s借据明细@余额(元)e~ >=500000000 then '5亿以上（含5亿）' "+
	 			...
	 			"else '5000万以下' end";
	 	DataDuebillInHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"贷款余额区间",groupBy,"");
	...
	private static void dueBillRHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		...
		Sqlca.executeSQL("Delete from Batch_Import_Process where HandlerFlag='"+HandlerFlag+"' and ConfigNo='"+sReportConfigNo+"' and OneKey='"+sOneKey+"'");
 		String groupBy="case "+
		 				...
		 				" else '其他条线' end";
 		DataDuebillRetailHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"归属条线",groupBy,"");
	 	//4、加工后，进行合计，横向纵向分析
