...
public class DBHandlerFactory{
	...
	public static void handle(String HandlerFlag,String sReportConfigNo,String sOneKey,ASUser CurUser,Transaction Sqlca) throws Exception{
		DBHandlerFactory.beforeHandle(sReportConfigNo, sOneKey, CurUser, Sqlca);
		if("RiskReport".toUpperCase().equals(HandlerFlag)){//全面风险报告
			DBHandlerFactory.riskReportHandle(HandlerFlag,sReportConfigNo, sOneKey, Sqlca);
		}else if("OperationReport".toUpperCase().equals(sReportConfigNo)){
			//DBHandlerFactory.contractHandle(sReportConfigNo,sReportConfigNo, sOneKey, Sqlca);
		}else if("Duebill".toUpperCase().equals(sReportConfigNo)){
			DBHandlerFactory.dueBillHandle(sReportConfigNo,sReportConfigNo, sOneKey, Sqlca);
		}else if("DuebillR".toUpperCase().equals(sReportConfigNo)){
	...
	private static void riskReportHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		//1、对中间表数据进行特殊处理 	 		 	
		DBRiskReportHandler.interimProcess(sReportConfigNo, sOneKey, Sqlca);
		//先清空目标表 
		Sqlca.executeSQL("Delete from Batch_Import_Process where HandlerFlag='"+HandlerFlag+"' and ConfigNo='"+sReportConfigNo+"' and OneKey='"+sOneKey+"'");
	 	DBRiskReportHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"归属条线","ManageDepartFlag","");
	 	//4、加工后，进行合计，横向纵向分析
	 	DBRiskReportHandler.afterProcess(HandlerFlag,sReportConfigNo, sOneKey, Sqlca);
	}
	/**
	 * 合同导入后处理
	 * @param sheet
	 ...
	 * @throws Exception
	 */
	private static void contractHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		//1、对中间表数据进行特殊处理 	 		 	
		AIContractHandler.interimProcess(sReportConfigNo, sOneKey, Sqlca);
		//先清空目标表 
		Sqlca.executeSQL("Delete from Batch_Import_Process where HandlerFlag='"+HandlerFlag+"' and ConfigNo='"+sReportConfigNo+"' and OneKey='"+sOneKey+"'");
	 	String groupBy="case when ~s合同明细@其他担保方式e~ like '%保证%' and ~s合同明细@其他担保方式e~ like '%软抵押%' then '保证+软抵押' "+
	 			...
	 			"else '其他担保' end";
	 	AIContractHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"混合担保方式",groupBy);
	 	//4、加工后，进行合计，横向纵向分析
	 	AIContractHandler.afterProcess(HandlerFlag,sReportConfigNo, sOneKey, Sqlca);
	}
	/**
	 * 借据导入后处理
	 * @param sheet
	...
	private static void dueBillHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		...
		AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"归属条线","~s借据明细@归属条线e~","");
 		String groupBy="case "+
				...
				"when ~s借据明细@经营类型(新)e~ like '%有色冶炼%' then '有色冶炼' "+
	 			"else '其他' end";
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"经营类型(新)",groupBy,"");
	 	...
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"期限业务品种",groupBy,"");
	 	...
	 	groupBy="case when ~s借据明细@主要担保方式e~ like '保证-%' then '保证' "+
	 			...
	 			"else '其他' end";
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"单一担保方式",groupBy,"");
	 	...
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"企业规模","~s借据明细@企业规模e~","");
	 	
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"业务品种","~s借据明细@业务品种e~","");
	 	...
	 	groupBy="case when ~s借据明细@国家地区e~ like '%太原市%' then '太原市' "+
	 			...
	 			"else '其他地区' end";
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"地区分类",groupBy,"");
	 	
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"机构分类","~s借据明细@直属行名称e~","");
	 	//贷款金额区间
	 	groupBy="case when ~s借据明细@余额(元)e~ >=500000000 then '5亿以上（含5亿）' "+
	 			...
	 			"else '5000万以下' end";
	 	AIDuebillHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"贷款余额区间",groupBy,"");
	...
	private static void dueBillRHandle(String HandlerFlag,String sReportConfigNo,String sOneKey,Transaction Sqlca) throws Exception {
		//1、对中间表数据进行特殊处理 	 		 	
		AIDuebillRHandler.interimProcess(sReportConfigNo, sOneKey, Sqlca);
		//清空目标表 
		Sqlca.executeSQL("Delete from Batch_Import_Process where HandlerFlag='"+HandlerFlag+"' and ConfigNo='"+sReportConfigNo+"' and OneKey='"+sOneKey+"'");
 		String groupBy="case "+
		 				...
		 				" else '其他条线' end";
 		AIDuebillRHandler.process(HandlerFlag,sReportConfigNo, sOneKey, Sqlca,"归属条线",groupBy,"");
	 	//4、加工后，进行合计，横向纵向分析
 		AIDuebillRHandler.afterProcess(HandlerFlag,sReportConfigNo, sOneKey, Sqlca);
	}
	/**
	 * 月度经营报告处理
	 * @param sheet
