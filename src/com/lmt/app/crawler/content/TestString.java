			...
			if (matcher != null && matcher.matches()) {
				...
				while (matcher != null && matcher.find()) {
					if (matcher.groupCount() > 3) {
						bString = matcher.group(matcher.groupCount() - 3);// 这个group包含所有符合正则的<a..></a>
						aString = matcher.group(matcher.groupCount() - 2);// 这个group包含url的<a ...>部分
						String url1 = matcher.group(matcher.groupCount() - 1);// 最后一个group就是url
						String urlDisplayContent = matcher.group(matcher.groupCount() - 0);// 最后一个group就是url
