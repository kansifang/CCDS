...
public class TestTabDivSerial {
	...
	public void channelParseProcess() {
		...
		while (hi.hasNext()) {
			...
			if ((tc.getTableRow() == this.channelNumber)||(this.channelNumber == -1)) {
				...
				while (hl.hasNext()) {
					...
					if (lt.getLinkText().length() < 8) { //a href=  可不是最少7个字符
						continue;
					}
					/** 过滤无效link* */
					if (isChannelLink(lt.getLinkText()) != SpiderConstant.COMMONCHANNEL) {
						continue;
					}
					/** 生成link的hashcode* */
	...
	private int isChannelLink(String linkText) {
			...
			if (bl) {
				...
				while (ti.hasNext()) {
					...
					if (node instanceof LinkTag) {
						...
						tc.getTextBuffer().append(collapse(node.getText().replaceAll(" ", "")));
					}
				}
				htmlContext.add(tc);
				return null;
