...
public class UpdateBusiness extends Bizlet {
	...
	//获取关联表
		...
		throws Exception
	 {
		 ...
		 String sLineID = DataConvert.toString(Sqlca.getString(sSql));
		 
		 //add by xqli 2010-07-31 更新额度的登记信息时，也更新额度分配的登记信息
		 Sqlca.executeSQL("" +
				 " update CL_INFO set "+
				 " InputOrg = '"+sOrgID+"', "+
				 " InputUser = '"+sUserID+"', "+
				 " UpdateTime = '"+sUpdateTime+"' "+
				 " where ParentLineID = '"+sLineID+"' "
		 		);
		...
		throws Exception
	 {
		 ...
		 if(sObjectType.equals("PutOutApply")) //出账
		 {
			 //更新出账信息
			 sSql = " update "+sObjectTable+" set "+
			 		" InputOrgID = '"+sOrgID+"', "+
		 ...
		 {
			 sSql = " update "+sObjectTable+" set "+
			 		" MFOrgID = getCoreOrgId('"+sOrgID+"'), "+//更新借据的核心机构代码
			 		" InputOrgID = '"+sOrgID+"', "+
			 		" InputUserID = '"+sUserID+"', "+
			 		" OperateOrgID = '"+sOrgID+"', "+
			 		" OperateUserID = '"+sUserID+"', "+
