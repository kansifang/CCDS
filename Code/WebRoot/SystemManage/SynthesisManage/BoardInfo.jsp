...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sSql ="select BoardNo,BoardName,BoardTitle,BoardDesc,IsPublish,IsNew,IsEject,FileName,"+
	...
	doTemp.setReadOnly("FileName,ContentType,ContentLength,UploadTime",true);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	/*~[Describe=数据校验;InputParam=无;OutPutParam=无;]~*/
	function verify(){
		sBoardName = getItemValue(0,getRow(),"BoardName"); 
		sBoardTitle = getItemValue(0,getRow(),"BoardTitle");
		sBoardTitle = sBoardTitle.replace(/(^\s*)|(\s*$)/g,'') ;
		sBoardName = sBoardName.replace(/(^\s*)|(\s*$)/g,'');
		if(sBoardTitle == "" || sBoardName == ""){
			alert("必输项不能为空格，请重新填写！");
			return false;
		}
		setItemValue(0,getRow(),"BoardName",sBoardName);
		setItemValue(0,getRow(),"BoardTitle",sBoardTitle);
		return true;
	}
