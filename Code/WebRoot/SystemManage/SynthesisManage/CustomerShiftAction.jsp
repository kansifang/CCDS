...
<%
	...
	int iCount = 0,iRiskSignalCount=0; //记录数
	int i = 0;//计数器
    String sSql = ""; //SQL语句
    ...
    sSql = 	" select count(BA.SerialNo) "+
    		...
    		" and (BA.PigeonholeDate is null or BA.PigeonholeDate = '') "+
    ...
    if(iCount==0 && iRiskSignalCount==0)
    {		
        ...
        try
        {
           	for(int j = 0 ; j < ChangeObject.length ; j ++)
	    	{
	        	...
	        	}else if(ChangeObject[j].equals("RiskSignal")){//预警信息
	...
	{
		if(iRiskSignalCount>0)
		{
			sFlag = "UNFINISHRISKSIGNAL";//存在在途的预警
		}else{
