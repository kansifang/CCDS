...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "合同历史详情"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%
	...
	//获得页面参数	
	String sBCSerialNo =  DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("BCSerialNo"));
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//从业务表中获得业务品种
	sSql = "select CustomerID,BAAgreement,ApplyType,RelativeSerialNo,CustomerID,BusinessType,OccurType,TempSaveFlag,ApproveDate from BUSINESS_CONTRACT_BAK where SerialNo = '"+sObjectNo+"' ";
	rs = Sqlca.getASResultSet(sSql);
	if(rs.next())
	{ 
	...
	doTemp.FromClause = " from BUSINESS_CONTRACT_BAK ";
	doTemp.WhereClause = " where BCSerialNo = '"+sBCSerialNo+"' ";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function goBack()
	{
		OpenPage("/SystemManage/SynthesisManage/HistoryContractList.jsp","_self","");
	}
