...
<%
	...
	sSql = 	" select EI.CustomerID,EI.EnterpriseName,getItemName('CertType',CI.CertType) as CertTypeName,CI.CertID,getItemName('IndustryName',IndustryName) as IndustryName,EI.EmployeeNumber,EI.SellSum,EI.TotalAssets,getItemName('Scope',EI.scope) as Scope,getItemName('EntScale',EI.LockEntScale) as LockEntScale"+
			...
			" and CI.CustomerID = EI.CustomerID and (EI.lockentscale = '02' or EI.lockentscale is null or EI.lockentscale = '') ";
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		   ...
		   {"true","","Button","调整企业规模","调整企业规模","viewAndEdit()",sResourcesPath}
		};
	%> 
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function viewAndEdit()
	{
		...
		ColName = getItemValue(0,getRow(),"ColName");
		TableName = getItemValue(0,getRow(),"TableName");	
		WhereClause = getItemValue(0,getRow(),"WhereClause");
		if (typeof(sCustomerID)=="undefined" || sCustomerID.length==0)
		{
			alert(getHtmlMessage('1'));//请选择一条信息！
			return;
		...
		{			
		   	sReturn = popComp("EntScaleChange","/SystemManage/ParameterManage/EntScaleChange.jsp","CustomerID="+sCustomerID,"dialogWidth=30;dialogHeight=15;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");		    
		    if(sReturn=='_ACCEPT_')
		    {
		    	RunMethod("CustomerManage","RelativeEntScale",sCustomerID+","+sEntSignal);
		    	//RunMethod("PublicMethod","UpdateColValue",ColName+","+TableName+","+WhereClause);
		        reloadSelf();
		    }else
		        return;
		}			
