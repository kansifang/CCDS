...
<%	
	...
	doTemp.setDDDWCode("Status","IsInUse");
    //filter过滤条件
    doTemp.setColumnAttribute("BelongOrgName,UserName,UserID,Status,DepartMentCode","IsFilter","1");
	doTemp.generateFilters(Sqlca);
	doTemp.parseFilterData(request,iPostChange);
	if(CurUser.hasRole("0J2"))
	{
		doTemp.WhereClause=doTemp.WhereClause+" and BelongOrg='9900' and DepartMent in('0010') ";
	}
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = 
        {
            {((CurUser.hasRole("098"))?"true":"false"),"","Button","新增","在当前机构中新增人员","my_add()",sResourcesPath},			
			{"false","","Button","引入","引入人员至当前机构","my_import()",sResourcesPath},
            {((CurUser.hasRole("098"))?"true":"false"),"","Button","停用","从当前机构中删除该人员","my_del()",sResourcesPath}, 
            {((CurUser.hasRole("098") || CurUser.hasRole("097"))?"true":"false"),"","Button","详情","查看用户详情","viewAndEdit()",sResourcesPath},
            {((CurUser.hasRole("099") || CurUser.hasRole("097"))?"true":"false"),"","Button","用户角色","查看并可修改人员角色","viewAndEditRole()",sResourcesPath},
            {((CurUser.hasRole("099"))?"true":"false"),"","Button","批量更新角色","批量更新角色","my_Addrole()",sResourcesPath},
			{((CurUser.hasRole("099"))?"true":"false"),"","Button","多用户更新角色","多用户更新角色","MuchAddrole()",sResourcesPath},
            {((CurUser.hasRole("099") || CurUser.hasRole("299") || CurUser.hasRole("0M2"))?"false":"false"),"","Button","用户权限","查看并可修改人员权限","viewAndEditRight()",sResourcesPath},
            {((CurUser.hasRole("099") || CurUser.hasRole("299") || CurUser.hasRole("0M2"))?"false":"false"),"","Button","转移","转移人员至其他机构","UserChange()",sResourcesPath},                       
            {((CurUser.hasRole("098") || CurUser.hasRole("299") || CurUser.hasRole("0M2") || CurUser.hasRole("0J2"))?"true":"false"),"","Button","初始密码","初始化该用户密码","ClearPassword()",sResourcesPath}            
	...
	function UserChange()
	{
        ...
        {	
		    ...
		    }else
		    {
				...
				//判断是否存在未交接业务
				sReturn=RunMethod("PublicMethod","GetColValue", "1,BUSINESS,String@ManageUserID@"+sUserID);
				if(typeof(sReturn) != "undefined" && sReturn != "") 
				{
					alert("该用户存在未交接业务,不能进行转移");
					return;
				}
				//调用页面更新
