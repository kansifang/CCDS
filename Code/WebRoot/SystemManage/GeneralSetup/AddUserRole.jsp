...
<%
	...
	sSql = " select RoleID || ' ' || RoleName as Allrole,RoleID" +
			...
			" and RoleID not in (select RoleID from USER_ROLE where UserID = '"+sUserID+"') ";
/*
...
*/
	...
	sSql1 = " select ri.RoleID || ' ' || ri.RoleName as Allrole,ri.RoleID" +
			" from USER_ROLE ur,ROLE_INFO ri "+
			" where ur.RoleID = ri.RoleID "+
			" and ur.UserID = '"+sUserID+"' "+
			" and ur.RoleID <> '800'";
...
<script language="JavaScript">
   ...
   function pushButton(buttonName,push) 
	...
	function doQuery()
	{
		...
		PopPage("/SystemManage/GeneralSetup/AddRolesAction.jsp?UserID=<%=sUserID%>&RolesID="+vReturn,"","dialogWidth=20px;dialogHeight=20px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
		self.close();
	}
