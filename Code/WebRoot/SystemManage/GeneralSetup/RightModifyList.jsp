...
<%	
	...
	if(sRightType.equals("0101"))
        sSql += " and ApplyRight = '"+CurOrg.OrgID+"' and ApplyStatus = '1' ";
	//add by xhyong 2009/08/18
	if(CurUser.hasRole("0A0") && CurUser.hasRole("0D0"))
	{
		sSql = sSql+" and 1 = 1";
	}
    else if(CurUser.hasRole("0A0")||CurUser.hasRole("2A0")||CurUser.hasRole("0M1"))//总行个人客户权限管理员,中心支行个人客户权限管理员,直属支行个人客户权限管理员
    {
    	sSql = sSql+" and exists(select 1 from CUSTOMER_INFO CI where CustomerID=CB.CustomerID and CustomerType like '03%')";
    }
    else if(CurUser.hasRole("0D0")||CurUser.hasRole("2D0")||CurUser.hasRole("0M0")){//总行公司客户权限管理员,中心支行公司客户权限管理员,直属支行公司客户权限管理员
    	sSql = sSql+" and exists(select 1 from CUSTOMER_INFO CI where CustomerID=CB.CustomerID and CustomerType not like '03%')";
    }
    
	if(sRightType.equals("0101"))//当前工作限制
	{
	    //  10--直属支行客户权限管理员  20--中心支行客户权限管理员 30--总行客户权限管理员 
	    if(CurUser.hasRole("0M0")||CurUser.hasRole("0M1"))//直属支行权限管理员
	    {
	    	sSql = sSql+" and Flag1 = '10'";
	    }else if(CurUser.hasRole("2A0")||CurUser.hasRole("2D0")) //中心支行客户权限管理员
	    {
	    	sSql = sSql+" and Flag1 = '20'";
	    }
	    else if(CurUser.hasRole("0A0")||CurUser.hasRole("0D0")) //总行客户权限管理员
	    {
	    	sSql = sSql+" and Flag1 = '30'";
	    }
	}
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		...
		{(sRightType.equals("0101")?"true":"false"),"","Button","审阅申请","审阅申请","CheckApply()",sResourcesPath}
		};
