...
//	String sIsUpdate = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("IsUpdate"));
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "ReserveDataEntInfo";
	...
	doTemp.setVisible("SubjectNo,DuebillNo,OverDueDays",false);
	//doTemp.setHTMLStyle("LoanAccount","style={width:155}");
    ...
    {
    	doTemp.setVisible("AFiveClassify,ManageStatFlag",false);
    }
    ...
    /*
    String sSql = "select MFiveClassify,AFiveClassify,ManageStatFlag from reserve_total where LoanAccount = '"+sLoanAccount+"' and AccountMonth ='"+ sAccountMonth +"'";
    ASResultSet rs = null;
    rs = Sqlca.getASResultSet(sSql);
    if(rs.next()){
			sMFiveClassify = DataConvert.toString(rs.getString("MFiveClassify"));//modify by ycsun 2008/11/07 防止产生空指针异常
			sAFiveClassify = DataConvert.toString(rs.getString("AFiveClassify"));
			sManageStatFlag = DataConvert.toString(rs.getString("ManageStatFlag")); //计提标志 1为组合，2为逐笔
	}
	rs.getStatement().close();
	//modify by ycsun 2008/11/07 
	if("1".equals(sManageStatFlag)){
		//组合计提显示组合计提数据库中相关字段
		doTemp.setVisible("MBadLastPrdDiscount,MBadPrdDiscount,MBadReserveSum,MBadMinusSum,MBadRetSum,MBadReserveBalance",false);
		doTemp.setVisible("ABadLastprdDiscount,ABadPrdDiscount,ABadReserveSum,ABadMinusSum,ABadRetSum,ABadReserveBalance",false);
	}else{
	    //逐笔计提隐藏正常计算数据
		doTemp.setVisible("MNormalReserveSum,MNormalMinusSum,MNormalReserveBalance,ANormalReserveSum,ANormalMinusSum,ANormalReserveBalance",false);
	    //逐笔计提（对于五级分类为正常进行逐笔计提时，其计算的值保存在带有bad的字段中，此时候显示时head要改为正常）
