			...
			if(sVouchType.length()>=3) 
			{
				...
				if(sVouchType.substring(0,3).equals("010"))
				{
		        	...
		        	while(rs.next())
		        	{
		            	...
		            	//检查保证人是否存在黑名单中
						sCount = Sqlca.getString("select count(SerialNo) from CUSTOMER_SPECIAL where CustomerID = '"+sGuarantorID+"' and SpecialType = '40' ");
						if( sCount != null && Integer.parseInt(sCount,10) > 0  )
						{
							sbTips.append( "保证人["+sGuarantorName+"]属于黑名单客户；"+"\r\n" );
						}	            	
