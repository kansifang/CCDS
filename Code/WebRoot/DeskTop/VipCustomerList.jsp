...
<%
	...
	if (sOrgId==null) sOrgId = CurOrg.OrgID;
...
<%
	sSql = "select sum(Balance) as InBalance,sum(OverDueBalance+DullBalance+BadBalance) as InBadBalance "+
			...
			" where (BusinessType like '1%' or BusinessType = '9010') and ManageOrgID in (select OrgID from ORG_INFO where SortNo like '"+sSortNo+"%')";
	...
	sSql = "select sum(Balance) as OutBalance"+
			" from BUSINESS_CONTRACT "+
			" where BusinessType like '2%'  and ManageOrgID in (select OrgID from ORG_INFO where SortNo like '"+sSortNo+"%') ";
