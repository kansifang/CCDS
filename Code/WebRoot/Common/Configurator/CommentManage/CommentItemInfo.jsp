...
<%
	...
	String sObjectType =  DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ObjectType",10));
	String sObjectNo =  DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ObjectNo",10));
	...
	{	
		if(sObjectType!=null && sObjectType.equals("ComponentDefinition"))
			sDefSortNo=Sqlca.getString("select OrderNo from REG_COMP_DEF where CompID='"+sObjectNo+"'");
		...
		ASResultSet rsTemp ;
		sSql = "select Count(*) from reg_comment_item where SortNo = '"+sDefSortNo+"'" ;
		rsTemp = Sqlca.getASResultSet(sSql) ;
		rsTemp.next();
		int iCountTmp = rsTemp.getInt(1);
		if(iCountTmp>0) {
			String tValue = sDefSortNo.substring(0,4) ;
			sSql = "select Max(SortNo) from reg_comment_item where Sortno Like '%"+tValue+"%'";
			ASResultSet rsTemp1 = Sqlca.getASResultSet(sSql) ;
			if(rsTemp1.next()) {
...
<%	
	...
	//doTemp.appendHTMLStyle("CommentItemID","style=cursor:hand onClick=\"javascript:parent.getCommentID()\" ");
    ...
    doTemp.appendHTMLStyle("CommentText"," onDBLClick=\"parent.editObjectValueWithScriptEditorForComment(this)\" style={width=400px;height=150px;overflow:scroll;} ");
...
/*
	...
	{
		var sReturnValue = window.showModalDialog("<%=sWebRootPath%>/GetSerialNo.jsp?sColumnName=SerialNo&TableName=Business_Duebill&rand="+randomNumber(),"", "dialogWidth:280px; dialogHeight:200px; help: no; scroll: no; status: no");
...
*/
	function saveRecord()
	{
		var sReturnValue = PopPage("/Common/Configurator/CommentManage/GetBooleanValue.jsp?sTempStr=<%=TempStr%>&rand="+randomNumber(),"", "dialogWidth:280px; dialogHeight:200px; help: no; scroll: no; status: no");
