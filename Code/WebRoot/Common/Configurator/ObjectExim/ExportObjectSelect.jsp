...
<%
	...
	String sSql;
	String sSortNo; //排序编号
	String sBelongOrgID = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("BelongOrg"));
    if (sBelongOrgID==null) sBelongOrgID="0";
%>
<%/*~END~*/%>
...
<%/*~BEGIN~不可编辑区~[Editable=false;CodeAreaID=List05;Describe=主体页面;]~*/%>
	...
	<table border="0" width="100%" height="100%" cellspacing="0" cellpadding="0" id="ListTable">
		...
		<script lauguage="javascript">
			...
			%>
				document.all("ListTitle").style.cssText += "display:none";
			<%
			}
			%>
		</script>
		...
		<tr>
		    <td class="ListDWArea">
				...
				<form method='POST'  name='customize' action="ExportObjectSelectStep2.jsp">
				...
				<tr height=1>
					...
					<tr>
					<tr><td>修改人(用户ID)：</td><td><input type="text" name="UpdateUser" id="UpdateUser" value="<%=CurUser.UserID%>" size=20  onDBLClick="this.value=''"></td><td></td><td></td><td></td></tr>
					</table>
					</td>
				</tr>
...
<%
sSql = "select ObjectType,ObjectName from OBJECTTYPE_CATALOG where SortNo like '20%' order by SortNo";
...
%>
		...
		function clearCriteria(){
			document.all('UpdateTimeStart').value='';
			document.all('UpdateTimeEnd').value='';
			document.all('UpdateUser').value='';
		}
	</script>
<%/*~END~*/%>
