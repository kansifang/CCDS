...
<%
    ...
    doTemp.WhereClause += " and (FLOW_OBJECT.ObjectType||FLOW_OBJECT.ObjectNo) "+
						  " in (select ObjectType||ObjectNo from FLOW_TASK where UserID = '"+CurUser.UserID+"' and PhaseNo='0035')"+
...
<script language=javascript>
	...
	function back()
	{
		...
		if(sBusinessType == "2010" || sBusinessType == "2020")
		{
			if(confirm("退回放贷申请人吗？"))
			{
		...
		}else if(confirm("退回放贷申请人吗？")) 
		{
			for(var iMSR = 0; iMSR < getRowCount(0) ; iMSR++)
			{
				...
				if(a == "√"){
					...
					PopPage("/Common/WorkFlow/AutoSubmitAction.jsp?ObjectType=PutOutApply&ObjectNo="+sReturn+"&FlowNo=PutOutFlow&PhaseNo=1000&PhaseAction=发回补充资料","","");
				}
			}
			alert("操作成功");
			reloadSelf();
	...
	function doPigeonHole(){
		...
		if (confirm("归档后不能再重新发送交易，请仔细核对要归档的交易申请，确定要归档该笔交易吗？")){
			sReturnStatus = PopPage("/Common/Exchange/ExchangePigeonHoleAction.jsp?SerialNoArray="+sReturn,"","resizable=yes;dialogWidth=60;dialogHeight=30;center:yes;status:no;statusbar:no");
			sReturn = sReturnStatus.split("@");
			sStatus=sReturn[0];
			sMsg = sReturn[1];
			alert(sMsg);
			reloadSelf();
	...
	function PigeonHole(){
		...
		if (confirm(sMessage)){
			sReturnStatus = PopPage("/Common/Exchange/ExchangePigeonHoleAction.jsp?SerialNoArray="+sReturnArrary,"","resizable=yes;dialogWidth=60;dialogHeight=30;center:yes;status:no;statusbar:no");
			sReturn = sReturnStatus.split("@");
			sStatus=sReturn[0];
			sMsg = sReturn[1];
			alert(sMsg);
