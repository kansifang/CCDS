...
<%
	...
	//设置必输项
	doTemp.setRequired("OccurType,OccurDate",true);
	//设置下拉框选择内容
	if(sApplyType.equals("IndependentApply"))
		doTemp.setDDDWSql("OccurType","select ItemNo,ItemName from CODE_LIBRARY where CodeNo = 'OccurType' and ItemNo <> '100' and IsInUse='1' order by SortNo");	
	if(sApplyType.equals("DependentApply"))
		doTemp.setDDDWSql("OccurType","select ItemNo,ItemName from CODE_LIBRARY where CodeNo = 'OccurType' and ItemNo ='010' and IsInUse='1'");
	//设置必输背景色
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function nextStep()
	{
		...
		//add by xhyong 根据发生方式跳到不同页面
		if(sOccurType == "030")//重组方案
		{
			...
			var sNPAReformNo = sReturn;//重组方案编号
			OpenPage("/CreditManage/CreditApply/CreditApplyCreationInfo2.jsp?ObjectType=<%=sObjectType%>&ApplyType=<%=sApplyType%>&PhaseType=<%=sPhaseType%>&FlowNo=<%=sFlowNo%>&PhaseNo=<%=sPhaseNo%>&OccurType="+sOccurType+"&OccurDate="+sOccurDate+"&NPAReformNo="+sNPAReformNo,"_self","");
		}else
		{
