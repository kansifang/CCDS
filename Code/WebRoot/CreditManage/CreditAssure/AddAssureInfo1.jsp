...
<%
	...
	//获得页面参数：对象编号
	String sObjectNo = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ObjectNo"));//合同流水号
	//将空值转化为空字符串
	if(sObjectNo == null) sObjectNo = "";
	//out.print(sObjectNo);
	//获得页面参数：担保方式、担保合同编号
    String sGuarantyType = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("GuarantyType"));
	String sSerialNo = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("SerialNo"));//担保信息编号
	//将空值转化为空字符串
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	if(!"".equals(sSerialNo))
	{
		...
		if("050".equals(sGuarantyType)||"060".equals(sGuarantyType))//抵押，质押
		{
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function initRow()
	{
		if (getRowCount(0) == 0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"BusinessCurrency","01");//币种
			bIsInsert = true;
		}
    }
