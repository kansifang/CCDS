...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List00;Describe=注释区;]~*/%>
	<%
	/*
		Author:   zwhu  2009.09.01
		Tester:
		Content: 该页面主要处理合同变更的审查审批列表
		Input Param:
		Output param:
		History Log: 
	 */
...
<script language=javascript>
	...
	/*~[Describe=提前还款取消;InputParam=无;OutPutParam=无;]~*/
	function cancel()
	{
		sContractSerialNo   = getItemValue(0,getRow(),"RelativeNo");
		sObjectType = "";
		sBusinessType = "";
		...
		//取合同的业务品种
		sReturn = RunMethod("PublicMethod","GetColValue","BusinessType,BUSINESS_CONTRACT,String@SerialNO@"+sContractSerialNo);
		if(typeof(sReturn) != "undefined" && sReturn != "") 
		{
			sReturnValue = sReturn.split('@');
			sBusinessType = sReturnValue[1];
		}
		var sTradeType = "6029";
		if(sBusinessType == '2110020' || sBusinessType == '1110027')
		{
			sReturn = RunMethod("BusinessManage","SendGD",sContractSerialNo+","+sObjectType+","+sTradeType);
