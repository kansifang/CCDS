...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{			
		...
		//检查输入的日期的逻辑
			sBeginDate = getItemValue(0,getRow(),"BeginDate");//最近到期日			
			sEndDate = getItemValue(0,getRow(),"GuarantyDate");//发行日	
			...
			{			
					...
					if (typeof(sEndDate)!="undefined" && sEndDate.length > 0)
					{
						...
						{		    
							alert('发行日期应该小于等于最近到期日期');//到期日必须晚于生效日！
							return false;		    
						}
					}
		...
		}else if(sSTempletNo=="OtherEffectsInfo"){
			sBeginDate = getItemValue(0,getRow(),"BeginDate");//购入日期		
			sEndDate = getItemValue(0,getRow(),"OwnerTime");//到期日	
			if (typeof(sBeginDate)!="undefined" && sBeginDate.length > 0)
			{			
					...
					if (typeof(sEndDate)!="undefined" && sEndDate.length > 0)
					{
						if(sEndDate <= sBeginDate)
						{		    
							alert('购入日期应该小于等于到期日期');//购入日期应该小于等于到期日期
							return false;		    
						}
					}
			...
			sBeginDate = getItemValue(0,getRow(),"BeginDate");//购入/起始日期		
			sEndDate = getItemValue(0,getRow(),"OwnerTime");//到期日	
			if (typeof(sBeginDate)!="undefined" && sBeginDate.length > 0)
			{			
					...
					if (typeof(sEndDate)!="undefined" && sEndDate.length > 0)
					{
						if(sEndDate <= sBeginDate)
						{		    
							alert('购入/起始日期应该小于等于到期日期');//购入/起始日期应该小于等于到期日期
							return false;		    
						}
					}
					
			}
		}else if(sSTempletNo=="OtherImpawnInfo"){
			sBeginDate = getItemValue(0,getRow(),"BeginDate");//购入/起始日期		
			sEndDate = getItemValue(0,getRow(),"OwnerTime");//到期日	
			if (typeof(sBeginDate)!="undefined" && sBeginDate.length > 0)
			{			
