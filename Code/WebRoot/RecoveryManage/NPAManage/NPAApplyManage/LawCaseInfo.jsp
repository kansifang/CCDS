...
<%
	...
	//获得组件参数（案件流水号、案件类型）
	String sSerialNo = DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("LawCaseSerialNo"));
	String sLawCaseType = DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("LawCaseType"));
	String sObejctNo = DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("ObjectNo"));
	String sObejctType = DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("ObjectType"));
	//将空值转化为空字符串
	if(sSerialNo == null ) sSerialNo = "";
	if(sLawCaseType == null ) sLawCaseType = "";
	if(sObejctNo == null ) sObejctNo = "";
	if(sObejctType == null ) sObejctType = "";
%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//设置比率范围
	doTemp.appendHTMLStyle("EnforcedCost"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=100 \" mymsg=\"破产清偿率的范围为[0,100]\" ");
	...
	//限制金额的范围和输入格式
	doTemp.appendHTMLStyle("AimSum"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"金额不能为负数！\" ");
	doTemp.appendHTMLStyle("Principal"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"其中：本金不能为负数！\" ");
	doTemp.appendHTMLStyle("InDebtInterest"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"表内利息不能为负数！\" ");
	doTemp.appendHTMLStyle("OutDebtInterest"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"表外利息不能为负数！\" ");
	doTemp.appendHTMLStyle("OtherCost"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"其他金额不能为负数！\" ");
	doTemp.appendHTMLStyle("LegalCost"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"我行债权申报金额不能为负数！\" ");
	//置不良贷款诉讼案件保存标志
	if("BadBizApply".equals(sObejctType))
	{
		dwTemp.setEvent("AfterUpdate","!PublicMethod.UpdateColValue(String@TempSaveFlag@2,BADBIZ_APPLY,String@SerialNo@#ObjectNo)");
	}
	//生成HTMLDataWindow
	Vector vTemp = dwTemp.genHTMLDataWindow(sSerialNo+","+sObejctNo+","+sObejctType);	
