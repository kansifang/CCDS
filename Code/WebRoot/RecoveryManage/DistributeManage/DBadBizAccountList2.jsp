...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "票据置换不良贷款台账"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%
	...
	sSql = " select '' as Status,ReturnFlag,getItemName('YesNo',ReturnFlag) as ReturnFlagName,"+
		    ...
		    " from BADBIZ_ACCOUNT "+
		    " where AccountType='020' and StateFlag='"+sStateFlag+"' ";
	...
	*/
	...
	//设置共用格式
	doTemp.setVisible("OrgChangeFlagName,UserChangeFlagName,ReturnFlagName,OrgChangeFlag,UserChangeFlag,ReturnFlag,BelongOrgID,CustomerTypeName,RelativeContractNo,CertType,RecoverOrgID,RecoverUserID,IsFinish,AccountNeedManage,BasicNeedManage",false);
	if(sDealType.substring(0,3).equals("110"))//指定不良资产管理机构
	{
	...
	//汇总			
	String[][] sListSumHeaders = {	{"Sum1","已置换本金初始余额"},
									{"Sum2","报告期清收处置本金合计"},
									{"Sum3","报告期货币资金收回"},
									{"Sum4","报告期以资抵债"},
									{"Sum5","报告期其他方式收回"},
...
<script language=javascript>
	...
	function account_Info()
	{
		...
		if (typeof(sSerialNo)=="undefined" || sSerialNo.length==0)
		{
			...
			popComp("BadBizAccountInfo","/RecoveryManage/AccountManage/BadBizAccountInfo.jsp","ComponentName=不良贷款台帐详情&SerialNo="+sSerialNo+"&StateFlag=<%=sStateFlag%>&AccountType=020&AccountEditFlag=03","","");
		}
	}
