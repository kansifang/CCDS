...
<%
	...
	sSql =  " select LawsuitStatus from LAWCASE_INFO where SerialNo = '"+sObjectNo+"' ";
	//我行的诉讼地位
	sLawsuitStatus = Sqlca.getString(sSql);
	if(sLawsuitStatus == null) sLawsuitStatus = "";
   	
   	//从破产案件基本信息中获得破产台帐的相关信息，以作为破产台帐的默认值
	if(sBookType.equals("080"))
	{
		sSql =  " select LawCaseOrg,Claim,ApplyDate from LAWCASE_INFO where SerialNo = '"+sObjectNo+"' ";
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//限制金额的范围和输入格式
	doTemp.appendHTMLStyle("JudgeSum,ApplyPrincipal,ApplyInInterest,ApplyOutInterest,ApplyOtherSum,JudgePaySum,"+
						 "ActualSum,ActualPrincipal,ActualInInterest,ActualOutInterest,ActualOtherSum ",
						 " myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"金额必须大于等于0！\" ");
	//设置比率范围
	doTemp.appendHTMLStyle("BankruptcyRate"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1000 \" mymsg=\"破产清偿率的范围为[0,1000]\" ");
	...
	//自动计算申请标的总额
	doTemp.appendHTMLStyle("ApplyPrincipal,ApplyInInterest,ApplyOutInterest,ApplyOtherSum"," onChange=\"javascript:parent.getAimSum()\" ");
	...
	//自动计算落实偿还总金额
	doTemp.appendHTMLStyle("ActualPrincipal,ActualInInterest,ActualOutInterest,ActualOtherSum"," onChange=\"javascript:parent.getActualSum()\" ");

	//根据不同的台帐类型过滤受理结果
	doTemp.setDDDWSql("CognizanceResult","select ItemNo,ItemName from CODE_LIBRARY where CodeNo = 'CognizanceResult' and Itemdescribe = '"+sItemdescribe+"' ");
	
	//根据不同的案件类型过滤生效判决书
	doTemp.setDDDWSql("JudgementNo","select ItemNo,ItemName from CODE_LIBRARY where CodeNo = 'JudgementNo' and Itemdescribe = '"+sItemdescribe1+"' ");

	//选择法院
	doTemp.setUnit("AcceptedCourt"," <input type=button class=inputDate  value=... name=button onClick=\"javascript:parent.getAgencyName()\">");
	doTemp.appendHTMLStyle("AcceptedCourt","  style={cursor:hand;background=\"#EEEEff\"} ondblclick=\"javascript:parent.getAgencyName()\" ");
	...
	if(!(sBookType.equals("068")))
	{
		...
		doTemp.appendHTMLStyle("WithdrawProposerName","  style={cursor:hand;background=\"#EEEEff\"} ondblclick=\"javascript:parent.getAgentName()\" ");
	}
	//选择主代理人
	doTemp.setUnit("CurrentAgent"," <input type=button class=inputDate  value=... name=button onClick=\"javascript:parent.getCurrentAgent()\">");
	doTemp.appendHTMLStyle("CurrentAgent","  style={cursor:hand;background=\"#EEEEff\"} ondblclick=\"javascript:parent.getCurrentAgent()\" ");
	
	
	//*****The below altered by FSGong 2005-03-01
	//设置比率范围
	doTemp.appendHTMLStyle("BankruptcyDischargeRate"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1000 \" mymsg=\"破产清偿率的范围为[0,1000]\" ");
	if (sBookType.equals("070"))	//执行台帐
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	/*~[Describe=破产清偿率校验;InputParam=date1,data2,rule;OutPutParam=无;]~*/
	function checkBankruptcyRate()
	{
		dBankruptcyRate = getItemValue(0,getRow(),"BankruptcyRate");
		if(dBankruptcyRate == "" || typeof(dBankruptcyRate) == "undefined") return true;
		if(dBankruptcyRate < 0 || dBankruptcyRate > 1000)
		{
			alert("破产清偿率超出0~1000的范围");
			return;
		}
		return true;
	}
	...
	function checkDate()
	{	
		sAppDate = getItemValue(0,getRow(),"AppDate");//申请日期
		sAcceptedDate = getItemValue(0,getRow(),"AcceptedDate");//受理日期
		sApproveDate = getItemValue(0,getRow(),"ApproveDate");//开庭日期
		sJudgedDate = getItemValue(0,getRow(),"JudgedDate");//判决日期
		sJudgementDate = getItemValue(0,getRow(),"JudgementDate");//判决书送达日期
		sIntoEffectDate = getItemValue(0,getRow(),"IntoEffectDate");//判决书生效日期
		
		//立案台帐
		if("<%=sTempletNo%>" == "BeforeLawsuitInfo")
		{
			if(compareDate(sAppDate,sAcceptedDate,"1")) 
			{
				alert("申请日期晚于受理日期！");
				return;
			}
		...
		//一审台帐,二审台帐
		if("<%=sTempletNo%>" == "FirstLawsuitInfo" || "<%=sTempletNo%>" == "SecondLawsuitInfo")
		{
			if(compareDate(sAppDate,sAcceptedDate,"1")) 
			{
				alert("申请日期晚于受理日期！");
			...
			if(compareDate(sAcceptedDate,sApproveDate,"1")) 
			{
				alert("受理日期晚于首次开庭日期！");
				return;
			}
			if(compareDate(sApproveDate,sJudgedDate,"1")) 
			{
				alert("首次开庭日期晚于判决日期！");
				return;
			}
			if(compareDate(sJudgedDate,sJudgementDate,"1")) 
			{
				alert("判决日期日期晚于判决书送达日期！");
				return;
			}
			if(compareDate(sJudgedDate,sIntoEffectDate,"1")) 
			{
				alert("判决日期日期晚于判决书生效日期！");
				return;
			}
		}
		//再审台帐
		if("<%=sTempletNo%>" == "LastLawsuitInfo")
		{
			if(compareDate(sAppDate,sAcceptedDate,"1")) 
			{
				alert("申请日期晚于再审立案日期！");
			...
			if(compareDate(sAcceptedDate,sJudgedDate,"1")) 
			{
				alert("再审立案日期晚于判决日期！");
				return;
			}
		...
		if("<%=sTempletNo%>" == "ArbitrateInfo")
		{
			...
			if(compareDate(sAcceptedDate,sJudgementDate,"1")) 
			{
				alert("受理日期晚于公证下达日期！");
				return;
			}
		...
		//仲裁台帐
		if("<%=sTempletNo%>" == "NotarizationInfo")
		{
			if(compareDate(sAppDate,sAcceptedDate,"1")) 
			{
				alert("仲裁申请日期晚于受理日期！");
	...
	//判决金额由四项合计获得：其中，判决本息可能是外币，需要转换为人民币合计。其他均为人民币。
     function getConfirmSum()
     {
		...
		var sReturn = PopPage("/RecoveryManage/PDAManage/PDADailyManage/PDAGetRMBExchangeRateDialog.jsp?ReclaimCurrency="+sConfirmPICurrency,"","re				sizable=yes;dialogWidth=0;dialogHeight=0;center:no;status:no;statusbar:no");
	...
	function getAimSum()
     	{ 
	       ...
	       if(typeof(sApplyOtherSum)=="undefined" || sApplyOtherSum.length==0) sApplyOtherSum=0; 
			...
			var sReturn = PopPage("/RecoveryManage/PDAManage/PDADailyManage/PDAGetRMBExchangeRateDialog.jsp?ReclaimCurrency="+sCurrency,"","re				sizable=yes;dialogWidth=0;dialogHeight=0;center:no;status:no;statusbar:no");
			sReturn = sReturn.split("@");
			sRatio=sReturn[0];
