...
<%@page import="com.amarsoft.web.config.definition.*"%>
<%@page import="com.amarsoft.web.config.loader.*"%>
...
<%
    ...
    try
	{
		...
		if(sOpenerClientID==null && sParentClientID!=null) sOpenerClientID = sParentClientID;
		...
		if(sToDestroyClientID!=null && !sToDestroyClientID.equals("") && !sToDestroyClientID.equals("undefined")){
			...
			if(destoyedComp!=null) parentOfDestroyedComp = destoyedComp.compParentComponent;
	    ...
	    if(oTmpDef==null){
			...
			{
				//自动注册该组件,
				if(sComponentURL==null) throw new Exception("请在第一次调用组件时，传入URL，格式：ComponentURL=/Main.jsp");
		...
		if(CurCompParent!=null) CurComp.compParentComponent = CurCompParent;
		else if(parentOfDestroyedComp!=null) CurComp.compParentComponent = parentOfDestroyedComp;
