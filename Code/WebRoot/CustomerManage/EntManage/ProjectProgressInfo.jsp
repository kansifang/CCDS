...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//设置已投资金额(元)范围
	doTemp.appendHTMLStyle("INVESTEDSUM"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"已投资金额(元)必须大于等于0！\" ");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{			
		//校验调查日期是否大于当前日期
		sUpToDate = getItemValue(0,0,"UPTODATE");//调查日期
		sToday = "<%=StringFunction.getToday()%>";//当前日期		
		if(typeof(sUpToDate) != "undefined" && sUpToDate != "" )
		{
			if(sUpToDate >= sToday)
			{		    
				alert(getBusinessMessage('155'));//调查日期必须早于当前日期！
				return false;		    
			}
		}		
