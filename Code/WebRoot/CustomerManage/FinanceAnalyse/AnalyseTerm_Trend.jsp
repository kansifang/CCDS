...
<%
	...
	String sFinanceBelong = "", sIndustryType = "";
...
<%
	int num = 0;
	//sSql = "select AccountMonth || ' ' || getItemName('FinanceScope',Scope) || ' ' || getItemName('FinancePeriod',Period) || ' ' || getItemName('FinanceCurrency',Currency) || ' ' || getItemName('FinanceUnit',CurrencyUnit),AccountMonth || '@' || Scope from Finance_Desc where CustomerID = '" + sCustomerID + "' order by Scope,AccountMonth,Period";
	sSql = " select ReportDate || ' ' || getItemName('ReportScope',ReportScope) || ' ' || getItemName('ReportPeriod',ReportPeriod) || ' ' || getItemName('Currency',ReportCurrency) || ' ' || getItemName('ReportUnit',ReportUnit)," +
			" ReportDate || '@' || ReportScope " +
			" from customer_fsrecord "+
			" where CustomerID = '" + sCustomerID + "' order by ReportScope,ReportDate,ReportPeriod";
	...
	<tr>
		<td>
			<table width='100%' border='1' cellpadding='0' cellspacing='5' bgcolor='#DDDDDD'>
				<tr>
					<td colspan="4">
						<span>选取要分析的财务报表(所选第一期报表作为基期)</span>
					</td>
				</tr>
				<tr>
					<td bgcolor='#DDDDDD'>
