...
<%
	...
	//System.out.println("....page...."+CurPage.getParameter("CustomerID"));
	//System.out.println("....comp...."+CurComp.getParameter("CustomerID"));
	...
	*/	
	...
	ASResultSet rsEnts=Sqlca.getASResultSet("select CustomerName from CUSTOMER_INFO where CustomerID = '" + sCustomerID + "'");
	if (rsEnts.next()) 
...
<%
	sSql = " select FM.ReportNo,FC.ReportName,FI.ItemNo,FI.ItemName "+
			...
			" and FM.ReportNo in ( select distinct ReportNo from FINANCE_RECORD where CustomerID = '" + sCustomerID + "') "+
	...
	rs = Sqlca.getASResultSet("select Accountmonth from FINANCE_RECORD where Customerid='" + sCustomerID + "' order by Accountmonth");
	num=0;
...
<td width='100%' colspan='2'>
	...
	<tr>
		...
		统计日期：<input name=startTime type=text size=10 value="1999/12" onclick="javascript:selectStartMonth(this.name)"  readonly style="cursor:hand">
		至&nbsp;<input name=endTime type=text size=10 value="2000/03" onclick="javascript:selectEndMonth(this.name)" readonly style="cursor:hand">
		</td>
