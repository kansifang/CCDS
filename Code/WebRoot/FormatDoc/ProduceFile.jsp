...
%>
	<script language=javascript >
		PopPage("<%=sJspFileName%>?DocID=<%=sDocID%>&ObjectNo=<%=sObjectNo%>&ObjectType=<%=sObjectType%>&SerialNo=<%=rs.getString("serialno")%>&CustomerID=<%=sCustomerID%>&Method=4&FirstSection=<%=sFirstSection%>&Attribute=<%=sAttribute%>&rand="+randomNumber(),"myprint10","dialogWidth=10;dialogHeight=1;status:no;center:yes;help:no;minimize:yes;maximize:no;border:thin;statusbar:no");
	</script>
<%    
			sFirstSection = "0";
		}
