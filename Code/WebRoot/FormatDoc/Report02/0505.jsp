...
<%
	...
	String sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportdate = '"+sYearN+"') and ReportScope = GETEVALUATESCOPE('"+sCustomerID+"','"+sYearN+"')";
	...
	sSql = " select * from REPORT_DATA where reportno in(select reportno from REPORT_RECORD"+
		   " where objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportdate = '"+sYearN_1+"') and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"','"+sYearN_1+"')";
	...
	sSql = " select * from REPORT_DATA where reportno in(select reportno from REPORT_RECORD"+
		   " where objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportdate = '"+sYearN_2+"') and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"','"+sYearN_2+"')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("Rowsubject");
...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >5.5、盈利能力分析 </font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
