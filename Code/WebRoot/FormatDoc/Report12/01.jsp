...
<%
	...
	String sSql = "select CustomerID,CustomerName,getBusinessName(BusinessType) as BusinessTypeName,BusinessType,"+
				" getItemname('YesNo',ContractFlag) as ContractFlag,TermMonth,BailRatio,"+
	...
	sSql = "select getitemname('currency',businesscurrency),sum(balance) "+ 
		   ...
		   "and (businesstype like '1%' or businesstype = '9010') "+
	...
	sSql = "select getitemname('currency',businesscurrency),sum(balance-isnull(bailsum,0)) as balance "+ 
		   "from BUSINESS_CONTRACT where customerid = '"+sCustomerID+"' and (businesstype like '2%' ) "+
		   "and balance >0 and (finishdate = '' or finishdate is null) group by businesscurrency ";
	rs2 = Sqlca.getResultSet(sSql);
	String bwBalance = "&nbsp;";
	while(rs2.next())
...
<%
	...
	sTemp.append("   <td class=td1 colspan=8 bgcolor=#aaaaaa ><font style=' font-size: 16pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >天津农商银行审查审批意见</font></td>"); 	
	sTemp.append("   </tr>");
	...
	sTemp.append("   <td width=16% align=center class=td1 ><font style=' font-size: 12pt;FONT-FAMILY:宋体;'>金额(万元)</font></td>");
