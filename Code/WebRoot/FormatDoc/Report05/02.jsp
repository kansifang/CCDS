...
<%
	...
	ASResultSet rs = Sqlca.getResultSet("select CustomerName,getitemname('Currency',BusinessCurrency) as CurrencyName from BUSINESS_APPLY where SerialNo='"+sObjectNo+"'");
	if(rs.next())
	{
		sCustomerName = rs.getString(1);
		if(sCustomerName == null) sCustomerName = "&nbsp;";

		sCurrency = rs.getString(2);
		if(sCurrency == null) sCurrency = " ";
	}
	rs.getStatement().close();
	
	rs = Sqlca.getResultSet("select CreditLevel,LicenseNo,CorpID,RegisterCapital,PaiclUpCapital,MostBusiness from ENT_INFO where CustomerID='"+sCustomerID+"'");
	if(rs.next())
	{
		sCreditLevel = rs.getString(1);
		if(sCreditLevel == null) sCreditLevel = " ";

		sLicenseNo = rs.getString(2);
		if(sLicenseNo == null) sLicenseNo = " ";
		
		sCorpID = rs.getString(3);
		if(sCorpID == null) sCorpID = " ";

		sRegisterCapital = DataConvert.toMoney(rs.getDouble(4)/10000);
		
		sPaiclUpCapital = DataConvert.toMoney(rs.getDouble(5)/10000);

		sMostBusiness = rs.getString(6);
		if(sMostBusiness == null) sMostBusiness = " ";
	}
	rs.getStatement().close();

	rs = Sqlca.getResultSet("select count(*) from Business_Contract where CustomerID='"+sCustomerID+"'");
	if(rs.next())
	{
		 if(rs.getInt(1)>1)sNewCustomer = "否";
		 else sNewCustomer = "是";
	}
	rs.getStatement().close();

	rs = Sqlca.getResultSet("select count(*) from CUSTOMER_SPECIAL where SpecialType in('010','020') and CustomerID = '"+sCustomerID+"'");
	if(rs.next()) 
	{
		if(rs.getInt(1) >=1 ) sIsWarner = "是";
		else sIsWarner = "否";
	}
	rs.getStatement().close();
...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=17 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >2、申请人信息</font></td>"); 	
	sTemp.append("   </tr>");
