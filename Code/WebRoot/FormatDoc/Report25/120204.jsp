...
<%
	...
	sTemp.append("<form method='post' action='120204.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("<tr>");	
	sTemp.append("<td class=td1 align=left colspan=16 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >"+sNo1[i]+".4银企合作关系</font></td>"); 		
	sTemp.append("<tr>");
	sTemp.append("<tr>");
    sTemp.append("<td colspan='4' align=left class=td1 nowrap > 币种：（折）人民币 单位：万元 </td>");
	sTemp.append("</tr>");	
  	sTemp.append("<td width=20% align=left class=td1 > 基本账户开户行  </td>");
    sTemp.append("<td width=35% align=left class=td1 >"+sBasicBanck+"&nbsp;</td>");
    sTemp.append("<td width=10% align=left class=td1 > 账号 </td>");
    sTemp.append("<td width=35% align=left class=td1 >"+sBasicAccount+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
  	sTemp.append("<td width=20% align=left class=td1 > 其他主要开户行名称</td>");
    sTemp.append("<td width=30% align=left class=td1 >"+sOtherBank+"&nbsp;</td>");
    sTemp.append("<td width=20% align=left class=td1 > 账号 </td>");
    sTemp.append("<td width=30% align=left class=td1 >"+sOtherBankAccount+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
  	sTemp.append("<td width=20% align=left class=td1 > 我行开户时间  </td>");
    sTemp.append("<td width=30% align=left class=td1 >"+sMyBank+"&nbsp;</td>");
    sTemp.append("<td width=20% align=left class=td1 > 账号 </td>");
    sTemp.append("<td width=30% align=left class=td1 >"+sAccountDate+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
    sTemp.append("<td width=20% align=left class=td1 nowrap > 与我行建立信贷关系日期 </td>");
