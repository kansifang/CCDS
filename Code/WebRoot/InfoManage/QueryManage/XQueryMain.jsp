...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Main03;Describe=定义树图;]~*/%>
	...
	if(sDBName.startsWith("DB2"))
	{
		sSqlTreeView = "from CODE_LIBRARY where CodeNo= 'XQueryListnew' and IsInUse='1' "+
	          " and (exists (select UR.RoleID from USER_ROLE UR where locate(CODE_LIBRARY.Attribute1,UR.RoleID)>0 or locate(CODE_LIBRARY.Attribute1,UR.RoleID)>0 and UR.UserID='"+CurUser.UserID+"' )  or  Attribute1='' or Attribute1 is null or Attribute1='All' ) "+
	          " and not exists (select UR.RoleID from USER_ROLE UR where locate(CODE_LIBRARY.Attribute2,UR.RoleID)>0 or locate(CODE_LIBRARY.Attribute2,UR.RoleID)>0 and UR.UserID='"+CurUser.UserID+"' )  and (Attribute2<>'All' or Attribute2='' or Attribute2 is null) " ;
	}else 
	{
		sSqlTreeView = "from CODE_LIBRARY where CodeNo= 'XQueryListnew' and IsInUse='1' "+
