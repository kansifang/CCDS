...
public class AnalyzeFinaceStructure extends Bizlet {
	...
	private Properties getReportData(Transaction Sqlca,String sCustomerID,String sFSAccountMonth,String sReportScope){
		...
		if(ReportData==null||bNeedGetData){
			...
			try{
				ASResultSet rs=Sqlca.getASResultSet("SELECT RowSubject,nvl(Col2Value,0)as Col2Value FROM REPORT_DATA where ReportNo in (select ReportNo from REPORT_RECORD where ObjectNo ='"+sCustomerID+"' and ReportDate='"+sFSAccountMonth+"' and ReportScope = '"+sReportScope+"')");
				while(rs.next()){
	...
	private double getRefundScore(){
		double ActualScore=0.0;
		//获取流动比率得分
		ActualScore+=getItemScore("915",4,0,1.5,1,8,Minus);
		//获取速动比率得分
	...
	private double getManagementSocre(){
		double ActualScore=0.0;
		//获取主营收入现金率得分
		ActualScore+=getItemScore("934",4,0,1,0.5,8,Minus);
		//获取应收帐款周转速度得分
	...
	private double getBenefitScore(){
		double ActualScore=0.0;
		//获取毛利率得分
		ActualScore+=getItemScore("931",2,0,0.2,0.04,12.5,Minus);
		//获取营业利润率得分
		ActualScore+=getItemScore("902",3,0,0.1,0.02,37.5,Minus);
		//获取净资产收益率得分
		//获取净资产
		double ActualAssetsValue=getItemValue("808");
		if(ActualAssetsValue<1000000000.0)
		...
		if(ActualAssetsValue<1000000000)
			ActualScore+=getItemScore("909",3,0,0.06,0.01,60,Minus);
		else if(ActualAssetsValue>=1000000000.0&&ActualAssetsValue<2000000000.0)
			ActualScore+=getItemScore("909",3,0,0.04,0.01,10,Minus);
		else
			ActualScore+=getItemScore("909",3,0,0.03,0.01,150,Minus);
		return ActualScore;
	}
	//获取财务状况得分
	private double getTotalFinaceScore(){
		double ActualScore=0.0;
		//获取财务结构得分
		ActualScore+=getFinaceScore();
		//获取偿债能力得分
		ActualScore+=getRefundScore();
		//获取经营能力得分
		ActualScore+=getManagementSocre();
		//获取经营效益得分
		ActualScore+=getBenefitScore();
		//获取有形长期资产得分
