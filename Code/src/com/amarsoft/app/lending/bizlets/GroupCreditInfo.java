...
public class GroupCreditInfo {
   ...
   public double getRate(String sCurrency, String sCurrency2, Transaction Sqlca) throws Exception {
      ...
      String s4 = " Select nvl(Price3,0) as Price3 From ERATE_INFO Where  Efficientdate =(select max(Efficientdate ) from ERATE_INFO ) And Currency  = '" +
        sCurrency + "' ";
      String s5 = " Select nvl(Price3,0) as Price3 From ERATE_INFO Where  Efficientdate =(select max(Efficientdate ) from ERATE_INFO )  And  Currency  = '" +
        sCurrency2 + "' ";
      try {
         ASResultSet asresultset = Sqlca.getASResultSet(s4);
         if (asresultset.next())
            d1 = asresultset.getDouble("Price3");
         asresultset.getStatement().close();
         ASResultSet asresultset1 = Sqlca.getASResultSet(s5);
   ...
   public  void AggregateCredit(String sGroupNo, Transaction Sqlca) throws Exception {
      ...
      int i = 0, j = 0, k = 0, m = 0, n = 0, p = 0, iCount = 0, iCount2 = 0,
        u = 0;

      sSql =
        " select getcustomername(CustomerID) as  customername from AGGREGATE_MEMBER " +
        " where AggregateNo='" + sGroupNo + "' and Attribute1='1' ";
      System.out.println("AGGREGATE_MEMBER    sSql====" + sSql);
      ...
      sSql = " select Attribute2 " +
        " from AGGREGATE_MEMBER" +
        " where  AggregateNo='" + sGroupNo + "' ";
      rs = Sqlca.getASResultSet(sSql);
      while (rs.next()) {
      ...
      if (sAttribute2.equals("1")) {
         ...
         sSql = " select SerialNo," +
           " getErate(BusinessCurrency,'01','" + StringFunction.getToday() +
           "')*nvl(BusinessSum,0)" +
           " as BusinessSum" +
           " from BUSINESS_APPROVE where  CustomerID in" +
           " (select CustomerID from AGGREGATE_MEMBER where " +
           " AggregateNo='" + sGroupNo + "' and Attribute1='1') " +
           " and CreditMode='1' and phaseno='1000' and nvl(CreditType,'01')!='11' and approveflag='1' and nvl(effectflag,'01') = '01' order by putoutdate ";
         System.out.println("集团公司    sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         //取内部批复流水号，批复金额币种
         while (rs.next()) {
         ...
         sSql = " select getBusinessName(BusinessType) as BusinessType,getItemName('Currency',Currency) as Currency," +
           ...
           " from CREDIT_INFO where  Objectno='" + sSerialNo +
           "' and ObjectType='BusinessApprove'" +
           " group by  BusinessType,Currency order by Currency";
         System.out.println("CREDIT_INFO    sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         while (rs.next()) {
            sBusinessType = rs.getString("BusinessType");
            sCurrency = rs.getString("Currency");
            sCreditSum = DataConvert.toMoney(rs.getDouble("CreditSum") /
                                             10000.0);
      ...
      //集团成员
      sSqldata =
        " select CustomerID,getCustomerName(CustomerID) as CustomerName, " +
        " MemberDescrible from AGGREGATE_MEMBER" +
        " where  AggregateNo='" + sGroupNo +
        "' and Attribute1='0' order by CustomerID";
      System.out.println("集团成员:   sSqldata====" + sSqldata);
      rsdata = Sqlca.getASResultSet(sSqldata);
      //集团成员循环
      while (rsdata.next()) {
         ...
         sSql = " select SerialNo,Describe6,Maturity from BUSINESS_APPROVE WHERE CreditMode='1' " +
           " and CustomerID='" + sCustomerID + "' and " +
           " ( MONTHS_BETWEEN(TO_DATE('" + StringFunction.getToday() +
           "','YYYY/MM/DD')," +
           " TO_DATE(PUTOUTDATE,'YYYY/MM/DD'))<=3 or PUTOUTDATE is null) " +
           " and phaseno='1000' and nvl(CreditType,'01')!='11' and approveflag='1' " +
           " and nvl(effectflag,'01') = '01' order by maturity ";
         System.out.println("起始日三个月内的有效综合授信批复sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         while (rs.next()) {
         ...
         if (sDescribe6.equals("020")) {
            sSql =
              " select count(SerialNo) from BUSINESS_CONTRACT where  CustomerID='" +
              sCustomerID + "' " +
              " and nvl(CreditType,'01')!='11' " +
              " and CreditMode='2' and phaseno='1000' and CreditAggreeMent='" +
              sObjectNo + "' " +
            ...
            if (sMaturity.compareTo(StringFunction.getToday()) >= 0 ||
               ...
               if (rs.next()) {
                  ...
                  BusinessSum[j][i] = DataConvert.toMoney(rs.getDouble(
                    "BusinessSum") / 10000.0);
                  ...
                  dApproveTotal[u] = dApproveTotal[u] +
                    rs.getDouble("BusinessSum") *
                    getRate(rs.getString("BusinessCurrency"), "01", Sqlca);

                  FlagBool = "0";
                  for (m = 0; m < k; m++) {
                     if (rs.getString("BusinessCurrency").equals(CurrencyTotal[
                       m])) {
                        dBusinessSum[m] = dBusinessSum[m] +
                          rs.getDouble("BusinessSum");
                        FlagBool = "1";
                     }
                  }
               ...
               System.out.println("综合授信协议对应的批复或综合授信批复授信品种明细 sSql====" + sSql);
               rs = Sqlca.getASResultSet(sSql);
               while (rs.next()) {
                  sBusinessType = rs.getString("BusinessType");
                  ...
                  CreditSum[j][i] = DataConvert.toMoney(rs.getDouble(
                    "CreditSum") / 10000.0);
                  ...
                  if (!sBusinessType.equals(sBusinessTypeLevel)) {
                     sSqldata3 =
                       ...
                       " sum(getErate(BusinessCurrency,'" + sCurrency + "','" +
                       StringFunction.getToday() + "')*" +
                       " nvl(BusinessSum,0)) as BusinessSum," +
                       " sum(getErate(BusinessCurrency,'" + sCurrency + "','" +
                       StringFunction.getToday() + "')*" +
                       " (nvl(Balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0))) as Balance " +
                       " from BUSINESS_CONTRACT where  CustomerID='" +
                       sCustomerID + "'  and nvl(CreditType,'01')!='11' " +
                       " and BusinessType like '" + sBusinessType +
                       "%' and (BusinessCurrency in" +
                       " (select ItemNo from Code_Library where codeno='Currency' " +
                       " minus select Currency from Credit_Info where Objectno='" +
                       sObjectNo + "' " +
                       " and ObjectType='BusinessApprove' and BusinessType ='" +
                       sBusinessType + "') or BusinessCurrency ='" + sCurrency +
                       "')" +
                     ...
                     sSqldata3 =
                       ...
                       sCustomerID + "'  and nvl(CreditType,'01')!='11' " +
                       " and BusinessType like '" + sBusinessType +
                       "%' and BusinessCurrency='" + sCurrency + "'" +
                       " and CreditMode='2' and phaseno='1000' " +
                       " and (Maturity>='" + StringFunction.getToday() + "' or  Maturity is null or nvl(balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0)>0)" +
                       " and CreditAggreement='" + sObjectNo + "'  " +
                       " group by BusinessCurrency  ";
                  }
                  System.out.println("取合同金额BUSINESS_APPROVE    sSqldata3====" +
                                     sSqldata3);
                  rsResult = Sqlca.getASResultSet(sSqldata3);
                  if (rsResult.next()) {
                     sBusinessSum3[j][i] = DataConvert.toMoney(rsResult.
                       getDouble("BusinessSum") / 10000.0);
                     sBalance3[j][i] = DataConvert.toMoney(rsResult.getDouble(
                       "Balance") / 10000.0);
                  ...
                  */
                  ...
                  temp.append("<td class=td1 align=left  colspan=1 >" +
                              sCustomerName + "&nbsp;</td>");
                  temp.append(
                    "<td class=td1 align=left  colspan=1 >&nbsp;</td>");
                  temp.append("<td class=td1 align=left  colspan=1 >" +
                              BusinessType[j][i] + "&nbsp;</td>");
            ...
            //公开批复
            sSql =
              " select count(*) from BUSINESS_CONTRACT where  CustomerID='" +
              sCustomerID + "' " +
              " and nvl(CreditType,'01')!='11' " +
              " and CreditMode='1' and phaseno='1000' and RelativeSerialNo='" +
              sObjectNo + "' ";
            System.out.println("公开批复 sSql" + sSql);
            rs = Sqlca.getASResultSet(sSql);
            if (rs.next()) {
               iCount6 = rs.getInt(1);
            }
            ...
            if (iCount6 > 0) {
               sSql =
                 " select SerialNo,Maturity from BUSINESS_CONTRACT where  CustomerID='" +
                 sCustomerID + "' " +
                 " and nvl(CreditType,'01')!='11' " +
                 " and CreditMode='1' and phaseno='1000' and RelativeSerialNo='" +
                 sObjectNo + "' ";
               System.out.println("614 sSql" + sSql);
               rs = Sqlca.getASResultSet(sSql);
               if (rs.next()) {
                  sSerialNo = rs.getString("SerialNo");
            ...
            if (iCount6 == 0) {
               sSql = " select BusinessCurrency,getItemName('Currency',BusinessCurrency) as Currency2," +
                 ...
                 " from BUSINESS_APPROVE BA where  SerialNo='" + sObjectNo +
                 "' ";
               System.out.println("批复下没有协议 sSql" + sSql);
               rs = Sqlca.getASResultSet(sSql);
               if (rs.next()) {
                  sCurrency = rs.getString("Currency2");
                  sBusinessCurrency = rs.getString("BusinessCurrency");
               ...
               sSql = " select BusinessType,getBusinessName(BusinessType) as BusinessType2,Currency,getItemName('Currency',Currency) as Currency2," +
                 ...
                 "' and ObjectType='BusinessApprove'" +
                 " group by  BusinessType,Currency order by BusinessType,Currency ";
               System.out.println("综合授信协议对应的批复授信品种明细    sSql====" + sSql);
               rs = Sqlca.getASResultSet(sSql);
               while (rs.next()) {
                  sBusinessType = rs.getString("BusinessType");
                  BusinessType[j][i] = rs.getString("BusinessType2");
                  ...
                  temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                              CreditSum[j][i] + "</td>");
                  temp.append(
                    "<td class=td1 align=middle  colspan=1 >&nbsp</td>");
                  temp.append(
                    "<td class=td1 align=right  colspan=1 >&nbsp;</td>");
                  temp.append(
                    "<td class=td1 align=right  colspan=1 >&nbsp;</td>");
               ...
               if (sMaturity2.compareTo(StringFunction.getToday()) >= 0 ||
                  ...
                  if (rs.next()) {
                     ...
                     FlagBool = "0";
                     for (m = 0; m < k; m++) {
                        if (rs.getString("BusinessCurrency").equals(
                          CurrencyTotal[m])) {
                           dBusinessSum[m] = dBusinessSum[m] +
                             rs.getDouble("BusinessSum");
                           FlagBool = "1";
                     ...
                     temp.append("<td class=td1 align=left  colspan=1 >" +
                                 sCustomerName + "&nbsp;</td>");
                     temp.append(
                       "<td class=td1 align=left  colspan=1 >综合授信</td>");
                     temp.append(
                       "<td class=td1 align=left  colspan=1 >授信额度</td>");

                     temp.append("<td class=td1 align=left  colspan=1 >&nbsp;" +
                                 sObjectNo + "</td>");
                     temp.append(
                       "<td class=td1 align=middle  colspan=1 >&nbsp;" +
                       sCurrency + "</td>");
                     temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                                 BusinessSum[j][i] + "</td>");
                     temp.append(
                  ...
                  sSql = " select BusinessType,getBusinessName(BusinessType) as BusinessType2,Currency,getItemName('Currency',Currency) as Currency2," +
                    ...
                    " group by  BusinessType,Currency order by BusinessType,Currency ";
                  System.out.println(
                    "综合授信协议对应的批复或综合授信批复授信品种明细CREDIT_INFO    sSql====" + sSql);
                  rs = Sqlca.getASResultSet(sSql);
                  while (rs.next()) {
                     sBusinessType = rs.getString("BusinessType");
                     BusinessType[j][i] = rs.getString("BusinessType2");
                     ...
                     if (!sBusinessType.equals(sBusinessTypeLevel)) {
                        sSqldata3 =
                          ...
                          " and CreditMode='2' and phaseno='1000' " +
                          " and (Maturity>='" + StringFunction.getToday() + "' or  Maturity is null or nvl(balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0)>0)" +
                          " and CreditAggreement in(select serialno  from business_contract where relativeserialno='" +
                          sObjectNo + "' and creditmode='1')  " +
                          " group by BusinessCurrency";
                     }
                     else {
                     ...
                     System.out.println(
                       "取合同金额BUSINESS_APPROVE    sSqldata3====" + sSqldata3);
                     rsResult = Sqlca.getASResultSet(sSqldata3);
                     if (rsResult.next()) {
                        sBusinessSum3[j][i] = DataConvert.toMoney(rsResult.
                          getDouble("BusinessSum") / 10000.0);
                        sBalance3[j][i] = DataConvert.toMoney(rsResult.
                          getDouble("Balance") / 10000.0);
                        /*FlagBool = "0";
                        for (m = 0; m < p; m++) {
                           if (rsResult.getString("BusinessCurrency2").equals(
                             CurrencyTotal5[m])) {
                     ...
                     */
                     ...
                     temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                                 CreditSum[j][i] + "</td>");
                     if (sBusinessSum3[j][i].equals("") &&
                         sBalance3[j][i].equals("")) {
                        temp.append(
                          "<td class=td1 align=middle  colspan=1 >&nbsp;</td>");
                     }
                     else {
         ...
         sSql = " select SerialNo,getBusinessName(BusinessType) as BusinessType,BusinessCurrency as BusinessCurrency2," +
           " getItemName('Currency',BusinessCurrency) AS BusinessCurrency," +
           " nvl(BusinessSum,0) as BusinessSum" +
           " from BUSINESS_APPROVE BA where  CustomerID='" + sCustomerID +
           "'  and nvl(CreditType,'01')!='11' " +
           " and CreditMode='2' and phaseno='1000' and approveflag='1' " +
           " and ( MONTHS_BETWEEN(TO_DATE('" + StringFunction.getToday() +
           "','YYYY/MM/DD')," +
           " TO_DATE(PUTOUTDATE,'YYYY/MM/DD'))<=3 or PUTOUTDATE is null ) " +
           " and ( Maturity>='" + StringFunction.getToday() +
           "' or Maturity is null) ";
         System.out.println("单笔单批批复批复有效  sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         sName3 = "单笔单批";
         while (rs.next()) {
            sObjectNo = rs.getString("SerialNo");
            ...
            BusinessSum[j][i] = DataConvert.toMoney(rs.getDouble("BusinessSum") /
              10000.0);
            ...
            dApproveTotal[u] = dApproveTotal[u] +
              rs.getDouble("BusinessSum") *
              getRate(rs.getString("BusinessCurrency2"), "01", Sqlca);
            ...
            System.out.println("单笔单批批复有合同（未结清）,批复有效    sSqldata3====" +
                               sSqldata3);
            rsResult = Sqlca.getASResultSet(sSqldata3);
            if (rsResult.next()) {
               sBusinessCurrency2 = rsResult.getString("BusinessCurrency");
               sBusinessSum2 = DataConvert.toMoney(rsResult.getDouble(
                 "BusinessSum") / 10000.0);
               sBalance2 = DataConvert.toMoney(rsResult.getDouble("Balance") /
                                               10000.0);
            ...
            temp.append("<tr>");
            temp.append("<td class=td1 align=left  colspan=1 >" + sCustomerName +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" + sName3 +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" +
                        BusinessType[j][i] + "&nbsp;</td>");
         ...
         sSql =
           ...
           " from CREDIT_INFO where  ObjectNo in(select serialno from BUSINESS_CONTRACT " +
           " where CreditMode='1' and phaseno='1000' " +
           " and RelativeSerialNo is null and CustomerID='" + sCustomerID +
           "' " +
         ...
         System.out.println("无批复的有效综合授信协议sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         sName2 = "无批复的协议";
         while (rs.next()) {
            ...
            BusinessSum[j][i] = DataConvert.toMoney(rs.getDouble("BusinessSum") /
              10000.0);
            sCreditBalance = DataConvert.toMoney(rs.getDouble("CreditBalance") /
                                                 10000.0);
            ...
            for (m = 0; m < n; m++) {
               if (rs.getString("Currency").equals(CurrencyTotal3[m])) {
                  dBusinessSum3[m] = dBusinessSum3[m] +
                    rs.getDouble("BusinessSum");
            ...
            dTotalBusinessSum = dTotalBusinessSum +
              rs.getDouble("BusinessSum") *
              getRate(rs.getString("Currency"), "01", Sqlca);
            dTotalBalance = dTotalBalance +
              rs.getDouble("CreditBalance") *
              getRate(rs.getString("Currency"), "01", Sqlca);
            ...
            temp.append("<td class=td1 align=left  colspan=1 >" + sCustomerName +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" + sName2 +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" +
                        BusinessType[j][i] + "&nbsp;</td>");
         ...
         sSql = " select getBusinessName(BusinessType) as BusinessType,BusinessCurrency as  BusinessCurrency2," +
           " getItemName('Currency',BusinessCurrency) AS BusinessCurrency," +
           " sum(nvl(BusinessSum,0)*getErate(BusinessCurrency,'01','" +
           StringFunction.getToday() + "')) as BusinessSum," +
           " sum((nvl(Balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0))*getErate(BusinessCurrency,'01','" +
           StringFunction.getToday() + "')) as Balance " +
           " from BUSINESS_CONTRACT where  CreditAggreement in(select serialno from BUSINESS_CONTRACT " +
           " where CreditMode='1' and phaseno='1000' " +
           " and RelativeSerialNo is null and CustomerID='" + sCustomerID +
           "'  and nvl(CreditType,'01')!='11' " +
           " and (finishdate is not null or nvl(FreezeType,'01') ='04' " +
           " or Maturity<'" + StringFunction.getToday() +
           "') and Maturity in(select max(Maturity) from BUSINESS_CONTRACT " +
           " where CreditMode='1' and phaseno='1000' " +
           " and RelativeSerialNo is null and CustomerID='" + sCustomerID +
           "'  and nvl(CreditType,'01')!='11' and (finishdate is not null or nvl(FreezeType,'01')='04' " +
           " or Maturity<'" + StringFunction.getToday() +
           "') " +
           " )) " +
           " and (Maturity>='" + StringFunction.getToday() + "' or  Maturity is null or nvl(balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0)>0)" +
           " and CreditMode='2' and phaseno='1000' group by BusinessType,BusinessCurrency order by BusinessType,BusinessCurrency";
         System.out.println("无批复的无效综合授信协议取合同   sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         while (rs.next()) {
            BusinessType[j][i] = rs.getString("BusinessType");
            Currency[j][i] = rs.getString("BusinessCurrency");
         ...
         sSql = " select getBusinessName(BusinessType) as BusinessType," +
           ...
           " and (( MONTHS_BETWEEN(TO_DATE('" + StringFunction.getToday() +
           "','YYYY/MM/DD')," +
           " TO_DATE(PUTOUTDATE,'YYYY/MM/DD'))>3 ) " +
           " or ( Maturity<'" + StringFunction.getToday() + "')) " +
           " ))" +
           " and CreditAggreement is null and CustomerID='" + sCustomerID +
           "'  and nvl(CreditType,'01')!='11' " +
           " and (Maturity>='" + StringFunction.getToday() + "'  or  Maturity is null or nvl(balance1,0)+nvl(Balance2,0)+nvl(InterestSum1,0)>0)" +
           " group by BusinessType,BusinessCurrency order by BusinessType,BusinessCurrency";
         System.out.println("无批复的业务合同，或批复（无效）下有未结清的合同 sSql====" + sSql);
         rs = Sqlca.getASResultSet(sSql);
         sName4 = "无批复的合同";
         while (rs.next()) {
            BusinessType[j][i] = rs.getString("BusinessType");
            ...
            temp.append("<td class=td1 align=left  colspan=1 >" + sCustomerName +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" + sName4 +
                        "&nbsp;</td>");
            temp.append("<td class=td1 align=left  colspan=1 >" +
                        BusinessType[j][i] + "&nbsp;</td>");
         ...
         if (sCustomerName.equals("")) {
            temp.append("<tr>");
            temp.append(
              "<td class=td1 align=left  colspan=1 ><B>占用总额度小计</B></td>");
            ...
            temp.append("<td class=td1 align=middle  colspan=1 >人民币</td>");
            temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                        DataConvert.toMoney(dApproveTotal[u] / 10000.0) +
                        "</td>");
            temp.append("<td class=td1 align=middle  colspan=1 >人民币</td>");
            temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                        DataConvert.toMoney(dTotalBusinessSum / 10000.0) +
                        "</td>");
            temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                        DataConvert.toMoney(dTotalBalance / 10000.0) + "</td>");
            temp.append("</tr> ");
         }
         j++;
         i = 0;
      ...
      for (m = 0; m < k; m++) {
         //System.out.println("CurrencyTotal["+m+"]"+CurrencyTotal[m]);
         System.out.println("dBusinessSum[" + m + "]" + dBusinessSum[m]);
      }

      for (m = 0; m < k; m++) {
         //System.out.println("CurrencyTotal3["+m+"]"+CurrencyTotal3[m]);
         //System.out.println("dBusinessSum3["+m+"]"+dBusinessSum3[m]);
      }
      //外币小计
      String sName = "";
      ...
      for (m = 1; m < k; m++) {
         ...
         temp.append("<tr>");
         temp.append(
           "<td class=td1 align=left  colspan=1 ><B>占用总额度外币小计</B></td>");
         temp.append("<td class=td1 align=left  colspan=1 >&nbsp;</td>");
         temp.append("<td class=td1 align=left  colspan=1 >&nbsp;</td>");
         temp.append("<td class=td1 align=middle  colspan=1 >&nbsp;</td>");
         temp.append("<td class=td1 align=middle  colspan=1 >&nbsp;" + sName +
                     "</td>");
         temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                     DataConvert.toMoney(dBusinessSum[m] / 10000.0) + "</td>");
         temp.append("<td class=td1 align=middle  colspan=1 >&nbsp;" + sName +
                     "</td>");
         temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                     DataConvert.toMoney(dsum / 10000.0) + "</td>");
         temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                     DataConvert.toMoney(dBala / 10000.0) + "</td>");
         temp.append("</tr> ");
      }
      //合同币种不再批复中
      String sFlag = "";
      for (m = 1; m < p; m++) {
         ...
         if (sFlag == "false") {
            ...
            temp.append("<td class=td1 align=middle  colspan=1 >&nbsp;" + sName +
                        "</td>");
            temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                        DataConvert.toMoney(dBusinessSum5[m] / 10000.0) +
                        "</td>");
            temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                        DataConvert.toMoney(dBalance5[m] / 10000.0) + "</td>");
            temp.append("</tr> ");
         }
      }
      ...
      temp.append("<td class=td1 align=middle  colspan=1 >人民币</td>");
      temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                  DataConvert.toMoney(dBusinessSum[0] / 10000.0) + "</td>");
      temp.append("<td class=td1 align=middle  colspan=1 >人民币</td>");
      temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                  DataConvert.toMoney(dBusinessSum5[0] / 10000.0) + "</td>");
      temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                  DataConvert.toMoney(dBalance5[0] / 10000.0) + "</td>");
      temp.append("</tr> ");
      //本外币合计
      //批复合计
      double dsum5 = 0.0;
      ...
      for (i = 0; i < p; i++) {
         if (CurrencyTotal5[i] == null)
            CurrencyTotal5[i] = "";
         dsum5 = dsum5 +
           dBusinessSum5[i] * getRate(CurrencyTotal5[i], "01", Sqlca);
      ...
      temp.append("<td class=td1 align=middle nowrap colspan=1 >人民币</td>");
      temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                  DataConvert.toMoney(dsum5 / 10000.0) + "</td>");
      temp.append("<td class=td1 align=right  colspan=1 >&nbsp;" +
                  DataConvert.toMoney(dBala / 10000.0) + "</td>");
      temp.append("</tr> ");
      if (sAttribute2.equals("1")) {
         ...
         for (i = 0; i < n; i++) {
            if (CurrencyTotal3[i] == null)
               CurrencyTotal3[i] = "";
            dTotal = dTotal -
              dBusinessSum3[i] * getRate(CurrencyTotal3[i], "01", Sqlca);
            //dBala=dBala+dBalance5[i]*getRate(CurrencyTotal5[i],"01",Sqlca);
         }
         temp.append("<tr>");
         temp.append(
           "<td class=td1 align=left  colspan=4 ><B>可用（批复）额度（本外币合计）</B></td>");
         //temp.append("<td class=td1 align=left  colspan=1 >&nbsp;</td>");
         //temp.append("<td class=td1 align=left  colspan=1 >&nbsp;</td>");
         //temp.append("<td class=td1 align=middle  colspan=1 >&nbsp;</td>");
         temp.append("<td class=td1 align=middle nowrap colspan=1 >人民币</td>");
         temp.append("<td class=td1 align=right nowrap colspan=1 >&nbsp;" +
                     DataConvert.toMoney(dTotal / 10000.0) + "</td>");
         ...
         for (i = 0; i < m; i++) {
            if (CurrencyTotal[i] == null)
               CurrencyTotal[i] = "";
            dsum = dsum +
              dBusinessSum[i] * getRate(CurrencyTotal[i], "01", Sqlca);
            //dBala=dBala+dBalance5[i]*getRate(CurrencyTotal5[i],"01",Sqlca);
         }
         for (i = 0; i < n; i++) {
            if (CurrencyTotal3[i] == null)
         ...
         for (i = 0; i < p; i++) {
            ...
            dsum5 = dsum5 +
              dBusinessSum5[i] * getRate(CurrencyTotal5[i], "01", Sqlca);
            dBala = dBala +
              dBalance5[i] * getRate(CurrencyTotal5[i], "01", Sqlca);
         }
         temp.append("<tr>");
...
/*   //测试使用
    public static void main(String [] argv)
    {
      try
      {
         ...
         Transaction Sqlca = null;//ConnectionManager.getTransaction("XD");//数据库采用直连的方式
