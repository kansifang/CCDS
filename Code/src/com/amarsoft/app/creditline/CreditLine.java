...
public class CreditLine {
    ...
    public void initFirstTierCL(Transaction Sqlca, String sLineID) throws Exception {
        ...
        sbSelect.append(" from CL_INFO where LineID = '" + sLineID + "' ");
    ...
    public void initSecondTierCL(Transaction Sqlca, String sLineID) throws Exception {
        ...
        sbLSSelect.append(" from CL_LIMITATION_SET where LineID = '" + sLineID + "' ");

        sSql = sbLSSelect.toString();
        String[][] sLSAttMatrix = Sqlca.getStringMatrix(sSql);
    ...
    private void getSubLineID(Transaction Sqlca,String sLineID)throws Exception {
    	this.sublineid = Sqlca.getStringArray("select LineID from CL_INFO where ParentLineID = '"+sLineID+"'");
    }
    ...
    private ICreditLineKeeper getKeeper(Transaction Sqlca) throws Exception {
        ...
        sClassName = (String) this.getType(Sqlca).getAttribute("CLKeeperClass");
        if (sClassName == null || sClassName.equals("")) {
            ...
            try {
                ...
                throw new ASException("类[" + sClassName + "]装载失败："
                        + ex.getMessage());
            }
        }
    ...
    public String enterCheckMode(Transaction Sqlca)throws Exception{
    	...
    	String CurCheckLogNo = DBFunction.getSerialNo("CL_CHECK_LOG","CheckNo","",Sqlca);
    	String sSql = "update CL_INFO set RecentCheck='"+CurCheckLogNo+"',RecentCheckStatus='1' where LineID='"+this.id()+"'";
    	Sqlca.executeSQL(sSql);

    	//向CL_CHECK_LOG插入记录
    	sSql = "insert into CL_CHECK_LOG (LineID,CheckNo,CheckTime) values('"+this.id+"','"+CurCheckLogNo+"','"+StringFunction.getTodayNow()+"')";
    	Sqlca.executeSQL(sSql);
    ...
    public void exitCheckMode(Transaction Sqlca)throws Exception{
    	...
    	String sSql = "update CL_INFO set RecentCheckStatus='0' where LineID='"+this.id+"'";
    	Sqlca.executeSQL(sSql);
    ...
    public CreditLineType getType(Transaction Sqlca) throws Exception {
    	if (this.type == null){
    		...
    		ASValuePool config = ASConfigure.getSysConfig("SYSCONF_CLTYPE",Sqlca);
    		CreditLineType tmpType = (CreditLineType)config.getAttribute(sCLTypeID);
    		this.type = tmpType;
    	}
    	return this.type;
    }
