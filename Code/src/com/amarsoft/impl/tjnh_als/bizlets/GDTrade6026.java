 ...
 *
 */
public class GDTrade6026 implements GDTradeInterface {
	public String runGDTrade(SocketManager sm,String sObjectNo,String sObjectType,Transaction Sqlca,String sFilePath,String sTradeDate){
		...
		String sCommercialNo ="", sBusinessType ="", sAccumulationNo ="",sTemp ="0",sResult="";
		try{
			
			sSql = " select bc.commercialno,bc.accumulationno, bc.businesstype, fo.phaseno " +
					" from business_contract bc,contract_modify cm,flow_object fo " +
					"where bc.serialno = cm.relativeno and cm.serialno = fo.objectno " +
					"and fo.objecttype = 'ContractModApply' and bc.serialno = '"+sObjectNo+"' ";
			...
			if(rs.next())
			{
				...
				sResult = rs.getString("phaseno");
			}
			rs.close();
			
			if("1000".equals(sResult))  //审批结果 0-不同意，1－同意
			{
				sTemp = "1";
			}
			//-----------------组合报文----------------------
			try {
				...
				sm.write(mb.setValue("ApproveResult",sTemp));//审批结果
