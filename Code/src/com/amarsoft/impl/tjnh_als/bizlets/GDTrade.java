...
public class GDTrade {
	...
	static boolean GDTradeType = true;//如果长连接就置为false;短连接为true,并且在交易结束时要sm.teardownConnection();
		...
		if(!GDTradeInited||GDTradeType){
			try{
				...
				if(!sm.setupConnection()){
					System.out.println("初始化Socekt失败!目前超时时间为["+props.getProperty("gdtimeout")+"],请注意！");
					return false;
				}
				in.close();
			}catch(Exception ex){
	...
	public static String runGDTrade(String sObjectNo,String sObjectType,String sTradeType,Transaction sqlca,String sFilePath,String sTradeDate) throws Exception{
		...
		try {
			...
			ex.printStackTrace();
			System.out.println(sTradeType+"对应交易的类不存在!请首先检查com.amarsoft.impl.tjnh_als.bizlets.GDTrade"+sTradeType+".java是否存在!");
		}
		return sReturn;
	}
