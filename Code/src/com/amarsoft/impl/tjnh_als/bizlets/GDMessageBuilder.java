...
public class GDMessageBuilder {
	...
	public final String CONV_CODESET = "GBK";// CP037;CP1047
	...
	public Logger logger = Logger.getLogger(GDMessageBuilder.class.getName());
	public Document sen_doc = null;
	
	public Document rcv_doc = null;

	/**
	 * 构造MessageBuilder
	 */
	public GDMessageBuilder() {
		InitMessageBuilder();
	}
	 ...
	 * 构造MessageBuilder
	 */
	public GDMessageBuilder(String SENXML_URL, String RCVXML_URL) {
	 ...
	 * 构造MessageBuilder
	 */
	public GDMessageBuilder(String SENXML_URL, String RCVXML_URL, String sFilePath) {
		this.SENXML_URL = SENXML_URL;
		this.RCVXML_URL = RCVXML_URL;
	...
	public byte[] setValue(String sElement, String str2)
		...
		if (sFixed.equals("R")){
			if(sChn.equals("Y") && Utils.getA2EByteLen(str2.toCharArray())>=iLen){
				...
				str2 = Utils.getRightFixedByte(str2, iLen, fixCharRight);//getRightFixedByte汉字长度为2，getRightFixedStr汉字长度为1
			}
		}
		//数字型左补零
		else{
	...
	/**
	 * 取得子结点对应值</p>
	 * 如BillGuaranty循环段中第一组循环体的名为Attribute1的节点值：
	 * gdmb.getChild("BillGuaranty","BillGuaranty0","Attribute1");
	 */
	public String getChild(String sMainChild,String sChild,String sElement) throws UnsupportedEncodingException {
		try{
			//GDMessageBuilder_Test.printjdom(rcv_doc);//打印测试XML
			return rcv_root.getChild(sMainChild).getChild(sChild).getChild(sElement).getAttributeValue("value").trim();
		}catch(Exception ex){
			logger.error("没有["+sElement+"]这个元素！请核对相关配置文件！");
			return "";
		}
	...
	/**
	 * 根据所得返回数据进行初始化xml,针对于存在XML循环体的情况，有循环体是XML中fixed配置为X的部分为主循环段，
	 * 对应的上面会有此循环段名称加Count字样的节点标识为此循环段的循环次数，此循环段的详细内容配置在以此循环段
	 * 为名称的XML配置文件中。循环体一般配置在报文最后部分.</p>
	 * 
	 * @param bRevBuf
	 * @param iWholeLen
	 * @return
	 * @throws UnsupportedEncodingException
	 */
	public boolean initRcv_withCyc(byte[] bRevBuf, int iWholeLen)
			throws UnsupportedEncodingException {
		int iBegin = 0;
		boolean bIsSucess = false;
		String sTr = "";
		try {
			for (int i = 0; i < rcv_root.getChildren().size(); i++) {
				...
				int iLen = Integer.parseInt(e.getChildText("length"));
				String sChn = e.getChildText("chn");
				String sFixed = e.getChildText("fixed");
				int k = 0;
				if("X".equals(sFixed)){
					String sChildName = e.getName();
					System.out.println(rcv_root.getChild(sChildName+"Count").getAttributeValue("value"));
					int iCount = Integer.parseInt(rcv_root.getChild(sChildName+"Count").getAttributeValue("value"));
					SAXBuilder rcv_sb_x = new SAXBuilder();
					Document rcv_doc_x =  rcv_sb_x.build(this.RCVXML_URL.replace(".xml","_")+sChildName+".xml");
					Element rcv_root_x = rcv_doc_x.getRootElement();
					
					while(iCount>0){
						//先生成根循环节点
						Element eMainChild = new Element(sChildName+k); 
						rcv_root.getChild(sChildName).addContent(eMainChild);
						for(int j = 0;j<rcv_root_x.getChildren().size();j++){
							Element ex = (Element) rcv_root_x.getChildren().get(j);
							iLen = Integer.parseInt(ex.getChildText("length"));
							sChn = ex.getChildText("chn");
							sFixed = ex.getChildText("fixed");
							byte[] bValue = new byte[iLen];
							//如果iBegin已经大于整包长度就直接退出,因为正常报文中可能会存在长度小于设定包头的情况
							if(iBegin>iWholeLen-1) break;
							System.arraycopy(bRevBuf, iBegin, bValue, 0, iLen);
							...
							sTr = new String(bValue, CONV_CODESET);
							//在根循环节上增加新的节点
							Element eChild = new Element(ex.getName()); 
							eChild.setAttribute("value",sTr);
							//为了保持一致性，其它属性也增加上去
							eChild.addContent(new Element("length").setText(String.valueOf(iLen)));
							eChild.addContent(new Element("fixed").setText(sFixed));
							eChild.addContent(new Element("chn").setText(sChn));
							
							rcv_root.getChild(sChildName).getChild(sChildName+k).addContent(eChild);
						}
						iCount--;
						k++;
					}
