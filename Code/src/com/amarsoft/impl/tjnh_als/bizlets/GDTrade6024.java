 ...
 *
 */
public class GDTrade6024 implements GDTradeInterface {
	public String runGDTrade(SocketManager sm,String sObjectNo,String sObjectType,Transaction Sqlca,String sFilePath,String sTradeDate){
		...
		String sSql="",sReturn="",sReturn1="";
		ASResultSet rs,rs1=null;
		String sCommercialNo ="", sBusinessType ="", sAccumulationNo ="",sTemp ="";
		double sContractSum = 0.0;
		try{
			
			sSql = " select BusinessType, CommercialNo, AccumulationNo from Business_Contract where SerialNo = '"+sObjectNo+"' ";
			...
			try {
				...
				System.out.println(mb.getValue("TradeType"));
				if(mb.getValue("TradeType").equals("3")){
					...
					{
						sReturn1="担保代偿金额:"+Double.parseDouble(mb.getValue("Sum1"))/10000.00+",担保代偿类型:"+
						(mb.getValue("Type1").equals("1")?"一次性代偿":"按月代偿")
						+",担保代偿期数:"+Integer.parseInt(mb.getValue("Term1"))+";";
					}else if("2110020".equals(sBusinessType))
					{
						sReturn1="担保代偿金额:"+Double.parseDouble(mb.getValue("Sum2"))/10000.00+",担保代偿类型:"+
						(mb.getValue("Type2").equals("1")?"一次性代偿":"按月代偿")
						+",担保代偿期数:"+Integer.parseInt(mb.getValue("Term2"))+";";
					}
					sReturn = "0000000@"+sReturn1;
				}else
