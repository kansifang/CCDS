 ...
 *
 */
public class GDTrade6022 implements GDTradeInterface {
	public String runGDTrade(SocketManager sm,String sObjectNo,String sObjectType,Transaction Sqlca,String sFilePath,String sTradeDate){
		...
		try{
			...
			sSql = " select BusinessType, CommercialNo, AccumulationNo, BusinessSum from Business_PutOut where SerialNo = '"+sObjectNo+"' ";
			...
			try {
				...
				if(mb.getValue("TradeType").equals("3")){
					...
					if(Math.abs(sContractSum - Double.parseDouble(mb.getValue("PutOutSum1"))/10000.00) > 0.00001)
					{
						sTemp = mb.getValue("CommercialNo")+"@0";
					}else
					{
						sTemp = mb.getValue("CommercialNo")+"@1";
					}
					sTemp = sTemp+"@"+mb.getValue("LoanFlag");
					sReturn = "0000000@"+sTemp;
					
					System.out.println("sContractSum------------------------"+sContractSum);
					System.out.println("PutOutSum1------------------------"+Double.parseDouble(mb.getValue("PutOutSum1"))/10000.00);
					System.out.println("PutOutSum2------------------------"+Double.parseDouble(mb.getValue("PutOutSum2"))/10000.00);
				}else
