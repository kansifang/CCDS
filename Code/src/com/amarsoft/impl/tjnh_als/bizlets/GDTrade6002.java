 ...
 *
 */
public class GDTrade6002 implements GDTradeInterface {
	public String runGDTrade(SocketManager sm,String sObjectNo,String sObjectType,Transaction Sqlca,String sFilePath,String sTradeDate){
		...
		try{	
			...
			"where RelativeSerialNo2='"+sObjectNo+"' order by serialno desc ";
			rs = Sqlca.getASResultSet(sSql);
			if(rs.next()){
				DBSerialNo = rs.getString("SerialNo");
			}
			rs.close();
			//-----------------组合报文----------------------
			try {
				...
				//以上为报文头部分，以下为报文体部分
				sm.write(mb.setValue("ContractSerialNo",DBSerialNo));//协议合同号
				...
				if(mb.getValue("TradeType").equals("3")){
					...
					sReturn="0000000@"+"总余额为["+DataConvert.toMoney(Double.parseDouble(mb.getValue("Balance"))/10000.00)+"],正常余额["+DataConvert.toMoney(Double.parseDouble(mb.getValue("NormalBalance"))/10000.00)+"],逾期余额["+DataConvert.toMoney(Double.parseDouble(mb.getValue("OverDueBalance"))/10000.00)+"],呆滞余额["+DataConvert.toMoney(Double.parseDouble(mb.getValue("DullBalance"))/10000.00)+"],呆账余额["+DataConvert.toMoney(Double.parseDouble(mb.getValue("BadBalance"))/10000.00)+"]";
					Sqlca.executeSQL("Update BUSINESS_CONTRACT set Balance="+(Double.parseDouble(mb.getValue("Balance"))/10000.00)+" where SerialNo='"+sObjectNo+"'");
