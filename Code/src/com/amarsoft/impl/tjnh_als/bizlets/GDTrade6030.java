 ...
 *
 */
public class GDTrade6030 implements GDTradeInterface {
	public String runGDTrade(SocketManager sm,String sObjectNo,String sObjectType,Transaction Sqlca,String sFilePath,String sTradeDate){
		...
		String sContractSerialNo="",sAccumulationNo="",sCommercialNo="",sChangType="";
		ASResultSet rs,rs1=null;
		int iImpawnGuarantyCount=0,iPawnGuarantyCount=0,iAssurerCount=0;
		try{	
			/////////////商贷号和委贷号//////////////
			sSql = "select CommercialNo,AccumulationNo,ChangType from BUSINESS_APPLY where SerialNo = '"+sObjectNo+"' order by SerialNo desc";
			...
			try {
				...
				sm.write(mb.setValue("ChangType",sChangType));			//变更类型
				//报文体部分填充完毕
				sm.flush();
	...
	public String insertGDMiddleTable(GDMessageBuilder mb,Transaction Sqlca){
		...
		try {
			...
			sSql =  "insert into GD_BUSINESSAPPLY ( "+
						...
						"'"+mb.getValue("ChangType")+"'"+
						")";
			Sqlca.executeSQL(sSql);	
			String ChangType = mb.getValue("ChangType");			
			//取变更类型，如果变更类型为“变更借款人”的时候，发送返回的数据，只更新申请表，不更新担保合同，变更类型为其他两项的时候保持原来的逻辑。
//			if("01".equals(ChangType)||"03".equals(ChangType))
//			{	
			//抵质押物担保信息插入中间表
