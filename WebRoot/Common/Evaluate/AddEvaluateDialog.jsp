...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Main02;Describe=定义变量，获取参数;]~*/%>
	...
	if (sObjectType.equals("Customer"))  //客户信用等级评估
	{
		...
		if (sCustomerType.equals("03")) 
			rs = Sqlca.getASResultSet("select CreditBelong from IND_INFO where CustomerID= '"+sObjectNo+"'");
		else 
			rs = Sqlca.getASResultSet("select CreditBelong from ENT_INFO where CustomerID= '"+sObjectNo+"'"); 
		if (rs.next()) 
	...
	{ 
		//取得相应申请人代码
		rs = Sqlca.getASResultSet("select CustomerID,CustomerName from  Business_Apply where SerialNo='"+sObjectNo+"'");
		if (rs.next()) 
		{	
	...
	{ 
		rs=Sqlca.getASResultSet("select orgname   from ORG_INFO where orgid='"+sObjectNo+"'");
		if(rs.next())  
			sObjectName=rs.getString(1);
		rs.getStatement().close();
	}
			...
			<tr> 
            	<td nowrap>  请输入客户最近的信用等级评估时间： 
					<%
						// zbdeng  2004/02/17
						String sMonth="";
						rs=Sqlca.getASResultSet("select AccountMonth from EVALUATE_RECORD where ObjectType='Customer' and ObjectNo='"+sCustomerID+"' order by AccountMonth DESC  ");
						if(rs.next())
