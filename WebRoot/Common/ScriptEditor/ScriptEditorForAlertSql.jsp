...
<%
String sScriptText = CurPage.getParameter("ScriptText",10);
String sSqlObjectType = CurPage.getParameter("SqlObjectType",10);
...
String sSql="select ObjectName,ObjectType from OBJECTTYPE_CATALOG where ObjectType ='"+sSqlObjectType+"'";
...
while(rs.next()) 
{
    ...
    sSql2 = "select distinct AttributeName,'<font color=red>{'||ObjectType||'}</font>.{'||AttributeName||'} ('||'MethodArgs' ||')',ltrim(rtrim(AttributeDescribe)),ltrim(rtrim(Remark)),SortNo from OBJECT_ATTRIBUTE "+
            "where ObjectType='"+rs.getString(2)+"' order by SortNo";
//out.println(sSql2);
//if(1==1) return;
