...
String sModelNo = DataConvert.toRealString(iPostChange,CurPage.getParameter("ModelNo",10));
...
String sSql="select RC1.ModelNo,RC1.ModelName,RC1.ModelName||'['||RC1.ModelNo||']' from REPORT_CATALOG RC1,REPORT_CATALOG RC2 "+
            "where RC1.ModelClass=RC2.ModelClass and RC2.ModelNo='"+sModelNo +"' order by RC1.ModelNo";
...
while(rs.next()) 
{
    ...
    sSql2 = "select distinct FI.ItemNo,FI.ItemName||'['||FI.ItemNo||']' from REPORT_MODEL RM,FINANCE_ITEM FI "+
            "where RM.RowSubject=FI.ItemNo and RM.ModelNo='"+rs.getString(1)+"' order by FI.ItemNo";
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
								<tr> 
									<td colspan=2 > 
                                        ...
                                        <TEXTAREA name="myTextArea" ROWS="35" COLS="80" ONSELECT="storeCaret(this);" onCLICK="storeCaret(this);" onKEYUP="storeCaret(this);" WRAP=soft style="overflow:scroll">
                                        </TEXTAREA>
										</div>
									</td>
								</tr>
...
//	Amar.doAction('SetScript',amarsoft2Real('<%=sScriptText%>'));
    ...
    function returnText(){
        sReturn = PopPage("/Common/ScriptEditor/TranslateAFSBackToSource.jsp");
