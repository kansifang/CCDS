...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "资源列表详情"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%	
	...
	sSql =  "select "+
			...
			"UpdateUser,"+
			"UpdateTime "+
		" from OBJECT_RES_LIST where ObjectType = '" + sObjectType + "' "+
		"and SortNo = '"+ sSortNo +"' ";
	...
	//doTemp.setDDDWCode("IsInUse","IsInUse");
 	...
 	doTemp.setHTMLStyle("TableID"," style={width:400px} ");
	if( sObjectType.trim().length() == 0 )
		doTemp.setReadOnly("InputUser,UpdateUser,InputOrg,InputUserName,UpdateUserName,InputOrgName,InputTime,UpdateTime",true);
	else
		doTemp.setReadOnly("ObjectType,SortNo,InputUser,UpdateUser,InputOrg,InputUserName,UpdateUserName,InputOrgName,InputTime,UpdateTime",true);
 	doTemp.setRequired("ObjectType,SortNo,TableID,ExportSql,",true);

	doTemp.setDDDWCode("EffStatus","TrueFalse");
 	doTemp.setHTMLStyle("ExportSql"," style={width:600px} ");
 	doTemp.setHTMLStyle("BeforeImpSql"," style={width:600px} ");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord()
	{
        ...
        OpenComp("ObjectResInfo","/Common/Configurator/ObjectExim/ObjectResInfo.jsp","","_self","");
	}
