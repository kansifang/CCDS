...
<%
	String sHeaders[][]={ 
				{"CLTeamID","产品分组编号"},
			    {"CLTeamName","产品分组名称"},
			    {"CLTeamContentName","产品分组内容"},
			    {"CLTeamLimit","产品分组限制条件"},
			    {"CLTeamRelaObj","产品分组关联对象"},
			    {"IsInUse","是否有效"},
			    {"InputUserName","登记人"},
			    {"InputOrgName","登记机构"},
			    {"InputTime","登记时间"},
	...
	String sSql = " select CLTeamID,CLTeamName,CLTeamType,CLTeamContentID,CLTeamContentName,CLTeamLimit, "+
	              " CLTeamRelaObj,IsInUse,InputUser,getUserName(InputUser) as InputUserName, "+
	              " InputOrg,getOrgName(InputOrg) as InputOrgName,InputTime,UpdateUser,UpdateTime "+	             
	              " from CL_TEAM "+
	...
	//设置弹出式选择窗口
	doTemp.setUnit("CLTeamContentName","<input type=button class=inputDate   value=\"...\" name=button1 onClick=\"javascript:parent.selectProductType();\"> ");
