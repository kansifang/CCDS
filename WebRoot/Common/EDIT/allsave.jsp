...
<%
	...
	" in ( select customerid from BUSINESS_APPLY where serialno='"+sSerialNo+"')";
	...
	if(sMethod.equals("2")) //save
	{
        ...
        PreparedStatement pre1 = Sqlca.conn.prepareStatement(sUpdate1);
        ...
        PreparedStatement pre2 = Sqlca.conn.prepareStatement(sUpdate2);
	...
	<tr height=1 valign=top id="buttonback" >
		<td>
	    	...
	    	<tr>
	    		...
	    		<td>
                       <script language=javascript>hc_drawButtonWithTip("保存当前内容","保存当前页的上下数据","javascript:my_save()","/cecm/Resources/6")</script>
	    		</td>
    		</tr>
	    	</table>
		</td>
...
<script language="JavaScript">
	function my_save()
	{
		//temp = parent.frames("OWC").document.all("Spreadsheet1").HTMLData;
		temp = parent.frames("OWC").document.all("reporttable").innerHTML;
