...
<script language=javascript>
	...
	function doSubmitIn()
	{
		...
		else if (sPhaseInfo == "Success")
		{
			...
			//刷新件数及页面
			OpenComp("GeneralApproveMain","/DeskTop/Components/GeneralApproveMain.jsp","ComponentName=需求评估&ComponentType=MainWindow&CodeNo=ApproveType&ApproveType=<%=sApproveType%>","left","")
	...
	function takeBack()
	{
		...
		if (sRetValue == "Commit")
		{
		    OpenComp("ApproveMain","/Common/BusinessFlow/ApproveMain.jsp","ComponentName=审查审批管理&ComponentType=MainWindow&ApproveType=<%=sApproveType%>","_top","")
		}		
	}
	...
	{		
		...
		{
			//退回任务操作   	
			sRetValue = PopPage("/Common/BusinessFlow/CancelTaskAction.jsp?SerialNo="+sSerialNo+"&rand=" + randomNumber(),"退回任务操作","dialogWidth=0;dialogHeight=0;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
			//如果成功，则刷新页面
			if (sRetValue == "Commit")
			{
				OpenComp("ApproveMain","/Common/BusinessFlow/ApproveMain.jsp","ComponentName=审查审批管理&ComponentType=MainWindow&ApproveType=<%=sApproveType%>","_top","")
