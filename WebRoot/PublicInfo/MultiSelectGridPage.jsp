...
<%
	...
	if(sAttribute5 == null) sAttribute5 = "";//返回值以什么分割譬如“@”等
	
	//将配置的变量用相对应的值替换	
	String []aParaStrings =sParaString.split("~");
	for(int i=0;i<aParaStrings.length;i++){
		String[] aParaString=aParaStrings[i].split(":");
		if(aParaString!=null&&aParaString.length>1){
	sSelCode = StringFunction.replace(sSelCode,"#"+aParaString[0],aParaString[1]);
	sSelFieldName = StringFunction.replace(sSelFieldName,"#"+aParaString[0],aParaString[1]);
	sSelReturnValue = StringFunction.replace(sSelReturnValue,"#"+aParaString[0],aParaString[1]);
	sSelFilterField = StringFunction.replace(sSelFilterField,"#"+aParaString[0],aParaString[1]);
	if("Compart".equals(aParaString[0])){
		sCompart=aParaString[1];
	}
		}else{
...
%>
	...
	{   
		try{			
			...
			{%>				
				sReturnValue += getItemValue(0,getRow(),"<%=sReturnValue[j]%>") + "<%=sCompart%>";
			<%}%>				
		}catch(e){
			return;
		}				
	...
	{   
		...
		try{
			...
			for(var iMSR = 0 ;iMSR < b; iMSR++)
			{
				...
				{			
					<%for(int j = 0 ; j < iReturnFiledNum ; j ++)
					{%>			
						sReturnValue += getItemValue(0,iMSR,"<%=sReturnValue[j]%>") + sCompart;
					<%}%>
				}				
			}
		}catch(e){
		...
		if(sReturnValue.length>0&&sReturnValue.lastIndexOf(sCompart)==sReturnValue.length-1){
