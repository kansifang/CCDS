...
//选择日期
function SelectDate(obj,strFormat,startDate,endDate,x,y){
...
Calendar.prototype.ReturnDate = function(datevalue,dt) {
	...
	//modify by fhlei 2012-11-7 15:05:50：不限制最早日期和最大日期
...
//modify by fhlei 2012-11-7 10:28:50：选择最小日期、今天、最大日期；
mvAry[mvAry.length] = ' <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;">';
mvAry[mvAry.length] = ' <tr style="">';//标题行，左按钮 | 日期选择 | 右按钮
//将“今天”改为图标符号按钮显示
mvAry[mvAry.length] = ' <th align="left" style="display: none" ><input name="calendarToday" type="button" id="calendarToday" value="';
mvAry[mvAry.length] = (calendar.DateMode == pickMode["day"]) ? Calendar.language["today"][this.lang] : Calendar.language["pickTxt"][this.lang];
mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:20px;font-size:12px;cursor:pointer"/></th>';

mvAry[mvAry.length] = ' <th align="right" width="33%"><input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:33px;height:20px;font-size:12px;cursor:pointer;"/></th>';
mvAry[mvAry.length] = ' <th align="left" width="12%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;color:#f6af3a;font-weight:bold" name="minDate" type="button" id="minDate" value="|&lt;&lt;"></th>';
mvAry[mvAry.length] = ' <th align="center" width="10%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:20px;color:#f6af3a;font-weight:bold" name="newCalendarToday" type="button" id="newCalendarToday" value="⊙"></th>';//今天
mvAry[mvAry.length] = ' <th align="right" width="12%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;color:#f6af3a;font-weight:bold" name="maxDate" type="button" id="maxDate" value="&gt;&gt;|"></th>';//向右input
mvAry[mvAry.length] = ' <th align="left" width="33%"><input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:33px;height:20px;font-size:12px;cursor:pointer"/></th>';
mvAry[mvAry.length] = ' </tr>';
mvAry[mvAry.length] = ' </table>';
//modify by fhlei end.
...
//modify by fhlei 2012-11-7 14:24:48：增加minDate,newCalendarToday,maxDate事件。
obj = this.getElementById("minDate");
obj.onclick = function () {var dReturnDate = new Date("1970","0","1","0","0","0");calendar.ReturnDate(dReturnDate,dReturnDate.format(calendar.dateFormatStyle));};
this.minDate = obj;

obj = this.getElementById("newCalendarToday");
obj.onclick = function () {
var today = (calendar.DateMode != pickMode["day"]) ?
new Date(calendar.year,calendar.month,calendar.day,calendar.hour,calendar.minute,calendar.second)
: new Date();
...
this.newCalendarToday = obj;

obj = this.getElementById("maxDate");
obj.onclick = function () {var dReturnDate = new Date("9999","11","31","23","59","59");calendar.ReturnDate(dReturnDate,dReturnDate.format(calendar.dateFormatStyle));};
this.maxDate = obj;
...
Calendar.prototype.show = function (dateObj, popControl) {
	...
	if(document.getElementById("myiframe0_cells")){
		var tempTopX = (xy.x + leftX-document.getElementById("myiframe0_cells").scrollLeft); // 修改 → 加入自定义偏移量
		this.panel.style.left = (tempTopX > 200 ? 200 : tempTopX) + "px";
	}
	else{
		var tempTopX = (xy.x + leftX); // 修改 → 加入自定义偏移量
		this.panel.style.left = (tempTopX > 200 ? 200 : tempTopX) + "px";
	}
	//alert("document.dateObj.scrollTop=" + (document.body.scrollHeight - document.body.clientHeight));
	//alert(document.body.outerHTML);
	//alert(DZ);
	//modify by fhlei 2012-11-6 21:53:31:限制日历框Y方向值太大；
	if(document.getElementById('div_my0')){
		var tempTopY = (xy.y + topY + dateObj.offsetHeight-document.getElementById('div_my0').scrollTop);
		this.panel.style.top = (tempTopY > 200 ? 200 : tempTopY) + "px";
	}
	else if(document.getElementById("myiframe0_cells")){
		var tempTopY = (xy.y + dateObj.offsetHeight-document.getElementById("myiframe0_cells").scrollTop);
		this.panel.style.top = (tempTopY > 200 ? 200 : tempTopY) + "px";
	}
	else{
		var tempTopY = (xy.y + topY + dateObj.offsetHeight);
		this.panel.style.top = (tempTopY > 200 ? 200 : tempTopY) + "px";		
	}
