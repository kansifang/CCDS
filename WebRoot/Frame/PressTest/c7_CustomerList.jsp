...
<%
	...
	//将空值转化为空字符串
	if(sCustomerType == null) sCustomerType = "01";
	if(sTempletNo == null) sTempletNo = "CustomerList1";
	//获得页面参数
...
<%	
	...
	//生成HTMLDataWindow
	Vector vTemp = dwTemp.genHTMLDataWindow(sUserID+","+sCustomerType+","+sOrgID);
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
			...
			{(sCustomerType.substring(0,2).equals("02")?"false":"true"),"","Button","加入重点客户连接","加入重点客户连接","addUserDefine()",sResourcesPath},
			{(sCustomerType.substring(0,2).equals("02")?"false":"true"),"","Button","权限申请","权限申请","ApplyRole()",sResourcesPath},
			{(sCustomerType.substring(0,2).equals("01")?"true":"false"),"","Button","改变客户类型","改变客户类型","changeCustomerType()",sResourcesPath},
		};
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord()
	{
		...
		if(typeof(sReturnValue) != "undefined" && sReturnValue.length != 0 && sReturnValue != '_CANCEL_')
		{
			...
			{
				sReturn = PopPage("/CustomerManage/AddCustomerAction.jsp?CustomerType="+sCustomerType+"&CustomerName="+sCustomerName+"&CertType="+sCertType+"&CertID="+sCertID+"&ReturnStatus="+sStatus+"&CustomerID="+sCustomerID+"","","");
				...
				//已经新增客户
				}else if(sReturn == "succeed" && sReturnStatus == "01")
				{
