...
<%
	...
	rs = Sqlca.getASResultSet("select AccountMonth,ModelNo,EvaluateDate,EvaluateScore,EvaluateResult,getOrgName(OrgID) as OrgName,getUserName(UserID) as UserName,CognDate,"+
		" CognResult,getOrgName(CognOrgID) as CognOrgName,getUserName(CognUserID) as CognUserName,Remark,PowerScore,PowerResult from EVALUATE_RECORD where ObjectType='"+sObjectType+"' and ObjectNo='"+sObjectNo+"' and SerialNo='"+sSerialNo+"'");
	if (rs.next())
	{	
	...
	//取得信用等级模版名称
	rs = Sqlca.getASResultSet("select ModelName from EVALUATE_CATALOG where ModelType='010' and ModelNo='"+sModelNo+"'");
	...
	//取得上期数据
	rs = Sqlca.getASResultSet("select SerialNo,EvaluateScore from EVALUATE_RECORD where ObjectType='Customer' and ObjectNo='"+sObjectNo+"' and AccountMonth='"+sPreAccountMonth+"'");
	if (rs.next())
