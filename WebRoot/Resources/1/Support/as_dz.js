...
function myAfterLoadGrid(iDW){
	var dwScriptFileUrl = location.protocol + '//' + location.host + 
	sResourcesPath + '/Support/extend.js?sCurJspName='+sCurJspName;
	...
	dwScript.id = "amarList";
	dwDOM.body.appendChild(dwScript);
	if(typeof(myAfterLoadGrid_stub)=='function'){
	myAfterLoadGrid_stub(iDW);
	}
}
...
function mySelectRow(){
	var dwDOM = document.all('myiframe0').contentWindow.document;
	// change all <input /> width
	var cells = dwDOM.all('dataTable').rows[getRow()+2].cells;
	for(k=2;k<dwDOM.all('dataTable').rows.length;k++){
		for(i=1;i<dwDOM.all('dataTable').rows[k].cells.length;i++){
			dwDOM.all('dataTable').rows[k].cells[i].childNodes[0].style.width = '100%';
		}
	}
	if(typeof(mySelectRow_stub)=='function'){
		mySelectRow_stub();
	}
}
...
/**取属性值，无该属性返回Null @author William @since 2005.01.06*/
...
function beforeKeyUp_show(myiframe)
{
	...
	return !myHandleSpecialKey(myiframe); //必须取反
}
...
//用于动态生成iframe并通过post将数据传递至模态窗口 byhu 2005.01.07
...
//用于动态生成 form 并通过post将数据传递至模态窗口 byhu 2005.01.07
...
/**自动排列输入项*/
/*remarked by lpzhang  replaced by new function 2009-8-11  
...
//  设置是否必须项 colName为'是"，initColName必输 add by zwhu
function setRequiredFlag3(colName,initColName)
{	
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
    if(isNotNull(sFieldTemp))
    {
	    for(var i=0;i<sFieldTemp.length;i++)
	    {
	      if(sFlag != "3" && sFlag !="undefined" && sFlag !="") 
 	  		setItemRequired(0,0,sFieldTemp[i],true);	 
 	      else
 	    	setItemRequired(0,0,sFieldTemp[i],false);	 
	    }
...
//  设置是否必须项 colName为'是"，initColName必输  add by zwhu
function setRequiredFlag4(colName,initColName)
{	
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
    if(isNotNull(sFieldTemp))
...
//  设置是否必须项 colName为'是"，initColName必输  add by zwhu
function setRequiredFlag5(colName,initColName)
{	
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
    if(isNotNull(sFieldTemp))
...
//  设置是否必须项 colName为'是"，initColName必输
function setRequiredFlag6(colName,initColName)
{
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
    if(isNotNull(sFieldTemp))
    {
	    for(var i=0;i<sFieldTemp.length;i++)
	    {
	      	if(sFlag == "2") {
		      	setItemRequired(0,0,sFieldTemp[i],true);
		      	if(i>1 && sFlag == "2"){
		 	  		setItemValue(0,0,sFieldTemp[i],"");
	 	    	    getASObject(0,0,sFieldTemp[i]).style.background ="#ffffff";	
	 	    	    getASObject(0,0,sFieldTemp[i]).removeAttribute("readOnly");  		 	  		
	 	  		}		 
	 		}else{
	 	    	setItemRequired(0,0,sFieldTemp[i],false);
	 	    	if(i>1 && sFlag == "1"){
	 	    	      setItemValue(0,0,sFieldTemp[i],0);
	 	    	      getASObject(0,0,sFieldTemp[i]).style.background ="#efefef";	
	 	    	      getASObject(0,0,sFieldTemp[i]).setAttribute("readOnly","true");    	      
		 		}		
	 	    }		 
	    }
    }
...
//如果colName得value =temp ，initcolName非必输 add by xhyong 2010-5-17
function setRequiredFlag7(colName,initColName,temp)
{
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
    if(isNotNull(sFieldTemp))
...
//循环判断colName的value是否等于temp中的值 ，设置initcolName中的字段为必输项 add by bqliu 2011-04-12
function setRequiredFlag8(colName,initColName,temp)
{
	sFlag = getItemValue(0,getRow(),colName);
	var sFieldTemp = initColName.split(",");
	var sValue = temp.split(",");

    if(isNotNull(sFieldTemp))
    {
	    for(var i=0;i<sFieldTemp.length;i++)
	    {
	      var sFieldTemp2 = sFieldTemp[i].split("@");
	      for(var j=0; j<sFieldTemp2.length; j++) {
	          if(sFlag == sValue[i]) 
 	  		      setItemRequired(0,0,sFieldTemp2[j],true);	 
 	          else
 	    	      setItemRequired(0,0,sFieldTemp2[j],false);	 
	      }
