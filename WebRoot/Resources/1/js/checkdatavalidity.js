...
//校验境外国别为香港特别行政区时外管编号  add by bgao in 2013-01-08
function CheckHKWGNO(CertID)
{
	checkedValue = CertID;		
	checkedValue = checkedValue.trim();	
	if (checkedValue.length > 0)
	{
		if (checkedValue.length != 11 )//外管编号为11位
		{		       
			return false;
		}
...
//校验境外金融机构外管编号 add by bgao in 2013-01-08
function CheckWGNO(CertID)
{
	checkedValue = CertID;			
	checkedValue = checkedValue.trim();	
	if (checkedValue.length > 0)
	{
		if (checkedValue.length != 9 )//外管编号为9位
		{		       
			return false;
		}
