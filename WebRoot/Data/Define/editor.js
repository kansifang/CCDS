var documentN =document;
var iframe0 =document.getElementsByName("myiframe0");
if(iframe0.length>0){
	documentN=iframe0[0].contentWindow.document;
}
documentN.write('<style type="text/css">\n');
documentN.write('.btn     { width: 22px; height: 22px; border: 1px solid buttonface; margin: 0; padding: 0; }\n');
documentN.write('.btnOver { width: 22px; height: 22px; border: 1px outset; }\n');
documentN.write('.btnDown { width: 22px; height: 22px; border: 1px inset; background-color: buttonhighlight; }\n');
documentN.write('.btnNA   { width: 22px; height: 22px; border: 1px solid buttonface; filter: alpha(opacity=25); }\n');
documentN.write('.cMenu     { background-color: threedface; color: menutext; cursor: Default; font-family: MS Sans Serif; font-size: 8pt; padding: 2 12 2 16; }');
documentN.write('.cMenuOver { background-color: highlight; color: highlighttext; cursor: Default; font-family: MS Sans Serif; font-size: 8pt; padding: 2 12 2 16; }');
documentN.write('.cMenuDivOuter { background-color: threedface; height: 9 }');
documentN.write('.cMenuDivInner { margin: 0 4 0 4; border-width: 1; border-style: solid; border-color: threedshadow threedhighlight threedhighlight threedshadow; }');
documentN.write('</style>\n');
function editor_defaultConfig(objname) {
	...
	this.btnList = {
		...
		"bold" : [ 'Bold', '加粗', 'parent.parent.editor_action(this.id)', 'ed_format_bold.gif' ],
		"italic" : [ 'Italic', '倾斜', 'parent.editor_action(this.id)','ed_format_italic.gif' ],
		"underline" : [ 'Underline', '下划线', 'parent.editor_action(this.id)','ed_format_underline.gif' ],
		"strikethrough" : [ 'StrikeThrough', 'Strikethrough','parent.editor_action(this.id)', 'ed_format_strike.gif' ],
		"subscript" : [ 'SubScript', 'Subscript', 'parent.editor_action(this.id)','ed_format_sub.gif' ],
		"superscript" : [ 'SuperScript', 'Superscript','parent.editor_action(this.id)', 'ed_format_sup.gif' ],
		"justifyleft" : [ 'JustifyLeft', 'Justify Left','parent.editor_action(this.id)', 'ed_align_left.gif' ],
		"justifycenter" : [ 'JustifyCenter', 'Justify Center','parent.editor_action(this.id)', 'ed_align_center.gif' ],
		"justifyright" : [ 'JustifyRight', 'Justify Right','parent.editor_action(this.id)', 'ed_align_right.gif' ],
		"orderedlist" : [ 'InsertOrderedList', 'Ordered List','parent.editor_action(this.id)', 'ed_list_num.gif' ],
		"unorderedlist" : [ 'InsertUnorderedList', 'Bulleted List','parent.editor_action(this.id)', 'ed_list_bullet.gif' ],
		"outdent" : [ 'Outdent', 'Decrease Indent', 'parent.editor_action(this.id)','ed_indent_less.gif' ],
		"indent" : [ 'Indent', 'Increase Indent', 'parent.editor_action(this.id)','ed_indent_more.gif' ],
		"forecolor" : [ 'ForeColor', '字体颜色', 'parent.editor_action(this.id)','ed_color_fg.gif' ],
		"backcolor" : [ 'BackColor', '背景颜色', 'parent.editor_action(this.id)','ed_color_bg.gif' ],
		"horizontalrule" : [ 'InsertHorizontalRule', 'Horizontal Rule','parent.editor_action(this.id)', 'ed_hr.gif' ],
		"createlink" : [ 'CreateLink', 'Insert Web Link','parent.editor_action(this.id)', 'ed_link.gif' ],
		"insertimage" : [ 'InsertImage', '插入图片', 'parent.editor_action(this.id)','ed_image.gif' ],
		"inserttable" : [ 'InsertTable', 'Insert Table','parent.editor_action(this.id)', 'insert_table.gif' ],
		"htmlmode" : [ 'HtmlMode', 'View HTML Source','parent.editor_setmode(\'' + objname + '\')', 'ed_html.gif' ],
		"popupeditor" : [ 'popupeditor', '放大编辑', 'parent.editor_action(this.id)','fullscreen_maximize.gif' ],
		"about" : [ 'about', '帮助', 'editor_about(\'' + objname + '\')','ed_about.gif' ],
		// Add custom buttons here:
		"custom1" : [ 'custom1', 'Purpose of button 1','parent.editor_action(this.id)', 'ed_custom.gif' ],
		"custom2" : [ 'custom2', 'Purpose of button 2','parent.editor_action(this.id)', 'ed_custom.gif' ],
		"custom3" : [ 'custom3', 'Purpose of button 3','parent.editor_action(this.id)', 'ed_custom.gif' ],
		// end: custom buttons
		"help" : [ 'showhelp', 'Help using editor', 'parent.editor_action(this.id)','ed_help.gif' ]
	};
}
function editor_generate(objname, userConfig) {
	...
	documentN.all[objname].config = config; // store config settings
	// set size to specified size or size of original object
	var obj = documentN.all[objname];
	...
	for ( var btnGroup in config.toolbar) {
		...
		for ( var btnItem in config.toolbar[btnGroup]) {
			...
			// fontname
			if (btnName == "fontname") {
				toolbar += '<select id="_'+ objname+ '_FontName" onChange="parent.editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; font-size: 12px;">';
			...
			// fontsize
			if (btnName == "fontsize") {
				toolbar += '<select id="_'+ objname+ '_FontSize" onChange="parent.editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 12px;">';
			...
			if (btnName == "fontstyle") {
				toolbar += '<select id="_'+ objname+ '_FontStyle" onChange="parent.editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 12px;">';
			...
			if (!btnObj) {
				alert("htmlArea error: button '"
						+ btnName
						+ "' not found in button list when creating the wysiwyg editor for '"
						+ objname
	...
	// build editor
	var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 bgcolor="buttonface" style="padding: 1 0 0 2" width='
			+ config.width
			+ ' unselectable="on"><tr><td>\n'
			+ toolbar
			+ '</td></tr></table>\n'
			+ '</td></tr></table></span>\n'
			+ '<textarea ID="_'
	...
	// add context menu
	editor += '<div id="_' + objname
			+ '_cMenu" style="position: absolute; visibility: hidden;"></div>';
	//  hide original textarea and insert htmlarea after it
	if (!config.debug) {
		documentN.all[objname].style.display = "none";
	}
	if (config.plaintextInput) { // replace nextlines with breaks
		var contents = documentN.all[objname].value;
		contents = contents.replace(/\r\n/g, '<br>');
		contents = contents.replace(/\n/g, '<br>');
		contents = contents.replace(/\r/g, '<br>');
		documentN.all[objname].value = contents;
	}
	// insert wysiwyg
	documentN.all[objname].insertAdjacentHTML('afterEnd', editor);
	// convert htmlarea from textarea to wysiwyg editor
	editor_setmode(objname, 'init');
	// call filterOutput when user submits form
	for ( var idx = 0; idx < documentN.forms.length; idx++) {
		var r = documentN.forms[idx].attachEvent('onsubmit', function() {
...
function editor_action(button_id) {
	...
	var button_obj = documentN.all[button_id];
	var editor_obj = documentN.all["_" + objname+ "_editor"];
	var config = documentN.all[objname].config;
	// help popup
	if (cmdID == 'showhelp') {
	...
	if (cmdID == 'popupeditor') {
		...
		var sDefaultModelessDialogStyle = "dialogLeft=" + (sScreenWidth * 0.2)
				+ ";dialogWidth=" + (sScreenWidth * 0.6) + "px;dialogHeight="+(sScreenHeight * 3 / 4)
				+ "px;resizable=yes;status:no;maximize:yes;help:no;";
		window.showModalDialog(sWebRootPath + "/Common/Configurator/MetaDataManage/fullscreen.html?"+objname,window.self,sDefaultModelessDialogStyle);
		return;
	}
	...
	// Custom2
		...
		var myText = showModalDialog(_editor_url + "popups/custom2.html",
				myTitle, // str or obj specified here can be read from dialog
				// as "window.dialogArguments"
	...
	// fgColor and bgColor
		...
		if (cmdID.toLowerCase() == 'subscript'
				&& editdoc.queryCommandState('superscript')) {
			editdoc.execCommand('superscript');
		}
		if (cmdID.toLowerCase() == 'superscript'
				&& editdoc.queryCommandState('subscript')) {
		...
		// insert image
			...
			showModalDialog(sWebRootPath + "/Common/Configurator/MetaDataManage/insert_image.html", editdoc,
					"resizable: no; help: no; status: no; scroll: no; ");
		}
		// insert table
		else if (cmdID.toLowerCase() == 'inserttable') {
			showModalDialog(
					_editor_url + "popups/insert_table.html?" + objname,
					window,
					"resizable: yes; help: no; status: no; scroll: no; ");
		} else {
...
function editor_event(objname, runDelay) {
	var config = documentN.all[objname].config;
	var editor_obj = documentN.all["_" + objname+ "_editor"]; // html editor object
	if (runDelay == null) {
		runDelay = 0;
	}
	var editdoc;
	var editEvent = editor_obj.contentWindow ? editor_obj.contentWindow.event
			: event;
	...
	if (editEvent && editEvent.keyCode) {
		...
		if ((ctrlKey && (ord == 121 || ord == 89)) || ctrlKey && shiftKey
				&& (ord == 122 || ord == 90)) { // catch ctrl-y, ctrl-shift-z (REDO)
		//      TODO: Add our own undo/redo functionality
			return;
		}
	}
...
function editor_updateToolbar(objname, action) {
	var config = documentN.all[objname].config;
	var editor_obj = documentN.all["_" + objname
			+ "_editor"];
	...
	if (action == "enable" || action == "disable") {
		...
		for ( var idxN in tbItems) {
			...
			var tbObj = documentN.all["_" + objname
					+ "_" + tbItems[idxN]];
			if (cmdID == "htmlmode" || cmdID == "about" || cmdID == "showhelp"
					|| cmdID == "popupeditor") {
				continue;
			...
			var isBtn = (tbObj.tagName.toLowerCase() == "button") ? true
					: false;
			if (action == "enable") {
	...
	var fontname_obj = documentN.all["_" + objname
			+ "_FontName"];
	if (fontname_obj) {
		...
		if (fontname == null) {
			...
			for (i = 0; i < fontname_obj.length; i++) {
				if (fontname.toLowerCase() == fontname_obj[i].text
						.toLowerCase()) {
	...
	var fontsize_obj = documentN.all["_" + objname
			+ "_FontSize"];
	if (fontsize_obj) {
	...
	var classname_obj = documentN.all["_" + objname
			+ "_FontStyle"];
	if (classname_obj) {
	...
	var IDList = Array('Bold', 'Italic', 'Underline', 'StrikeThrough',
			'SubScript', 'SuperScript', 'JustifyLeft', 'JustifyCenter',
			'JustifyRight', 'InsertOrderedList', 'InsertUnorderedList');
	for (i = 0; i < IDList.length; i++) {
		var btnObj = documentN.all["_" + objname
				+ "_" + IDList[i]];
		if (btnObj == null) {
			continue;
		}
...
function editor_setmode(objname, mode) {
	var config = documentN.all[objname].config;
	var editor_obj = documentN.all["_" + objname+ "_editor"];
	// wait until documentN is fully loaded
	if (documentN.readyState != 'complete') {
		setTimeout(function() {editor_setmode(objname, mode)}, 25);
		return;
	}
	// define different editors
	var TextEdit = '<textarea ID="_' + objname + '_editor" style="width:'
			+ editor_obj.style.width + '; height:' + editor_obj.style.height
			+ '; margin-top: -1px; margin-bottom: -1px;"></textarea>';
	var RichEdit = '<iframe ID="_' + objname + '_editor"    style="width:'
			+ editor_obj.style.width + '; height:' + editor_obj.style.height
			+ ';"></iframe>';
	...
	if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
		...
		editor_obj = documentN.all["_" + objname+ "_editor"];
	...
	//
		...
		if (mode == 'init') {
			contents = documentN.all[objname].value;
		} // on init use original textarea content
		// create editor
		editor_obj.outerHTML = RichEdit;
		editor_obj = documentN.all["_" + objname
				+ "_editor"];
		// get iframe documentN object
		// create editor contents (and default styles for editor)
		var html = "";
		html += '<html><head>\n';
		if (config.stylesheet) {
			html += '<link href="' + config.stylesheet
...
function editor_focus(editor_obj) {
	...
	if (editor_obj.tagName.toLowerCase() == 'textarea') { // textarea
		...
		var editdoc = editor_obj.contentWindow.document; // get iframe editor documentN object
...
function editor_about(objname) {
	showModalDialog(_editor_url + "popups/about.html", window,
			"resizable: yes; help: no; status: no; scroll: no; ");
}
...
function editor_insertHTML(objname, str1, str2, reqSel) {
	...
	if (documentN.all[objname]
			&& editor_obj == null) {
		documentN.all[objname].focus();
		documentN.all[objname].value = document
				.frames("myiframe0").document.all[objname].value
				+ str1 + str2;
		return;
	}
	// error checking
	...
	// insertHTML for plaintext textarea
		...
		editor_obj.focus();
		sRange = documentN.selection.createRange();
...
function _isMouseOver(obj, event) { // determine if mouse is over object
	...
	if (mouseX >= objLeft && mouseX <= objRight && mouseY >= objTop
			&& mouseY <= objBottom) {
		return true;
	}
	return false;
...
function editor_cMenu_generate(editorWin, objname) {
	...
	} ], [ '---', null, null ], [ 'Select All', 'Ctrl-A', function() {
	} ], [ 'Clear All', '', function() {
	} ], [ '---', null, null ], [ 'About this editor...', '', function() {
	...
	var cMenuHeader = ''
			+ '<div id="_'
			+ objname
			+ '_cMenu" onblur="editor_cMenu(this);" oncontextmenu="return false;" onselectstart="return false"'
	...
	var cMenuList = '';
	var cMenuFooter = '' + ' <tr><td colspan=2 height=1></td></tr>'
			+ '</table></div>';
	for ( var menuIdx in editor_cMenu.options) {
		...
		// menu option
			...
			cMenuList += '<tr class="cMenu" onMouseOver="editor_cMenu(this)" onMouseOut="editor_cMenu(this)" onClick="editor_cMenu(this, \''
					+ menuIdx + '\',\'' + objname + '\')">';
			if (menuKey) {
				cMenuList += ' <td align=left class="cMenu">' + menuName
						+ '</td><td align=right class="cMenu">' + menuKey
						+ '</td>';
			} else {
				cMenuList += ' <td colspan=2 class="cMenu">' + menuName
						+ '</td>';
			}
			cMenuList += '</tr>';
		}
	...
	documentN.all['_' + objname + '_cMenu'].outerHTML = cMenuHTML;
...
function editor_cMenu_setPosition(parentWin, editorWin, objname) { // set object position that won't overlap window edge
	...
	var mouseX = event.clientX
			+ parentWin.documentN.all['_' + objname
					+ '_editor'].offsetLeft;
	var mouseY = event.clientY
			+ parentWin.documentN.all['_' + objname
					+ '_editor'].offsetTop;
	...
	var pageH = documentN.body.clientHeight
			+ documentN.body.scrollTop;
	var pageW = documentN.body.clientWidth
			+ documentN.body.scrollLeft;
...
function editor_cMenu(obj, menuIdx, objname) {
	...
	if (action == "mouseover" && !obj.disabled
		...
		documentN.all['_' + objname + '_cMenu'].style.visibility = "hidden";
		var menucode = editor_cMenu.options[menuIdx][2];
		menucode();
	} else if (action == "blur") {
