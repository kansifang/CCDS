...
<%
	...
	//获得页面参数，电子合同模板编号
	   ...
	   String sBussinessType = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("BussinessType"));
	   
	 //将空值转化为空字符串
	   if(sEDocNo == null) sEDocNo = "";
	   if(sEDocType == null) sEDocType = "";
	   if(sBussinessType == null) sBussinessType = "";
	   System.out.println("sBussinessType="+sBussinessType);
	//获得组件参数
%>
...
<%
	...
	if (sEDocType.equals("010")) {
		sSql =	" select TypeNo,TypeName " +
				" from BUSINESS_TYPE "+
				" where "+
				//" TypeSortNo <> '' " + 
				//" and "+
				" TypeNo like '"+sBussinessType+"%'" + 
				" and "+
				" TypeNo not in (select TypeNo from EDOC_RELATIVE where EDocNo<>'' ) "+
				" order by TypeNo";
	}
	//担保类合同
