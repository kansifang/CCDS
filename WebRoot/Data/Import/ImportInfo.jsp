...
<%
	...
	String sSql;
	//获得组件参数
	String sReportDate = DataConvert.toString(DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ReportDate")));//setFilter 
	String sConfigNo = DataConvert.toString(DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ConfigNo")));//doTemp.setColumnAttribute("报表类型","IsFilter","1");
%>
<%
	/*~END~*/
%>
...
%>
	<%
    	...
    	//定义SQL语句
    	sSql = " SELECT  ConfigNo as 报表类型,OneKey as 报表日期,ImportIndex as 序号"+
    				sS+
    				",ImportNo as 导入号,ImportTime as 导入时间,UserID as 导入人"+
    	 			" FROM Batch_Import_Interim " +
		  			" WHERE ConfigNo='"+sConfigNo+"' and OneKey='"+sReportDate+"'";
		ASDataObject doTemp = new ASDataObject(sSql);
		//设置表头
	    doTemp.setHeader(sHeaders);
		doTemp.UpdateTable="Batch_Import_Interim";
		doTemp.setKey("报表类型,报表日期,序号",true);
