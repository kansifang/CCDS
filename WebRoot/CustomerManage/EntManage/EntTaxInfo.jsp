...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//设置纳税金额（元）范围
	doTemp.appendHTMLStyle("TaxSum"," myvalid=\"parseFloat(myobj.value,10)>=0 \" mymsg=\"纳税金额（元）必须大于等于0！\" ");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function ValidityCheck()
	{
		...
		//校验纳税日期是否大于当前日期
		sTaxPayingDate = getItemValue(0,0,"TaxPayingDate");//纳税日期
		sToday = "<%=StringFunction.getToday()%>";//当前日期		
		if(typeof(sTaxPayingDate) != "undefined" && sTaxPayingDate != "" )
		{
			if(sTaxPayingDate >= sToday)
			{		    
				alert(getBusinessMessage('168'));//纳税日期必须早于当前日期！
				return false;		    
			}
		}		
		
		sBeginDate = getItemValue(0,getRow(),"BeginDate");//区间开始日期			
		sEndDate = getItemValue(0,getRow(),"EndDate");//区间结束日期
		...
		if (typeof(sBeginDate)!="undefined" && sBeginDate.length > 0)
		{
			if(typeof(sEndDate)!="undefined" && sEndDate.length > 0)
			{
				...
				{		    
					alert(getBusinessMessage('169'));//区间结束日期必须晚于区间开始日期！
					return false;		    
				}
			}	
		}
