...
<%!
	...
	public String[][] getPropertyValue(String sReportNo,Transaction Sqlca) throws Exception
	{
		...
		rs = Sqlca.getASResultSet("select Col2value from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject ='804'");
		if(rs.next())
		{	
		...
		rs = Sqlca.getASResultSet("select Col2value from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject ='807'");
		if(rs.next())
		{
		...
		{
			//短期借款及一年内到期的长期负债
			rs = Sqlca.getASResultSet("select sum(Col2value) as Col2value from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('201','211')");
			...
			rs = Sqlca.getASResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('801','101','106','108','110','19h','119','19m','805','202','203','806','808','301')");
			while(rs.next())
			{
	...
	public String[][] getSYValue(String sReportNo,Transaction Sqlca) throws Exception
	{
		...
		rs = Sqlca.getASResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('501','502','505','503','507','508','509','515','517')");
		while(rs.next())
		{
			String sRowSubject = rs.getString("RowSubject");
			if(sRowSubject == null) sRowSubject = "";
	...
	public String[] getXJValue(String sReportNo,Transaction Sqlca) throws Exception
	{
		...
		ASResultSet rs = null;
		rs = Sqlca.getASResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('a20','a27','810','811','812','813')");
		while(rs.next())
		{
			String sRowSubject = rs.getString("RowSubject");
			if(sRowSubject == null) sRowSubject = "";
	...
	public String[] getCZValue(String sReportNo,Transaction Sqlca) throws Exception	
	{
		...
		ASResultSet rs = null;
		rs = Sqlca.getASResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('914','915','916','911','999','901','909','932','907','908','905',' ','906')");
		while(rs.next())
		{
			String sRowSubject = rs.getString("RowSubject");
			if(sRowSubject == null) sRowSubject = "";
	...
	public String[] getYLValue(String sReportNo,Transaction Sqlca) throws Exception	
	{
		...
		ASResultSet rs = null;
		rs = Sqlca.getASResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sReportNo+"' And RowSubject in ('501','509','505','510','517')");
		while(rs.next())
		{
			String sRowSubject = rs.getString("RowSubject");
			if(sRowSubject == null) sRowSubject = "";
...
<%
	...
	String sListingCorpOrNot = "";	//是否上市公司	
	...
	sSql = " select EnterpriseName,RegisterAdd,OfficeAdd,getItemName('OrgType',OrgType) as OrgType,"+
		   ...
		   " getItemName('YesNo',ListingCorpOrNot) as ListingCorpOrNot "+
	...
	if(rs.next())
	{
		...
		sListingCorpOrNot = rs.getString("ListingCorpOrNot");
		...
		if(sListingCorpOrNot==null) sListingCorpOrNot = "";
	}
	rs.getStatement().close();
	...
	sSql = " Select CustomerName,getItemName('Currency',CurrencyType) as CurrencyType,InvestmentSum "+
		   " from CUSTOMER_RELATIVE where CustomerID='"+sCustomerID+"' "+
	...
	sSql = " Select IPODate,getItemName('IPOName',BourseName) as BourseName,StockCode,StockName "+
		   " from ENT_IPO where CustomerID='"+sCustomerID+"'";
	rs3 = Sqlca.getASResultSet(sSql);
	...
	sSql = " select IssueDate,getItemName('BondType',BondType) as BondType,"+
		   ...
		   " from ENT_BONDISSUE where CustomerID='"+sCustomerID+"'";
	rs4 = Sqlca.getASResultSet(sSql);
	...
	sSql = " select getBusinessName(CL.BusinessType) as BusinessType,"+
		   ...
		   " where BC.CustomerID='"+sCustomerID+"' "+
	...
	sSql = " Select ClassifySum1,(ClassifySum1-ClassifySum1ly) as ClassifySum11,"+
		   ...
		   " where InputDate=(Select Max(InputDate) from T_Fact_Loan_M)"+
		   " and CustomerID='"+sCustomerID+"'";
	rs10 = Sqlca.getASResultSet(sSql); 
	...
	sSql = " Select Sum(BusinessSum) as Balance, "+
		   ...
		   " from Business_Contract "+
		   " where CustomerID = '"+sCustomerID+"' "+
		   " and substr(BusinessType,1,4) in ('1010','1030','1040','1050','1060','1080','1090','1100','1110','1120','2030','2040','2050','2070','2080','2090','2100','2110') "+
		   " and BusinessType not in ('2050020','2050030')"+
		   " and Maturity like '"+sYear+"%'";
	rs11 = Sqlca.getASResultSet(sSql);
	if(rs11.next())
	{
	...
	sSql = " Select Sum(BusinessSum) as Balance, "+
		   ...
		   " from Business_Contract "+
		   " where CustomerID = '"+sCustomerID+"' "+
		   " and BusinessType in ('2020','2050020','2050030')"+
		   " and Maturity like '"+sYear+"%'";
	rs11 = Sqlca.getASResultSet(sSql);
	if(rs11.next())
	{
	...
	//取最新资产负债表日期
	 rs = Sqlca.getASResultSet("select ReportDate,ReportNo from REPORT_RECORD "+
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate ='"+sLastMonth+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate ='"+sBeginYear+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate ='"+sLastYear+"'");
	if(sReportNo==null) sReportNo = "";
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sSYYear1+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sSYYear2+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sSYYear3+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate ='"+sXJYear1+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate ='"+sXJYear2+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate ='"+sXJYear3+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	//取最新财务指标表日期
	 rs = Sqlca.getASResultSet("select ReportDate,ReportNo from REPORT_RECORD "+
		  " where ObjectNo ='"+sCustomerID+"' And ModelNo like '%9' And  ReportDate = (select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+sCustomerID+"' And ModelNo like '%9')");
	if(rs.next())
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%9' And  ReportDate ='"+sCZMonth2+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%9' And  ReportDate ='"+sCZMonth3+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%9' And  ReportDate ='"+sCZMonth4+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sCZMonth1+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sCZMonth2+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
		...
		sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sCZMonth3+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
	...
	sReportNo = Sqlca.getString("select ReportNo from REPORT_RECORD "+
		  		" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate ='"+sCZMonth4+"'");
	if(sReportNo==null) sReportNo = "";
	if(!sReportNo.equals(""))
	{
   ...
   <td class="underlinetd" colspan="4"><table width="100%" height="1783" border="0" cellpadding="2" cellspacing="0">
     ...
     <tr>
	  ...
	  <tr>
       ...
       <td class="underlinetd">是否上市公司</td>
       <td class="underlinetd"><%=sListingCorpOrNot%></td>
     </tr>
     <tr>
