...
<%	
	String sHeaders[][] = { 
							...
							{"ProjectName","项目名称"},
							{"ProjectLevelName","项目级别"},
							{"ProjectInvestment","项目总投资"},
						    {"OrgName","登记机构"},
						    {"UserName","登记人"}
	                        
			              };   		   		
	sSql = 	" select I.ProjectNo,R.ObjectNo,R.ObjectType,getItemName('ProjectStyle',I.ProjectType) as ProjectTypeName,"+
		  	" getItemName('ProjectType',I.ProjectLevel) as ProjectLevelName,ProjectName," + 
	      	" getOrgName(I.InputOrgID) as OrgName,I.InputUserID as InputUserID,getUserName(I.InputUserID) as UserName " + 
	      	" from PROJECT_RELATIVE R,PROJECT_INFO I " + 
	      	" where R.objecttype='"+sObjectType+"' and R.ObjectNo= '" + sObjectNo + "' " + 
	      	" and I.ProjectNo=R.ProjectNo " + 
	      	" order by R.ProjectNo";	
    //用sSql生成数据窗体对象
	ASDataObject doTemp = new ASDataObject(sSql);
	//设置表头
	doTemp.setHeader(sHeaders);
	doTemp.setAlign("ProjectTypeName,ProjectLevelName","2");
	//设置修改表名
	doTemp.UpdateTable = "PROJECT_RELATIVE";
	//设置主键
	doTemp.setKey("ProjectNo,ObjectNo,ObjectType",true);	 //为后面的删除
	//设置不可见项
	doTemp.setVisible("ProjectNo,ObjectNo,ObjectType,InputUserID",false);	    
	//通常用于外部存储函数得到的字段
	doTemp.setUpdateable("ProjectTypeName,ProjectLevelName",false);   
	doTemp.setHTMLStyle("UserName"," style={width:80px} ");
	doTemp.setHTMLStyle("OrgName"," style={width:200px} ");		
	//过滤器
	doTemp.setColumnAttribute("ProjectName","IsFilter","1");
	...
	//设置setEvent
	String sTable="PROJECT_RELATIVE";
	//事件调用，在后台直接写类
	dwTemp.setEvent("AfterDelete","!ProjectManage.DeleteProjectRelative(#ProjectNo,#ObjectType,#ObjectNo,"+sTable+")");
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		{"true","","Button","新增","新增项目信息","newRecord()",sResourcesPath},
		{"true","","Button","引入","引入项目","doImport()",sResourcesPath},
		{"true","","Button","详情","查看项目详情","viewAndEdit()",sResourcesPath},
		{"true","","Button","删除","删除项目信息","deleteRecord()",sResourcesPath},
		};
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	{		
		...
		{			
			...
			var sReturnValue=PopPage("/CustomerManage/EntManage/AddProjectAction.jsp?"+thisObjectInfo+sProjectInfo,"","resizable=yes;dialogWidth=21;dialogHeight=19;center:yes;status:no;statusbar:no");
	...
	function deleteRecord()
	{
	    ...
	    //判断是否有删除权限
		var sReturnValue=PopPage("/CustomerManage/EntManage/AddCheckProjectAction.jsp?ObjectNo=<%=sObjectNo%>&ProjectNo="+sProjectNo+" ","","");
	...
	function viewAndEdit()
	{
		...
		{       
			openObject("Project",sProjectNo,"001");			
		}
	}
