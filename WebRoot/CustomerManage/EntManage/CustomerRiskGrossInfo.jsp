...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "授信风险总量详情"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sSql =   " select ObjectType,ObjectNo,SerialNo,AccountMonth, "+
					" EvaluateScore,Remark,OrgID,getOrgName(OrgID) as OrgName, "+
	...
	doTemp.setRequired("EvaluateScore",true);
	...
	doTemp.setHTMLStyle("AccountMonth,UserName,InputDate,UpdateDate"," style={width:80px}");
	doTemp.setReadOnly("AccountMonth,UserName,OrgName,InputDate,UpdateDate,EvaluateScore",true);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	{		
		...
		sReturn = RunMethod("CustomerManage","ComputeRiskGross","<%=sCustomerID%>");
		if (typeof(sReturn) != "undefined")
		{
			Money = amarMoney(sReturn,2);
			setItemValue(0,0,"EvaluateScore",Money);
			alert("测算成功!");
		}
	}
	...
	function goBack()
	{
		OpenPage("/CustomerManage/EntManage/CustomerRiskGrossList.jsp?","_self","");
	}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"ObjectType","RiskGross");
