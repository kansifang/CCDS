...
<%
	...
	//先插入一条纪录以供显示编辑
	String sSerialNo=DBFunction.getSerialNo("RISK_SIGNAL","SerialNo",Sqlca);

	//String sSql="insert into RISK_SIGNAL(ObjectType,ObjectNo,SerialNo,SignalType,SignalStatus) values('Customer','"+sCustomerID+"','"+sSerialNo+"','"+sSignalType+"','01')";

	String sSql="insert into RISK_SIGNAL(ObjectType,ObjectNo,SerialNo,SignalType) values('Customer','"+sCustomerID+"','"+sSerialNo+"','"+sSignalType+"')";
...
<script language=javascript>
	function Save()
	{
		...
		{			
			PopPage("/CustomerManage/EntManage/SignalAction.jsp?ActionType=Add&SerialNo=<%=sSerialNo%>&Remark="+sRemark+"&SignalStatus="+sSignalStatus,"_self",OpenStyle);
			self.close();
		}
	}
	...
	function Cancel()
	{
		...
		{			
			PopPage("/CustomerManage/EntManage/SignalAction.jsp?ActionType=Delete&SerialNo=<%=sSerialNo%>","_self",OpenStyle);
			self.close();
		}
	}
	//关闭窗口前提示是否保存添加的信息
	...
	{		
		if(event.clientX>360&&event.clientY<0||event.altKey)
		{
			if(confirm("保存该预警信息吗？"))
			{
				...
				PopPage("/CustomerManage/EntManage/SignalAction.jsp?ActionType=Delete&SerialNo=<%=sSerialNo%>","_self","");
		}
	}
  function initRow()
	{
