...
<script language=javascript>
	...
	function ValidityCheck()
	{
		...
		sBeginDate = getItemValue(0,getRow(),"BeginDate");//债券起始日期			
		sEndDate = getItemValue(0,getRow(),"EndDate");//债券到期日期	
		
		if (typeof(sBeginDate)!="undefined" && sBeginDate.length > 0)
		{			
			if(sBeginDate >= sToday)
			{		    
				alert(getBusinessMessage('165'));//债券起始日期必须早于当前日期！
				return false;		    
			}
			
			if(sBeginDate >= sUpToDate)
			{		    
				alert(getBusinessMessage('167'));//债券起始日期必须早于统计截止日期！
				return false;		    
			}
			...
			if(typeof(sEndDate)!="undefined" && sEndDate.length > 0)
			{
				...
				{		    
					alert(getBusinessMessage('166'));//债券到期日期必须晚于债券起始日期！
					return false;		    
				}
			}	
		}
