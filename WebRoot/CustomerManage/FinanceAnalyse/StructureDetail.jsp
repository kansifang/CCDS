		...
		<td style='BORDER-bottom: #000000 1px solid;' id="reporttable">
			...
			%>
					<td colspan="<%=iSubTitleSpan%>" align="center">
						<%=StringFunction.getSeparate(sAccountMonth,"/",1) + "年" + StringFunction.getSeparate(sAccountMonth,"/",2) + "月"%>
					</td>
			<%
				}
			%>
			...
			<%
				...
				//rsTemp = Sqlca.getResultSet("select FinanceItemNo,DisplayName from Finance_Data where CustomerID = '" + sCustomerID + "' and AccountMonth = '" + sAccountMonth + "' and ReportNo = '" + sReportNo + "' and Scope = '" + sScope + "' order by DisplayNo");
...
function graphShow()
{
    ...
    PopPage("/CustomerManage/FinanceAnalyse/ShowGraph.jsp?GraphType="+document.all("GraphType").value+"&AccountMonths=<%=sAccountMonths%>&ItemNames="+sItemNames+"&ItemValues="+sItemValues+"&rand="+randomNumber(),"_blank",sDefaultDialogStyle);
