						...
						<div style="overflow:auto;position:absolute;width:100%;height:100%">
							...
							<%}else{%>
								<tr>
									...
									<td>从</td>
									<td><input type=text name="ROW_<%=sFSItems[i][1]%>_FROM" value="<%=sFSDatas[i][0]%>"  <%=sFSItems[i][4]%>></td>
									<td <%=(sFSItems[i][4]!=null && sFSItems[i][4].toUpperCase().indexOf("READONLY")>=0)?"style={display:none}":""%>>到</td>
									<td <%=(sFSItems[i][4]!=null && sFSItems[i][4].toUpperCase().indexOf("READONLY")>=0)?"style={display:none}":""%>><input type=text name="ROW_<%=sFSItems[i][1]%>_TO" value="<%=sFSDatas[i][1]%>" <%=sFSItems[i][4]%> ></td>
								</tr>
							<%}%>
							<%
							}	
