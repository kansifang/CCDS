...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "预警解除"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%
	...
	/*
	//预警报告日期
	sAlarmApplyDate = Sqlca.getString("select EndTime from FLOW_TASK where ObjectNo = '"+sSerialNo+"' and phaseno='0010'");
	if(sAlarmApplyDate == null) sAlarmApplyDate = "";
	if(sAlarmApplyDate.length()>10)
	{
		sAlarmApplyDate=sAlarmApplyDate.substring(0,10);
	}
	*/
...
<%	
	...
	String[][] sHeaders = {	
							{"RelativeSerialNo","原申请预警信号发起流水号"},
							{"CustomerName","客户名称"},
							{"SignalLevel","预警级别"},
							{"CustomerBalance","授信余额"},
							{"BailSum","保证金金额"},
							{"CustomerOpenBalance","敞口金额"},
							{"MessageContent","预警解除说明"},
							{"ApproveDate","授信批准日期"},
							{"CreditLevel","信用等级"},
	...
	sSql =  " select SerialNo,ObjectType,ObjectNo,RelativeSerialNo,GetCustomerName(ObjectNo) as CustomerName, "+
	...
	doTemp.setReadOnly("RelativeSerialNo,SignalLevel,ObjectNo,CustomerName,CustomerBalance,BailSum,CustomerOpenBalance,AlarmApplyDate,ApproveDate,CreditLevel,InputUserName,InputOrgName,InputDate,UpdateDate",true);
