...
<%
	...
	String[][] sHeaders = {
							{"GuarantyID","抵质押物编号"},							
							{"GuarantyName","抵质押物名称"},
							{"GuarantyTypeName","抵质押物类型"},
							{"GuarantyStatusName","抵质押物状态"},
							{"HoldDate","入库日期"},
							{"LostDate","出库日期"},
	...
	sSql = 	" select SerialNo,GuarantyID,GuarantyName,GuarantyType, "+
			" getItemName('GuarantyList',GuarantyType) as GuarantyTypeName, "+
			" getItemName('GuarantyStatus',GuarantyStatus) as GuarantyStatusName, "+
			" GuarantyStatus,HoldDate,LostDate,PlanReturnDate,FactReturnDate, "+
			" InputOrg,getOrgName(InputOrg) as InputOrgName,InputUser, "+	
			" getUserName(InputUser) as InputUserName,InputDate "+		
			" from GUARANTY_AUDIT "+
			" where GuarantyID = '"+sGuarantyID+"' ";
	
	//当抵质押物状态为临时出库和再回库时，仅显示临时出库信息		
	if(sGuarantyStatus.equals("03") || sGuarantyStatus.equals("01"))
		sSql += " and GuarantyStatus = '03' ";		
	//通过Sql产生ASDataObject对象doTemp	
	ASDataObject doTemp = new ASDataObject(sSql);
	doTemp.setHeader(sHeaders);
	doTemp.UpdateTable = "GUARANTY_AUDIT";
	doTemp.setKey("SerialNo",true);
	doTemp.setHTMLStyle("GuarantyStatusName","  style={width:80px}  ");
	//设置字段不可见属性
	doTemp.setVisible("SerialNo,GuarantyType,GuarantyStatus,InputOrg,InputUser",false);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function deleteRecord()
	{
		...
		{			
			...
			if(sInputUser == "<%=CurUser.UserID%>" && "<%=sGuarantyStatus%>" == "03" 
	...
	function viewAndEdit()
	{
		...
		{
			OpenComp("AddGuarantyAudit","/CreditManage/GuarantyManage/GuarantyAuditInfo.jsp","SerialNo="+sSerialNo+"&GuarantyStatus=<%=sGuarantyStatus%>","_self");
		}
	}
