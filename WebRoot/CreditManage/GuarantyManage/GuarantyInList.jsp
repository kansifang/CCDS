...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "已入库抵质押物信息列表"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%
	...
	sTempletNo = "GuarantyList";
	ASDataObject doTemp = new ASDataObject(sTempletNo,Sqlca);
	doTemp.WhereClause += " and (GuarantyType like '010%' or GuarantyType like '020%') and GuarantyStatus = '02' " + 
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		...
		{(sGuarantyStatus.equals("01")?"true":"false"),"","Button","入库","抵押物入库","loadGuaranty()",sResourcesPath},
		{(sGuarantyStatus.equals("02")?"false":"false"),"","Button","价值变更","抵押物价值变更","valueChange()",sResourcesPath},
		{(sGuarantyStatus.equals("02")?"false":"false"),"","Button","信息变更","抵押物其他信息变更","otherChange()",sResourcesPath},
//		{(sGuarantyStatus.equals("02")?"true":"false"),"","Button","临时出库","抵押物临时出库","unloadGuaranty1()",sResourcesPath},
		...
		{(!sGuarantyStatus.equals("01")?"false":"false"),"","Button","出入库历史记录","查看出入库历史记录","viewGuarantyAudit()",sResourcesPath},
	...
	function viewAndEdit()
	{
		...
		}else 
		{
			...
			if(sPawnType.substring(0,2)=="01")
				{
					popComp("ViewPawnInfo","/CreditManage/GuarantyManage/ViewPawnInfo.jsp","GuarantyStatus=<%=sGuarantyStatus%>&GuarantyID="+sGuarantyID+"&PawnType="+sPawnType);
				}
			//当对象是质押	
			else if(sPawnType.substring(0,2)=="02")
				{
					popComp("ViewImpawnInfo","/CreditManage/GuarantyManage/ViewImpawnInfo.jsp","GuarantyStatus=<%=sGuarantyStatus%>&GuarantyID="+sGuarantyID+"&PawnType="+sPawnType);
				}
		}
	}
