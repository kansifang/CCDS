...
<%
	...
	if(sObjectType.equals("AfterLoan")||sObjectType.equals("ReinforceContract")) sObjectType="BusinessContract";
	//out.print(sObjectNo);
%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord()
	{
		...
		OpenPage("/CreditManage/GuarantyManage/AddAssureImpawnInfo1.jsp?ContractNo=<%=sContractNo%>&ObjectNo=<%=sObjectNo%>&ObjectType=<%=sObjectType%>&ImpawnType="+sImpawnType,"_self");
	}
	...
	function importGuaranty()
	{
		...
		{
			//进行质物引入关联关系建立操作
			sReturn=RunMethod("BusinessManage","InsertGuarantyRelative","<%=sObjectType%>"+","+"<%=sObjectNo%>"+","+"<%=sContractNo%>"+","+sGuarantyID+","+"New"+","+"Import");
	...
	function viewAndEdit()
	{
		...
		{
			OpenPage("/CreditManage/GuarantyManage/AddAssureImpawnInfo1.jsp?ContractNo=<%=sContractNo%>&ObjectNo=<%=sObjectNo%>&ObjectType=<%=sObjectType%>&GuarantyID="+sGuarantyID+"&ImpawnType="+sImpawnType,"_self");
		}
	}
