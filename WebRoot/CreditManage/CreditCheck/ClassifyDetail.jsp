...
<%@page import="com.amarsoft.web.config.CodeManager;"%>
<%
	//定义变量：计数器
	int i = 0,iCount = 0;
	//定义变量：查询结果集
	...
	//获取页面参数：动作、分类类型、对象类型、对象编号、流水号、会计月份、模型号
	...
	rs = Sqlca.getASResultSet(sSQLText);
	if(rs.next())
	{
	...
	if(sAction.equals("_NEW_")) //新建一个classify
	{
...
<%
		}else
		{
			...
			//在风险分类记录表中新增一条分类记录
			sSerialNo = Classify.newClassify(sObjectType,sObjectNo,sAccountMonth,sModelNo,sClassifyDate,sOrgID,sUserID,Sqlca); 
			
			sSql = 	" update CLASSIFY_RECORD set BusinessBalance = "+dBalance+", "+
	...
	classify = new Classify(sObjectType,sObjectNo,sSerialNo,Sqlca);
	if(sAction.equals("_UPDATE_") || sAction.equals("_CLASSIFY_")) //保存,计算时也进行保存
	{
			...
			do 
	     	{
	    		i++;
	     		...
	     		{
	     			Sqlca.executeSQL(" update CLASSIFY_DATA set ItemValue = '"+sItemValue+"' where ObjectType = '"+sObjectType+"' and ObjectNo = '"+sObjectNo+"' and SerialNo = '"+sSerialNo+"' and ItemNo = '"+classify.ClassifyData.getString("ItemNo")+"' ");
	     		}
	...
	if(sAction.equals("_CLASSIFY_")) //开始计算
	{
		classify.getValue(); // 取值		
		try
		{
			classify.classify(); // 计算
		...
		classify.getRecord();  //重新取数据
		classify.getData();  
	}

	if(sAction.equals("_DELETE_")) //删除数据
	{
...
<script language=javascript> 
	...
	/*~[Describe=测算;InputParam=无;OutPutParam=SerialNo;]~*/
   	function classify()
   	{
		document.report.action="ClassifyDetail.jsp?Action=_CLASSIFY_&ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&SerialNo=<%=sSerialNo%>&AccountMonth=<%=sAccountMonth%>&ModelNo=<%=sModelNo%>&Rand="+randomNumber();
  	...
  	/*~[Describe=保存;InputParam=无;OutPutParam=SerialNo;]~*/
   	function updateData()
   	{
		document.report.action="ClassifyDetail.jsp?Action=_UPDATE_&ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&SerialNo=<%=sSerialNo%>&AccountMonth=<%=sAccountMonth%>&ModelNo=<%=sModelNo%>&Rand="+randomNumber();
		document.report.submit(); 
   	}
   	...
   	function deleteData()
   	{
		...
		{
			document.report.action="ClassifyDetail.jsp?Action=_DELETE_&ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&SerialNo=<%=sSerialNo%>&AccountMonth=<%=sAccountMonth%>&ModelNo=<%=sModelNo%>&Rand="+randomNumber();
			document.report.submit(); 
		}   
   	}  
...
<table width="100%" border="0" align="center" height="100%" cellspacing="0" cellpadding="0">
  ...
  <tr align="left" valign="top"> 
    <td width="100%"> 
      <div id="Layer1" style="position:absolute; width:100%; height:100%; z-index:1; overflow: scroll"> 
       <form name="report" method="post"> <center><b><%=sCustomerName +" "+ sBusinessName +" "+  classify.AccountMonth %>风险分类表<br>
		...
		<input type=hidden name=PageClientID value="<%=CurPage.ClientID%>">
		  ...
		  %>
		  <%=CodeManager.getItemName("ClassifyResult",s_tmp,Sqlca)%>
		  </b> 
        </center>
