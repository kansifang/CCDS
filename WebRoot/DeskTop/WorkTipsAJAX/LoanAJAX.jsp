...
<% 	
	...
	else if(sFlag.equals("1"))
	{
		sSql = 	" select getBusinessName(BP.BusinessType)||'&nbsp;['||BP.CustomerName||']'||'&nbsp;['||FT.PhaseName||']', "+
				" BP.BusinessSum,FT.BeginTime,FT.PhaseName,FT.PhaseNo,FT.PhaseType ,getNodeNo(FlowNo,PhaseNo) as NodeNo";
		sSql = sSql+ WhereCase;	
		rsTips = Sqlca.getResultSet(sSql);
	while(rsTips.next())
	{
...
%>
                	...
                	<%
                				...
                				{//待复核
                	%>
                     	   <td align="left" ><%=sTipsFlag%><a href="javascript:OpenComp('PutOutApproveMain','/Common/WorkFlow/ApproveMain.jsp','ApproveType=ApprovePutOutApply&NodeNo=<%=rsTips.getString("NodeNo")%>','_top','')"><%=rsTips.getString(1)%></a>&nbsp;</td>
                	<%
                				}
                	%>
