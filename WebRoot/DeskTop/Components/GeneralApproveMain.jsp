...
%>
	<%
		...
		sFolderUnfinished = tviTemp.insertFolder("root","当前工作","",ij++);
		...
		while (rs.next()){	
			...
			tviTemp.insertPage(sFolderUnfinished,sPageShow,"openPhase(\""+ sApproveType +"\",\""+ sPhaseType +"\",\""+sFlowNo+"\",\""+sPhaseNo+"\",\"N\")",iLeaf++);
		}
		rs.getStatement().close();
		//4、设置当前用户对于该类对象的已完成工作菜单项
		sFolderfinished = tviTemp.insertFolder("root","已完成工作","",ij++);
		sSql = sSql0+" and (nvl(EndTime,'') <> '' or EndTime <> ' ')"+sGO;
		rs = Sqlca.getASResultSet(sSql);
		rs.beforeFirst();
		while (rs.next()){		 
			...
			if(sWorkCount.equals(""))
				...
				sPageShow  = sPhaseName+" "+sWorkCount+" 件(完)";		
			tviTemp.insertPage(sFolderfinished,sPageShow,"openPhase(\""+ sApproveType +"\",\""+ sPhaseType +"\",\""+sFlowNo+"\",\""+sPhaseNo+"\",\"Y\")",iLeaf++);
		}
		rs.getStatement().close();
	%>
<%
...
%>
	...
	function TreeViewOnClick(){
		...
		eval(html2Real(sCurItemDescribe));
		setTitle(getCurTVItem().name);
	}
	/*~[Describe=treeview单击选中事件;InputParam=无;OutPutParam=无;]~*/
	function openPhase(sApproveType,sPhaseType,sFlowNo,sPhaseNo,sFinishFlag)
	{
		var sCurItemDescribe_url = "<%=sCompName%>?ApproveType="+sApproveType+"&FlowNo="+sFlowNo+"&PhaseType="+sPhaseType+"&PhaseNo="+sPhaseNo+"&FinishFlag="+sFinishFlag;
