...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "DataBaseInfo";
	String sTempletFilter = "1=1";

	ASDataObject doTemp = new ASDataObject(sTempletNo,sTempletFilter,Sqlca);
	doTemp.UpdateTable = "Reserve_Para";
	doTemp.setDDDWCode("LossScope","LossScope");
	doTemp.setCheckFormat("BaseDate","3");
	doTemp.setHTMLStyle("ALossTerm","style={width:80}");
    ...
    doTemp.setHTMLStyle("LastAccountMonth","style={width:80}");
    doTemp.setUnit("RetCancelSum","（大于该金额逐笔计提）");
    //doTemp.appendHTMLStyle("LossScope"," onchange=\"javascript:parent.selDDChange()\" ");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord(sPostEvents)
	{
		if(bIsInsert){
			...
			sReturnValue = RunMethod("新会计准则","DataBaseInfo",sAccountmonth);
			if(sReturnValue==1)
			{
	...
	function my_cal(){
	    ...
	    if(sLossScope != "M")	{//取算数平均值
	    	sReturn = PopComp("CalLossRateAction","/BusinessManage/ReserveDataPrepare/CalLossRateAction.jsp","LossScope="+sLossScope+"&AccountMonth="+sAccountMonth+"&rand="+randomNumber()," ","dialogWidth=20;dialogHeight=10;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no;close:no");
	        ss = sReturn.split('@');
	        setItemValue(0,0,"ALossRate1",ss[0]);
	        setItemValue(0,0,"ALossRate2",ss[1]);
	    }
	}
	function selDDChange(){
	  bReadOnly = false;
	  sLossScope  = getItemValue(0,getRow(),"LossScope");
	  if(sLossScope == "020" || sLossScope == "030"){
	      bReadOnly = true;
	  }
	  setItemReadOnly(0,0,"ALossRate1",bReadOnly);
	  setItemReadOnly(0,0,"ALossRate2",bReadOnly);   
	}
	function goBack()
	{
		self.close();
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{
		var sTableName = "Reserve_Para";//表名
