...
//	                        {"AFiveClassifyName","审计五级分类"},
    ...
    sSql = " select AccountMonth,AccountMonth as AccountMonth1,LoanAccount,DuebillNo,CustomerID,CustomerName,PutoutDate,"+
    	   " CustomerOrgCode,CertType,IndustryType,Scope,"+
...
//	doTemp.setColumnAttribute("AccountMonth,LoanAccount,CustomerName,AuditStatFlag,ManageStatFlag,ManageuserName","IsFilter","1");
    ...
    doTemp.setVisible("AccountMonth,DuebillNo,CustomerOrgCode,CustomerID,IndustryType,"+
     	              "Scope,MFiveClassify,AFiveClassify,AFiveClassifyName,AuditStatFlag,ManageStatFlag,Manageuserid,ManageOrgID,"+
     	              "Region,LoanType,CertType",false);
	...
	doTemp.setDDDWSql("CertType","select ItemNo,ItemName from code_library where codeno = 'CertType' and isinuse='1' and bankno is not null");
   	
    doTemp.generateFilters(Sqlca);
    doTemp.setFilter(Sqlca,"1","AccountMonth","");
    doTemp.setFilter(Sqlca,"2","LoanAccount","");
    doTemp.setFilter(Sqlca,"3","CertType","");
    doTemp.setFilter(Sqlca,"4","CustomerName","");
    doTemp.setFilter(Sqlca,"5","Region","");    
    doTemp.setFilter(Sqlca,"6","RMBBalance","");
    doTemp.setFilter(Sqlca,"7","MFiveClassify","");
    doTemp.setFilter(Sqlca,"8","AuditStatFlag","");    
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	*/
	...
	function viewAndEdit()
	{
		...
		var sReturn = PopComp("ReserveDataIndInfo","/BusinessManage/ReserveDataPrepare/ReserveDataIndInfo.jsp","LoanAccount="+sLoanAccount+"&AccountMonth="+sAccountMonth+"&IsManageUser="+sIsManageUser,"resizable=yes;dialogWidth=210;dialogHeight=100;center:yes;status:no;statusbar:no");
		reloadSelf();
	}
