...
<%
	...
	try{
	    if(sType.equals("Input") && sSerialNo.equals("")){//录入信息员批量提交		    	   
	        String s1 = " select RN.AssetNo as AssetNo, RN.AccountMonth as AccountMonth " + 
	                    "  from Reserve_NoCredit RN " + 
	        ...
	        while(r1.next()){
	           sDeclareSerialNo[i] = r1.getString("AssetNo")== null ? "" :r1.getString("AssetNo");
	           sDeclareCashInfo[i] = "没有录入减值准备相关信息";
	           i = i + 1;
	           sReturnValue = "99";
	        }
	        r1.getStatement().close();
	        if(i ==0){//所有的不良记录已经都在Reserve_Record表中存在
	      	String stemp = "select RR.ObjectNo as DuebillNo, RR.LoanAccount as LoanAccount, RR.AccountMonth as AccountMonth, " + 
	                     	...
	                     	" where RR.AccountMonth = RT.AccountMonth and RR.LoanAccount = RT.LoanAccount ";
	   ...
	   if(!sSerialNo.equals("")){//单笔提交
	       String stemp = "select RR.LoanAccount as LoanAccount, RR.AccountMonth as AccountMonth, " + 
	                     " RR.ConfirmDate as  ConfirmDate, " + 
	                     " RR.Result1 as Result1, RR.Result2 as Result2, RR.Result3 as Result3, RR.Result4 as Result4, RR.MResult as MResult " + 
	                     " from reserve_record RR, Reserve_Nocredit RN " + 
	                     " where RR.serialno = '" + sSerialNo + "'" + 
	                     " and RR.AccountMonth = RN.AccountMonth and RR.LoanAccount = RN.AssetNo ";
	       ASResultSet rsTemp = Sqlca.getASResultSet(stemp);
	       if(rsTemp.next()){
	          int k = i;
	          String sLoanAccount = rsTemp.getString("LoanAccount")== null ? "" :rsTemp.getString("LoanAccount");
	          String sConfirmDate = rsTemp.getString("ConfirmDate")== null ? "" :rsTemp.getString("ConfirmDate");
	         ...
	         if(k != i){
	             sDeclareSerialNo[i] = sLoanAccount;
	             i = i + 1;
	             sReturnValue = "99";
	          }
	       }
