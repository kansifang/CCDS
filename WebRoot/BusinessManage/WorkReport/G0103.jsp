...
<%
	...
	if(FinishFlag !=null&&FinishFlag.length()>0)
	{
		sButtons[0][0] = "false";	
	}
...
<% 
	...
	sTemp.append("	<form method='post' action='0102003.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=center colspan=23 bgcolor=#aaaaaa ><font style=' font-size: 14pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >担保情况</font></td>"); 	
	sTemp.append("   </tr>");	
	
	//保证情况
	String ssSql =  " select BC.SerialNo,getBusinessName(BC.BusinessType) as BusinessTypeName, "+
	" getItemName('Currency',BC.BusinessCurrency) as Currency, "+
	" BC.BusinessSum,BC.Balance,getItemName('ContractType',GC.ContractType) as ContractType,GC.GuarantyValue,GC.GuaranryOpinion  "+
	" from CONTRACT_RELATIVE CR,GUARANTY_CONTRACT GC,BUSINESS_CONTRACT BC "+
	" where CR.ObjectType = 'GuarantyContract' "+
	" and CR.ObjectNo = BC.SerialNo "+
	" and CR.SerialNo = GC.SerialNo "+			
	" and GC.ContractStatus = '020' "+
	" and GC.GuarantyType='010010'"+
	" and GC.CustomerID = '"+sObjectNo+"' ";
	
	rs = Sqlca.getResultSet(ssSql);
	while(rs.next()){
		...
		String ContractType=rs.getString("ContractType");//担保类型
		if(ContractType == null) ContractType = " "; 
		String GuarantyValue=rs.getString("GuarantyValue");//担保总金额
		if(GuarantyValue == null) GuarantyValue = " "; 
		String GuaranryOpinion=rs.getString("GuaranryOpinion");//担保意愿
		if(GuaranryOpinion == null) GuaranryOpinion = " "; 
	    ...
	    sTemp.append("   <td width=22% align=center class=td1 >合同金额(元)</td>");
	    sTemp.append("   <td align=right class=td1 >"+sBusinessSum+"&nbsp;</td>");
	    sTemp.append("   </tr>");
		sTemp.append("   <tr>");
	  	sTemp.append("   <td width=22% align=center class=td1 >合同余额(元) </td>");
	    sTemp.append("   <td align=right class=td1 >"+sBalance+"&nbsp;</td>");
	    sTemp.append("   <td width=22% align=center class=td1 >担保类型 </td>");
	    sTemp.append("   <td align=right class=td1 >"+ContractType+"&nbsp;</td>");
	   	sTemp.append("   </tr>");
		sTemp.append("   <tr>");
	  	sTemp.append("   <td width=22% align=center class=td1 >担保总金额 </td>");
	    sTemp.append("   <td align=right class=td1 >"+GuarantyValue+"&nbsp;</td>");
	    sTemp.append("   <td width=22% align=center class=td1 >担保意愿 </td>");
	    sTemp.append("   <td align=right class=td1 >"+GuaranryOpinion+"&nbsp;</td>");
	   	sTemp.append("   </tr>");
	}
	rs.getStatement().close();
	
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=center colspan=23 bgcolor=#aaaaaa ><font style=' font-size: 14pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >抵质押物信息</font></td>"); 	
	sTemp.append("   </tr>");	
	...
	while(rs.next())
	{
	    ...
	    sTemp.append("   </tr>");
		sTemp.append("   <tr>");
		sTemp.append("   <td align=center class=td1 >评估价值(元)</td>");
		...
		sTemp.append("   <td align=center class=td1 >权属人名称</td>");
