...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View01;Describe=定义页面属性;]~*/%>
	...
	String PG_CONTENT_TITLE = "&nbsp;&nbsp;公司业务&nbsp;&nbsp;"; //默认的内容区标题
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View03;Describe=定义树图;]~*/%>
	<%
	//定义Treeview
	HTMLTreeView tviTemp = new HTMLTreeView(SqlcaRepository,CurComp,sServletURL,"公司业务","right");
	...
	if(CurUser.hasRole("604"))
		...
		sSqlTreeView = "from CODE_LIBRARY where CodeNo='ReserveView' and IsInUse='1' and ItemNo in ('0010','0010010','0010011','0030') ";
...
<%/*~BEGIN~可编辑区[Editable=true;CodeAreaID=View05;]~*/%>
	...
	function TreeViewOnClick()
	{
		...
		if(sCurItemID=="0010010" || sCurItemID=="0010012" || sCurItemID=="0010014" || sCurItemID=="0010016")
		{
			...
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=UnFinished&Type=Confirm&Grade=02");
			}else if(<%=CurUser.hasRole("602")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=UnFinished&Type=Confirm&Grade=03");
			}else if(<%=CurUser.hasRole("603")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=UnFinished&Type=Confirm&Grade=04");
			}else if(<%=CurUser.hasRole("480")%> || <%=CurUser.hasRole("280")%> || <%=CurUser.hasRole("080")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=UnFinished&Type=Input&Grade=01");
			}
		}else if(sCurItemID=="0010011" || sCurItemID=="0010013" || sCurItemID=="0010015" || sCurItemID=="0010017")
		{
			if(<%=CurUser.hasRole("601")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=Finished&Type=Confirm&Grade=02");
			}else if(<%=CurUser.hasRole("602")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=Finished&Type=Confirm&Grade=03");
			}else if(<%=CurUser.hasRole("603")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=Finished&Type=Confirm&Grade=04");
			}else if(<%=CurUser.hasRole("480")%> || <%=CurUser.hasRole("280")%> || <%=CurUser.hasRole("080")%>)
			{
				openChildComp("InputEntList","/BusinessManage/ReserveManage/InputEntList.jsp","Action=Finished&Type=Input&Grade=01");
			}
		}
		else if(sCurItemID=="0020")
		{
			openChildComp("ResultConfirmList","/BusinessManage/ReserveManage/ResultConfirmList.jsp","Action=Finished&Type=Audit&Grade=05");
		}else if(sCurItemID=="0030")
		{
			openChildComp("ManualConfirmEntList","/BusinessManage/ReserveManage/ManualConfirmEntList.jsp","");
		}
