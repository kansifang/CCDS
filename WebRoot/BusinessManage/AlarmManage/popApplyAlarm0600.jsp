			...
			if(sVouchType.length()>=3) 
			{
				...
				if(sVouchType.substring(0,3).equals("010"))
				{
		        	...
		        	while(rs.next())
		        	{
		            	...
		            	//检查该客户是否存在已生效的预警信息
						sCount = Sqlca.getString("select count(SerialNo) from RISK_SIGNAL where ObjectType = 'Customer' and ObjectNo = '"+sGuarantorID+"' and SignalType = '1' ");
						if( sCount != null && Integer.parseInt(sCount,10) > 0  )
						{
							sbTips.append( "保证人["+sGuarantorName+"]存在生效的预警信号；"+"\r\n" );
						}					
					}
