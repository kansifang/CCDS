			...
			if(sVouchType.length()>=3) 
			{
				...
				if(sVouchType.substring(0,3).equals("010"))
				{
		        	...
		        	while(rs.next())
		        	{
						...
						{	
							sbTips.append("保证人["+sGuarantorName+"]在他行未结清的授信业务笔数："+sCount+";\n\r");
							...
							if(rs1.next())
							{
								...
								sbTips.append("保证人["+sGuarantorName+"]在他行未结清的授信业务发放总金额（折人民币）："+DataConvert.toMoney(sBusinessSum)+";\n\r");
								sbTips.append("保证人["+sGuarantorName+"]在他行未结清的授信业务总余额（折人民币）："+DataConvert.toMoney(sBalanceSum)+";\n\r");
							}
