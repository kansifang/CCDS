...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "机构参数管理"; // 浏览器窗口标题 <title> PG_TITLE </title>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View03;Describe=定义树图;]~*/%>
	<%
	//定义Treeview
	HTMLTreeView tviTemp = new HTMLTreeView(SqlcaRepository,CurComp,sServletURL,"机构参数管理","right");
	tviTemp.ImageDirectory = sResourcesPath; //设置资源文件目录（图片、CSS）
	tviTemp.toRegister = false; //设置是否需要将所有的节点注册为功能点及权限点
	tviTemp.TriggerClickEvent=true; //是否自动触发选中事件

	//定义树图结构
	String sSqlTreeView = " from ORG_INFO where OrgID in (select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%') and OrgLevel in ('0','3','6')";
...
<%/*~BEGIN~可编辑区[Editable=true;CodeAreaID=View05;]~*/%>
	...
	function TreeViewOnClick()
	{
		...
		{
			OpenComp("OrgArgList","/SystemManage/GeneralSetup/OrgArgList.jsp","ComponentName=机构参数管理&OrgID="+sOrgID,"right");
