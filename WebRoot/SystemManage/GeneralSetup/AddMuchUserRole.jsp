...
<%
	...
	int num = 0;
	sSql = " select RoleID || '   ' || RoleName as Allrole,RoleID" +
...
*/
	...
	sSql1 = " select UserID|| '   ' || UserName as AllUser,UserID" +
			" from USER_INFO readpast"+
			" where BelongOrg = '"+o_User.OrgID+"' ";
   ...
   function pushButton(buttonName,push) 
	...
	function doQuery()
	{
		if(confirm(getBusinessMessage("906")))//您确定给所选的所有用户新增所选的所有角色吗？
	    {
    		...
    		PopPage("/SystemManage/GeneralSetup/AddMuchRolesAction.jsp?Action=Add&UsersID="+vReturn1+"&RolesID="+vReturn,"","dialogWidth=20px;dialogHeight=20px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
    		self.close();
	    }
	}
	...
	function doDelete()
	{
        if(confirm(getBusinessMessage("909")))//您确定要删除所选用户中您所选的所有角色吗？
        {
    		...
    		PopPage("/SystemManage/GeneralSetup/AddMuchRolesAction.jsp?Action=Del&UsersID="+vReturn1+"&RolesID="+vReturn,"","dialogWidth=20px;dialogHeight=20px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
    		self.close();
        }     
	}
	function doDefault()
