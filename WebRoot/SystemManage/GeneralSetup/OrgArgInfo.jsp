...
<%
    ...
    String ssOrgID = sOrgID.substring(0,3);
...
<%	
	...
	String sTempletNo = "OrgArgInfo";
	String sTempletFilter = "1=1";
	ASDataObject doTemp = new ASDataObject(sTempletNo,sTempletFilter,Sqlca);

	if("918010100".equals(sOrgID)){
		doTemp.setVisible("CreditCompanySum",true);
		doTemp.setRequired("CreditCompanySum",true);
	}
    //设置上级机构选择方式
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
	String sButtons[][] = {
		{(CurUser.hasRole("098")?"true":"false"),"","Button","保存","保存修改","saveRecord()",sResourcesPath},
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=自定义函数;]~*/%>
    ...
    function doReturn(){
		OpenPage("/SystemManage/GeneralSetup/OrgArgList.jsp","_self","");
	}
	
	function selectUser(){
		sParaString = "OrgID"+","+"<%=ssOrgID%>";
		sParaString=sParaString+",OrgFlag,020";
		setObjectValue("SelectClassifyConUser",sParaString,"@VitualID@0@VitualIDName@1",0,0,"");
	}
	</script>
<%/*~END~*/%>
