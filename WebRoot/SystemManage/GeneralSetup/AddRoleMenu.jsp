...
<%int num = 0;
	sSql =  " select ItemNo || ' ' || ItemName as AllMainMenu,ItemNo" +
			" from CODE_LIBRARY "+
			" where CodeNo = 'MainMenu' "+
			" and RelativeCode not like '%"+sRoleID+"%' "+
			" and IsInUse = '1' ";
	...
	sSql1 = " select ItemNo || ' ' || ItemName as AllMainMenu,ItemNo" +
			" from CODE_LIBRARY "+
			" where CodeNo = 'MainMenu' "+
			" and RelativeCode like '%"+sRoleID+"%' "+
			" and IsInUse = '1' ";
	sSql1 += " order by SortNo ";
...
<script language="JavaScript">
   ...
   function pushButton(buttonName,push) 
	...
	function doQuery()
	{
		...
		PopPage("/SystemManage/GeneralSetup/AddMenuAction.jsp?RoleID=<%=sRoleID%>&ItemNo="+vReturn,"","dialogWidth=20px;dialogHeight=20px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
		self.close();
	}
