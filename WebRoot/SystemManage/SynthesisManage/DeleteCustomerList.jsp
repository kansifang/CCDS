...
<%
	...
	sSql = 	" select distinct CI.CustomerID,CI.CustomerName,getItemName('CustomerType',CI.CustomerType) as CustomerTypeName, "+
			...
			" from CUSTOMER_INFO CI left join CUSTOMER_BELONG CB on  CB.CustomerID = CI.CustomerID "+
			" where (CB.OrgID is null or CB.OrgID in (select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%')) ";
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		   {((CurUser.hasRole("097"))?"false":"true"),"","Button","清理","清理无效客户信息","clearCustomer()",sResourcesPath}
		};
	%> 
<%/*~END~*/%>
