...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "关联方管理详细页面"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sHeaders[][] = {               
	 ...
	 sSql = " select SerialNo,SectionType,CertType,CertID,CustomerID,RelativeName,Sum1,Sum2,BeginDate,EndDate,Attribute3, "+
		    " Remark,InputOrgID,getOrgName(InputOrgID) as OrgName,InputUserID,getUserName(InputUserID) as UserName,"+
		    " InputDate,UpdateDate "+
		    " from RELATIVE_SPECIAL "+
	...
	//设置必输项
	doTemp.setRequired("Attribute3,BeginDate,CertID,CertType,RelativeName",true);
	//设置只读列
	...
	//设置字段输入长度
	doTemp.setLimit("RelativeName",40);
	...
	dwTemp.setEvent("BeforeUpdate","!CustomerManage.InsertHistoryInfoLog(#SerialNo,"+CurUser.UserID+",ChangeRelativeList)");
	doTemp.setUnit("RelativeName"," <input type=button value=.. onclick=parent.selectCustomer()>");
	doTemp.setHTMLStyle("CertID"," onchange=parent.getCustomerName() ");
	dwTemp.setEvent("AfterInsert","!CustomerManage.AddCustomerInfo(#CustomerID,#RelativeName,#CertType,#CertID,,#InputUserID)");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function selectCustomer()
	{
		...
		setObjectValue("SelectOwner","","@CustomerID@0@RelativeName@1@CertType@2@CertID@3",0,0,"");	    
	}
