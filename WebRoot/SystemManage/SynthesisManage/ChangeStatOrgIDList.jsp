...
<%
	...
	sSql = sSql = " select '' as BCFlag,SerialNo,CustomerName,getOrgName(StatOrgID) as StatOrgName, "+
		  " StatOrgID,getBusinessName(BusinessType) as BusinessTypeName, "+
	              ...
	              " (select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%') ";
		    ...
		    }else
		    {
				...
				for(var i = 0 ; i < b ; i++)
				{
					...
					if(a == "√")
					{
						...
						//调用页面更新
						sReturn = PopPage("/SystemManage/SynthesisManage/ChangeStatOrgIDAction.jsp?SerialNo="+sSerialNo+"&FromOrgID="+sFromOrgID+"&FromOrgName="+sFromOrgName+"&ToOrgID="+sToOrgID+"&ToOrgName="+sToOrgName+"","","dialogWidth=21;dialogHeight=11;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no"); 
						if(sReturn == "TRUE")
