...
<%
	...
	//将修改的数据进行备份
	dwTemp.setEvent("AfterUpdate","!BusinessManage.DeleteBusinessContractBak(#SerialNo,BusinessPutOut)");
	//生成HTMLDataWindow
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord()
	{
		if(confirm("您确认将对出账详情信息做修改吗？确定请点击“确定”按钮！")){
			if(vI_all("myiframe0"))
			{
				//保存业务详情时将当前出账记录插入到BP_B备份表中
				sBPBSerialNo = initSerialNo();
				sObjectNo = getItemValue(0,getRow(),"SerialNo");
				sReturn = RunMethod("BusinessManage","AddBusinessContractBak",sBPBSerialNo+","+sObjectNo+","+"<%=CurOrg.OrgID%>"+","+"<%=CurUser.UserID%>"+",BusinessPutOut");
				if(sReturn == 1){
					as_save("myiframe0");
				}else{
					alert("保存历史数据失败！");
