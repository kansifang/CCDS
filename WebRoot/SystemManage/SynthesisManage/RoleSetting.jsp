...
<%
	...
	{
		sSql = "select a.RoleID as RoleID,b.RoleName as RoleName  from   User_Role a,Role_info b where  a.RoleID = b.RoleID and  a.UserID ='"+sUserID+"' "+
		         " and a.RoleID not in (select c.RoleID from User_Role c,Role_info d where  c.RoleID = d.RoleID and  c.UserID ='"+sToUserID+"') " ;//原来列表框的数据
		//out.println(sSql);
	...
	out.println("</script>");
		...
		sSql = "select a.RoleID as RoleID,b.RoleName as RoleName  from   User_Role a,Role_info b where  a.RoleID = b.RoleID and  a.UserID ='"+sToUserID+"' and a.Remark='1' " ;//转换后列表框的数据
		rs = Sqlca.getASResultSet(sSql);
		int inum=0;
		while(rs.next())
		{	
...
<input type='hidden' name='initialString' value='<%=request.getParameter("iniString")%>'>
...
function doQuery(){
	...
	sReturn = PopPage("/SystemManage/SynthesisManage/UpdateRoleAction.jsp?Action=<%=sAction%>&FromOrgID=<%=sFromOrgID%>&ToUserID=<%=sToUserID%>&RoleID=<%=sRoleID%>&UserID=<%=sUserID%>&Value="+value+"&rand="+randomNumber(),"","dialogWidth=35;dialogheight=17;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
