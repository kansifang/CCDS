...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "特殊客户维护信息列表"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%	
	...
	//sSql = " select CustomerName,BeginDate,EndDate,getOrgName(UpdateOrg) as UpdateOrgName,getUserName(UpdateUser) as UpdateUser,UpdateDate "+
	//	   " from CUSTOMER_SPECIALLOG  where SerialNo ='"+sSerialNo+"' order by UpdateDate ";
	if(sSerialNo=="" )
	{
		sSql = " select CustomerName,BeginDate,EndDate,getOrgName(UpdateOrg) as UpdateOrgName,getUserName(UpdateUser) as UpdateUser,UpdateDate "+
		  	   " from CUSTOMER_SPECIALLOG  where InstertType = 'ChangeSpecialCustList' order by UpdateDate ";
	}else
	{
		sSql = " select CustomerName,BeginDate,EndDate,getOrgName(UpdateOrg) as UpdateOrgName,getUserName(UpdateUser) as UpdateUser,UpdateDate "+
		       " from CUSTOMER_SPECIALLOG  where SerialNo ='"+sSerialNo+"' order by UpdateDate ";
