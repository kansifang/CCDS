    ...
    try{
    	if("1".equals(sInitFlag)){
    		...
    		Sqlca.executeSQL(" update code_library set IsInuse = Case "+
    				         " when codeno in ('IndCreditTempletType','CreditTempletType') and "+
    				         " (itemno like '11%' or itemno like '31%' or itemno like '51%' or itemno like '21%') then '1' "+
                             " when codeno in ('IndCreditTempletType','CreditTempletType') and "+
                             " (itemno like '50%' or itemno like '10%' or itemno like '30%' or itemno like '20%') then '2' "+
                             " else IsInuse end ");
    		...
    		Sqlca.executeSQL(" update dataobject_library  set colunit = case "+
                             " when dono = 'IndividualInfo' and colname = 'CreditBelongName' then '<input class=\"inputdate\" value=\"...\" type=button onClick=parent.selectCreditTempletType()>' "+
                             " when dono = 'EnterpriseInfo01' and colname = 'CreditBelongName' then '' "+
                             " when dono = 'EnterpriseInfo03' and colname = 'CreditBelongName' then '' "+
                             " else colunit end  ");
