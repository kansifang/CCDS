...
<%
	...
	sSql = " select * from REPORT_DATA where ReportNo in (select ReportNo from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN+"') and ReportScope = GETFSREPORTSCOPE('"+sGuarantyNo+"','"+sYearN+"')";
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN_1+"') and ReportScope = GETFSREPORTSCOPE('"+sGuarantyNo+"','"+sYearN_1+"')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("Rowsubject");
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN_2+"') and ReportScope = GETFSREPORTSCOPE('"+sGuarantyNo+"','"+sYearN_2+"')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("Rowsubject");
