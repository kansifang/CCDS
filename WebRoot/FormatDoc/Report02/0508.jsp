...
<%
	...
	String sSql = "select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
		  		+" where reportdate = '"+sYearN_1+"' and objectno = '"+sCustomerID+"' and ReportName ='现金流量表') and RowSubject = '813' and ReportScope = GETEVALUATESCOPE('"+sCustomerID+"','"+sYearN_1+"')";
	String sValue = "";
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN+"' and objectno = '"+sCustomerID+"' and ReportName ='现金流量表') and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"','"+sYearN+"')";
...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=22 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >5.8、现金流量分析</font></td>"); 	
	sTemp.append("   </tr>");	
