...
//取最新资产负债表日期
	...
	{
		sSql = "select substring(ReportDate from 1 for 4) as Year,substring(ReportDate from 5 for 2) as Month,ReportNo from REPORT_RECORD "+
	...
	{
		sSql = "select substr(ReportDate,1,4) as Year,substr(ReportDate,5,2) as Month,ReportNo from REPORT_RECORD "+
				" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate = (select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1') and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate)";
	...
	if(sDBName.startsWith("INFORMIX"))
	{
		sSql =  " select substring(ReportDate from 1 for 4) as Year,ReportNo from REPORT_RECORD "+
				" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate like '%12'  and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate) order by Year Desc";
	}else if(sDBName.startsWith("ORACLE"))
	{
...
//*****************************************损益表结构和对比*****************************
	...
	{
		sSql =" select substring(ReportDate from 1 for 4) as Year,ReportNo from REPORT_RECORD "+
			  " where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate like '%12'  and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate) order by Year Desc" ;
	}else if(sDBName.startsWith("ORACLE"))
	{
		sSql =" select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
			  " where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate like '%12'  and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate) order by Year Desc" ;
...
//年报
	...
	{
		sSql =  " select substring(ReportDate from 1 for 4) as Year,ReportNo from REPORT_RECORD "+
				" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate like '%12' and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate) order by Year Desc";
	}else if(sDBName.startsWith("ORACLE"))
	{
		sSql =  " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
				" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate like '%12' and ReportScope = GETFSREPORTSCOPE('"+sCustomerID+"',ReportDate) order by Year Desc";
...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4.3、财务简表分析</font></td>"); 	
	sTemp.append("   </tr>");
	sTemp.append("<tr>");
