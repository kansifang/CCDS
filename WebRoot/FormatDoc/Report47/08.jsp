...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4、授信申请用途</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
  	...
  	if(sBusinessType.equals("2010") || sBusinessType.equals("2020"))
  	...
  	if(sBusinessType.equals("1080010010") || sBusinessType.equals("1080010020") || sBusinessType.equals("1080010030") || sBusinessType.equals("1080010040"))
  		sTemp.append("     出口打包贷款：要求说明申请人和进口方的贸易合同关系（贸易标的、金额、付款条件、交货日期、付款日期）。说明出口打包贷款项下的信用证要素（开证行、编号、有效期、金额）。检查信用证和贸易合同的主要要素是否匹配，信用证是否存在影响收汇的软条款，检查出口押汇申请的金额和期限是否与信用证条款匹配；检查申请人是否收到进口商的预付款项（如有），调查申请人和进口商的历史贸易记录。调查申请人履行出口贸易合同的能力和计划。");
  	...
  	if(sBusinessType.substring(0,4).equals("1090"))
  		sTemp.append("     保理：要求说明保理项下的应收账款的清单，列明各笔应收账款的金额、债务人，期限和约定付款日。查验大额应收款项的交易合同、增值税发票和发货凭证，如果不能提供，要求说明原因。调查大额应收款的债务人的经营范围是否和交易合同相一致。调查大额应收款的债务人和申请人的历史贸易记录和付款记录。");
