...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View02;Describe=定义变量，获取参数;]~*/%>
	...
	String sApplyType = "",sLawCaseType = "",sLawCaseSerialNo = "";
	String sTable="";
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View03;Describe=定义树图;]~*/%>
	...
	sSql="select ApplyType from "+sTable+" where SerialNo='"+sObjectNo+"'";
	...
	//获取案件类型
	sSql =  " select SerialNo,LawCaseType from LAWCASE_INFO where ObjectNo = '"+sObjectNo+"' "; 
	rs = Sqlca.getASResultSet(sSql);
	if(rs.next()){
		sLawCaseType = DataConvert.toString(rs.getString("LawCaseType"));
		sLawCaseSerialNo = DataConvert.toString(rs.getString("SerialNo"));
	}
	rs.getStatement().close();
	...
	String sSqlTreeView = "from CODE_LIBRARY where CodeNo= 'BadBizView' ";
	sSqlTreeView += "and (RelativeCode like '%"+sApplyType+"%' or RelativeCode='All') " +
	         		" and IsInUse = '1' " ;
...
<%/*~BEGIN~可编辑区[Editable=true;CodeAreaID=View05;]~*/%>
	...
	function openChildComp(sCompID,sURL,sParameterString){
		...
		sLawCaseSerialNo="<%=sLawCaseSerialNo%>";
		...
		sParaStringTmp=sParaStringTmp.replace("#LawCaseType",sLawCaseType);
		sParaStringTmp=sParaStringTmp.replace("#LawCaseSerialNo",sLawCaseSerialNo);
