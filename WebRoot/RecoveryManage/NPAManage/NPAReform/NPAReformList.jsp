...
<%
 	...
 	if(sItemID.equals("010"))	//未批准重组方案:被屏蔽，当然在任何时候都可以放开，不是么？
	{
		sSql = 	" select BA.SerialNo as SerialNo,BA.ApplyType as ApplyType,getItemName('ReformType',BA.ApplyType) as ApplyTypeName," + 	
				...
				" where  BA.OperateUserID = '"+CurUser.UserID+"' " +
				" and  BA.OperateOrgID = '"+CurOrg.OrgID+"' " +
	...
	{	
		sSql = 	" select BA.SerialNo as SerialNo,BA.ApplyType as ApplyType,getItemName('ReformType',BA.ApplyType) as ApplyTypeName," + 	
				...
				" and FO.PhaseNo = '1000' "+
				" and BA.PigeonholeDate is null "+
				" and BA.SerialNo not in (select ObjectNo from CONTRACT_RELATIVE where ObjectType='NPARefromApply') "+
				" and BA.BusinessType = '6010' order by SerialNo desc " ;	
	}
	...
	{	
		sSql = 	" select BA.SerialNo as SerialNo,BA.ApplyType as ApplyType,getItemName('ReformType',BA.ApplyType) as ApplyTypeName," + 	
				...
				" where ObjectType='NPARefromApply') "+
				" and BA.BusinessType = '6010' order by SerialNo desc " ;
	}
	...
	{	
		sSql = 	" select BA.SerialNo as SerialNo,BA.ApplyType as ApplyType,getItemName('ReformType',BA.ApplyType) as ApplyTypeName," + 	
				...
				" and BA.SerialNo = FO.ObjectNo "+
				" and FO.ObjectType = 'NPAReformApply' "+
				" And (BA.Pigeonholedate is not null or BA.Pigeonholedate<>'')"+
				" and BA.BusinessType = '6010' order by SerialNo desc " ;
	}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function NewRecord()
	{
		...
		if (sItemID=="020")
		{
			var sReturn = createObject("NPAReformApply",
				"ApplyType=NPAReformApply~FlowNo=NPAReformFlow~PhaseNo=1000");//已经批准未执行。
		}

		if (sItemID=="030")
		{
			var sReturn = createObject("NPAReformApply",
				"ApplyType=NPAReformApply~FlowNo=NPAReformFlow~PhaseNo=8000");//否决的方案。
		}
		
		if(sReturn=="" || sReturn=="_CANCEL_" || typeof(sReturn)=="undefined") return;
		var sObjectNo = sReturn;  //申请编号
		openObject("NPARefromApply",sObjectNo,"001");
		reloadSelf();
	}
	...
	function DeleteRecord()
	{
		...
		if(confirm(getHtmlMessage(2))) //您真的想删除该信息吗？
		{
			...
			PopPage("/RecoveryManage/NPAManage/NPAReform/NPAReformDelete.jsp?SerialNo="+sSerialNo+	"&ObjectType=NPAReformApply&FlowNo=NPAReformFlow","","resizable=yes;dialogWidth=16;dialogHeight=10;center:yes;status:no;statusbar:no");
			as_del("myiframe0");
	...
	function cancelarch()
	{
		...
		else
		{
			if(confirm("真的要取消归档吗？")) 
			{
				sReturn = PopPage("/Common/WorkFlow/AddPigeonholeAction.jsp?ObjectType="+sObjectType+"&ObjectNo="+sObjectNo+"&Pigeonholed=Y","","");
				if(typeof(sReturn)!="undefined" && sReturn!="failed") 
				alert("取消归档成功！");
				self.location.reload();
			}
		...
		else
		{
			...
			if(sItemID=="060" || sItemID=="080")
			{
				...
				if (sItemID=="020")
				{
					...
					{	//特殊的标志位，用来决定是否显示重组台帐：除了060和080之外，均不显示重组台帐。
						sViewID = "003";  
					}
