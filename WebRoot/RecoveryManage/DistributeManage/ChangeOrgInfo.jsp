...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "不良资产管理机构变更"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "ManageChangeOrgInfo";
...
<script language=javascript>
	...
	function my_ChangeUserAction()
	{
		...
		var sReturn=RunMethod("PublicMethod","UpdateColValue","String@RecoverOrgID@"+sRecoverOrgID+"@String@RecoverUserID@None@String@OrgChangeFlag@1,BADBIZ_ACCOUNT,String@SerialNo@<%=sObjectNo%>");
	}
...
<script language=javascript>
	...
	{
		sOldOrgID = getItemValue(0,getRow(),"OldOrgID");//原管理人员			
		sNewOrgID = getItemValue(0,getRow(),"NewOrgID");//现管理人员
		if(typeof(sOldOrgID) != "undefined" && sOldOrgID != "" && 
		typeof(sNewOrgID) != "undefined" && sNewOrgID != "")
		{
			if(sOldOrgID == sNewOrgID)
			{
				alert("现管理机构不能与原管理机构相同！");
				return false;
			}
		}
		return true;
	...
	function getNewOrgName()
	{
		setObjectValue("SelectWholeOrg","","@NewOrgID@0@NewOrgName@1",0,0,"");
	}
</script>
<%/*~END~*/%>
