...
*	Tester:
*	Describe: 不良资产台账管理表内不良贷款台账台帐信息列表
*	Input Param:
*	Output Param:  
*		
*	
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "表内不良贷款台账"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%
	...
	String sHeaders[][] = {
						...
						{"BadLoanType","不良贷款初始类别"},
						{"BadLoanTypeName","不良贷款初始类别"},
						{"InitializeBalance","不良贷款初始本金"},
						{"ClassifyResult","风险分类初始情况"},
						{"ClassifyResultName","风险分类初始情况"},
						{"DisposeTotalSum","报告期清收处置本金合计"},
						{"MoneyReturnSum","报告期货币资金收回"},
						{"ReformUpSum","报告期重组上调"},
						{"NotReformUpSum","报告期非重组上调"},
						{"PayDebtSum","报告期以资抵债"},
						{"CVSum","报告期核销"},
						{"MetathesisSum","报告期资金置换"},
						{"OtherReturnSum","报告期其他方式收回"},
						{"CleanInterest1","报告期清收表内利息"},
						{"CleanInterest2","报告期清收表外利息"},
						{"FinalBalance","期末结欠本金余额"},
						{"FinalInterest","期末结欠利息余额"},
						{"ISLawFlag","是否起诉"},
						{"ReturnLawSum","诉讼收回本金"},
						{"ReturnLawInterest","诉讼收回利息"},
						{"FactUser","实际用款人"},
						{"CertType","借款人证件类型"},
	...
	*/
	...
	//设置共用格式
	doTemp.setVisible("Status,ClassifyResult,BadLoanType,BelongOrgID,RelativeContractNo,LoanType,CertType,RecoverOrgID,RecoverUserID,IsFinish,AccountNeedManage,BasicNeedManage",false);
	...
	{
		doTemp.setColumnAttribute("ClassifyResult,BadLoanType,BelongOrgName,CustomerName,LoanAccountNo,CertType,CertID","IsFilter","1");
	}else if(sStateFlag.equals("10"))//已登记
	{
		doTemp.setColumnAttribute("ClassifyResult,BadLoanType,BelongOrgName,CustomerName,LoanAccountNo,CertType,CertID,FactUser,FinalBalance,AccountNeedManage,BasicNeedManage,IsFinish,RecoverOrgName,RecoverUserName","IsFilter","1");
	}else if(sStateFlag.equals("03"))//已取消
	{
		doTemp.setColumnAttribute("ClassifyResult,BadLoanType,BelongOrgName,CustomerName,LoanAccountNo,CertType,CertID,FactUser,FinalBalance,RecoverOrgName,RecoverUserName","IsFilter","1");
	}else if(sStateFlag.equals("80"))//已终结
	{
		doTemp.setColumnAttribute("ClassifyResult,BadLoanType,BelongOrgName,CustomerName,LoanAccountNo,CertType,CertID,FactUser,FinalBalance,RecoverOrgName,RecoverUserName,BadBizFinishDate","IsFilter","1");
	}else
	{
	...
	//汇总			
	String[][] sListSumHeaders = {	{"Sum1","不良贷款初始本金"},
									{"Sum2","报告期清收处置本金合计"},
									{"Sum3","报告期货币资金收回"},
									{"Sum4","报告期重组上调"},
									{"Sum5","报告期非重组上调"},
...
<script language=javascript>
	...
	function account_Maintenance()
	{
		...
		if (typeof(sSerialNo)=="undefined" || sSerialNo.length==0)
		{
			...
			popComp("BadBizAccountInfo","/RecoveryManage/AccountManage/BadBizAccountInfo.jsp","ComponentName=不良贷款台帐详情&SerialNo="+sSerialNo+"&StateFlag=<%=sStateFlag%>&AccountType=010&AccountEditFlag=01","","");
			reloadSelf();
		}
	}
	...
	function account_Info()
	{
		...
		if (typeof(sSerialNo)=="undefined" || sSerialNo.length==0)
		{
			...
			popComp("BadBizAccountInfo","/RecoveryManage/AccountManage/BadBizAccountInfo.jsp","ComponentName=不良贷款台帐详情&SerialNo="+sSerialNo+"&StateFlag=<%=sStateFlag%>&AccountType=010&AccountEditFlag=03","","");
		}
	}
