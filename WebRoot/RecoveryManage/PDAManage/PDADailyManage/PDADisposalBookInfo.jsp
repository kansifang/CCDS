...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "抵债资产处置台账详情"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String mySql = " select Flag from ASSET_INFO where  SerialNo = '" + sObjectNo +"' ";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord()
	{
		...
		//如果是出租台帐,那么,出租日期与到期日期必须作时间上的判断.
		if (sDispositionType=="01")
		{
			var sBargainDate=getItemValue(0,0,"BargainDate");		
			var sUptodate=getItemValue(0,0,"UptoDate");		
			if (sBargainDate>sUptodate)
			{
				alert("出租日期不能晚于截止日期!");
				return;
			}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"ManageUserName","<%=CurUser.UserName%>");
