...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=View02;Describe=定义变量，获取参数;]~*/%>
	<%
		...
		sSql = " select AssetType,AssetStatus,Flag from ASSET_INFO where SerialNo = '"+sSerialNo+"' ";
...
<%/*~BEGIN~可编辑区[Editable=true;CodeAreaID=View05;]~*/%>
	...
	function TreeViewOnClick()
	{
		...
		}else if (sCurItemDescribe2=="PDABasicInfo") //基本信息
		{
			...
			OpenChildComp(sCurItemDescribe2,sCurItemDescribe1,"ComponentName="+sCurItemName+"&SerialNo="+sSerialNo+"&AssetType="+sAssetType+"&ObjectType="+sObjectType);		
			setTitle(sCurItemName);
		}else if (sCurItemDescribe2=="PDARelativeContractList") //相关合同信息
		{
			var sSerialNo="<%=sSerialNo%>";
			OpenChildComp(sCurItemDescribe2,sCurItemDescribe1,"ComponentName="+sCurItemName+"&SerialNo="+sSerialNo);
			setTitle(sCurItemName);
		}else if(sCurItemDescribe1 != "null" && sCurItemDescribe1 != "root")
		{
			var sSerialNo="<%=sSerialNo%>"; //资产编号
			var sObjectType="<%=sObjectType%>";//AssetInfo			
			OpenChildComp(sCurItemDescribe2,sCurItemDescribe1,"ComponentName="+sCurItemName+"&ObjectType="+sObjectType+"&ObjectNo="+sSerialNo);
