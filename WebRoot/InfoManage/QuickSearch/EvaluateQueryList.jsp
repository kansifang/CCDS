...
<%
	...
	" AccountMonth,ModelNo,getItemName('CreditTempletType',ModelNo) as ModelNoName,EvaluateScore,EvaluateResult,CognScore,CognResult,UserID,OrgID " +
	       	" from EVALUATE_RECORD" +
	" where ObjectType = 'Customer' "+
	" and OrgID in  (select BelongOrgID from ORG_BELONG where OrgID='"+CurOrg.OrgID+"') "+
	" and ModelNo not in('CreditLine') ";
	...
	doTemp.setVisible("UserID,OrgID,ModelNo",false);

	//设置不可见项
	doTemp.setVisible("SerialNo,ObjectType",false);

	//设置显示文本框的长度及事件属性
	doTemp.setHTMLStyle("CustomerName,ModelNoName","style={width:250px} ");  
	doTemp.setHTMLStyle("EvaluateDate,AccountMonth,EvaluateResult,CognResult","style={width:70px} ");  
		
	//设置对齐方式
	doTemp.setAlign("EvaluateDate,AccountMonth,EvaluateResult,CognResult","2");
