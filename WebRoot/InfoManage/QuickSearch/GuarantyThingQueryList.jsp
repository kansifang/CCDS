...
<%	
	...
	String sHeaders[][] =	{ 							
								...
								{"OwnerType","权属人类型"},
								{"EvalCurrencyName","抵押物评估币种"},
								{"EvalNetValue","抵押物评估价值"},
								{"GuarantyCurrencyName","抵押物认定币种"},
								{"ConfirmValue","抵押物认定价值"},
								{"EvalUnitPrice","评估单价"},
								{"AboutSum2","抵押物债权金额"},
								{"GuarantyRate","抵押率"}
							}; 
	...
	sSql =	" select GI.GuarantyID,GI.GuarantyName,GI.GuarantyType,getItemName('GuarantyList',GI.GuarantyType) as GuarantyTypeName,GI.OwnerName, " +
			...
			" GI.GuarantyCurrency,getItemName('Currency',GI.GuarantyCurrency) as GuarantyCurrencyName,GI.ConfirmValue,"+
			" GI.EvalUnitPrice,GI.AboutSum2,GI.GuarantyRate ,GR.ObjectNo " +
	       	" from GUARANTY_INFO GI,GUARANTY_RELATIVE GR " +
			" where GI.GuarantyID = GR.GuarantyID and GR.ObjectType='BusinessContract' "+
			" and GuarantyType like '010%' and InputOrgID in  (select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%') ";
	...
	//设置下拉框
	doTemp.setDDDWSql("GuarantyType","select ItemNo,ItemName from CODE_LIBRARY where CodeNo = 'GuarantyList' and ItemNo  like '010%' and length(ItemNo) > 3");
