...
<%
	...
	" BP.ICType,getItemName('BackLoanType',BP.ICType) as ICTypeName,BD.LCATimes as OverdueDays,BC.TermMonth,"+
	" BP.Purpose,case when BC.BusinessType like '1125%' then BC.Purpose else getItemName('LoanPurpose',BP.Purpose) end as PurposeName,"+//bllou 微小部门业务取合同的“用途”
	" BC.Flag2,getItemName('YesNo',BC.Flag2) as Flag2Name,BP.FixCYC,"+
	...
	" BD.InterestBalance2,BD.FineBalance1,BD.FineBalance2,BD.ActualBusinessRate,BD.PutOutDate,BD.Maturity,"+
	" #CMIntSum as CMIntSum,#AMIntSum as AMIntSum,"+//added by bllou 计算当月利息收入和累计利息收入 20120327		
	...
	" getOrgName(BD.OperateOrgID) as ManageOrgName,getUserName(BD.OperateUserID) as ManageUserName,case when BC.BusinessType like '1125%' then BC.Describe1 end as Describe1 " +//bllou 为微小企业增加审批人
	...
	//设置对齐方式
	doTemp.setAlign("RateFloat,BusinessSum,BusinessRate,Balance,OverdueBalance,InterestBalance1,InterestBalance2,FineBalance1,FineBalance2,CMIntSum,AMIntSum","3");	
	//小数为2，整数为5
	doTemp.setCheckFormat("BusinessSum","2");	
	doTemp.setCheckFormat("PutOutDate,Maturity","3");
	doTemp.setType("RateFloat,BusinessSum,ActualBusinessRate,Balance,OverdueBalance,InterestBalance1,InterestBalance2,FineBalance1,FineBalance2,CMIntSum,AMIntSum,TermMonth,OverdueDays","Number");
