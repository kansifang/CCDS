...
<%
	...
	String sHeaders[][] = { 							
					...
					{"AdjustRateType","利率启用方式(如遇人行利率调整，计息处理方式)"},
					{"MFOrgName","入账机构"},
					{"ManageOrgName","管户机构"},
					{"ManageUserName","管户人"},
					{"ClassifyResult","五级分类"},
	...
	*/
	...
	" getItemName('AdjustRateType',BP.AdjustRateType) as AdjustRateType,"+//added by bllou  20120426 利率启动方式
	...
	" getUserName(BD.OperateUserID) as ManageUserName " +
	    	" from BUSINESS_DUEBILL BD left join Business_PutOut BP on BD.RelativeSerialNo1 = BP.SerialNo join ENT_INFO EI on BD.CustomerID=EI.CustomerID  left join Als_UpdateEntScope AUS on EI.CustomerID = AUS.CustomerID,BUSINESS_CONTRACT BC left join ALS_UPDATEBUSIINDUSTRY AU on BC.SerialNo=AU.ApplyNo"+
	" where BC.BusinessType like '1%' "+
	...
	*/
	...
	doTemp.setHTMLStyle("OldDirectionName","style={width:350px} "); 
	...
	//小数为2，整数为5
	doTemp.setCheckFormat("BailRatio,BusinessSum,Balance,CK","2");	
