...
<%
	...
	if (sStatResult.equals("1")){
		...
		baseSql = StringFunction.replace(baseSql,"#ManageOrgID","(select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%')");
		baseSql = StringFunction.replace(baseSql,"#OperateOrgID","(select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%')");
		...
		querySql = StringFunction.replace(querySql,"#ManageOrgID","(select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%')");
		querySql = StringFunction.replace(querySql,"#OperateOrgID","(select OrgID from ORG_INFO where SortNo like '"+CurOrg.SortNo+"%')");
