...
<%
	...
	//定义变量
	String sSql="";//--存放sql语句  
	String environmentOrgID   = CurOrg.OrgID;//--当前机构
	String environmentUserID  = CurUser.UserID;//--当前用户
	String environmentOrgName = CurOrg.OrgName;//--当前机构名称
	//获得组件参数
	String sQueryType  =  DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("type"));
...
<%
	...
	String[] c = StringFunction.toStringArray(request.getRealPath(request.getServletPath()),ASConfigure.sCurSlash);
	for(int i = 0; i< (c.length-1); i++){
		xmlPath = xmlPath + c[i] + ASConfigure.sCurSlash;
	}
	...
	<table align='center' border="0" cellpadding="0" cellspacing="0" width="100%" height="99%" bgcolor='#FFFFFF'>
		<tr valign='top'>
			<td width="750">
				...
				<form name="Main" method="post" action="XResult.jsp" target="Result" >
					<input type=hidden name=CompClientID value="<%=CurComp.ClientID%>">
					<tr bordercolor="#FFFFFF"><td colspan=2>&nbsp;</td></tr>
					<tr bordercolor="#FFFFFF">
						<td valign='middle'>&nbsp;<img alt='查询当前设置'  border='0' src='<%=sResourcesPath%>/xquery.jpg' onclick="javascript:DataSubmit();" style='cursor:hand'>&nbsp;&nbsp;<img alt='清除当前设置' border='0' src='<%=sResourcesPath%>/xclear.jpg' onclick="javascript:Reset();changeResult(Main.elements['StatResult;;Other;'].value);" style='cursor:hand'>&nbsp;</td>
						...
						<td valign='top' width='25%' bgcolor='#00659C'>
							<table border=0 cellPadding=0 cellSpacing=0  style="CURSOR: hand" width="100%" >
								...
								<TR bgColor=#EEEEEE id=<%=temp[i][0]%>SelectTab vAlign=center >
									<TD  align=right valign="middle"><IMG alt="" border=0 id=<%=temp[i][0]%>SelectTab3 onclick="javascript:showHideContent('<%=temp[i][0]%>Select');"  src="<%=sResourcesPath%>/expand.gif" style="CURSOR: hand" width="15" height="15"></TD>
								...
								<TABLE border=0 cellPadding=0 cellSpacing=0 width="100% bgcolor="#FFFFFF">
									...
									<TR valign='top'>
										<TD colSpan=3 align="right" bgcolor='#FFFFFF'>
											<textarea  onclick="javascript:do<%=temp[i][0]%>Select();" rows="6" cols="70" name="<%=temp[i][0]%>NameList;;Other;" readonly  value="" ></textarea>
...
%>
					<tr bordercolor="#FFFFFF"><td valign='middle'>&nbsp;<img alt='查询当前设置' border='0' src='<%=sResourcesPath%>/xquery.jpg' onclick="javascript:DataSubmit();" style='cursor:hand'>&nbsp;&nbsp;<img alt='清除当前设置' border='0' src='<%=sResourcesPath%>/xclear.jpg' onclick="javascript:Reset();changeResult(Main.elements['StatResult;;Other;'].value);" style='cursor:hand'>&nbsp;</td>
					<tr bordercolor='#FFFFFF'><td colspan=2 height='20px' bgcolor='#FFFFFF'>&nbsp;</td></tr>
		...
		<form name="ChangeScheme" method="post" action="XQuery.jsp" target="_right"></form>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function doGroupSelect(){
		...
		sReturn = PopPage("/InfoManage/QueryManage/XSelect.jsp?iniString="+Main.elements["GroupList;;Other;"].value+"&disableString=<%=query.DisAvailableGroupColumns%>&scope=<%=query.availableGroupColumns%>&type=all&nametype=2&rand="+randomNumber(),"","dialogWidth=45;dialogheight=20;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
		if (sReturn!=""){
			if(sReturn=="@"){
	...
	function doDisplaySelect(){
		sReturn ="";
		sReturn = PopPage("/InfoManage/QueryManage/XSelect.jsp?iniString="+Main.elements["DisplayList;;Other;"].value+"&disableString=<%=query.DisAvailableDisplayColumns%>&scope=<%=query.availableDisplayColumns%>&type=all&nametype=1&rand="+randomNumber(),"","dialogWidth=45;dialogheight=20;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
		if (sReturn!=""){
			if(sReturn=="@"){
	...
	function doOrderSelect(){
		sReturn =PopPage("/InfoManage/QueryManage/XSelect.jsp?iniString="+Main.elements["OrderList;;Other;"].value+"&scope="+Main.elements["DisplayList;;Other;"].value+"&type=all&nametype=1&rand="+randomNumber(),"","dialogWidth=45;dialogheight=20;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
		if (sReturn!=""){
			if(sReturn=="@"){
				Main.elements["OrderList;;Other;"].value="";
				Main.elements["OrderNameList;;Other;"].value="";
	...
	function ShowSelect(sName){
		...
		//alert(sSelectCol);alert(sSelectedCol);

		//modify in 2008/04/10,2008/02/15 for xquery
		//sReturn = PopPage("/InfoManage/QueryManage/XSelectX.jsp?SelectCol="+sSelectCol+"&SelectedCol="+sSelectedCol+"","","dialogWidth=25;dialogheight=26;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
	...
	function ShowSelect(sInputName,sColName){
		...
		sReturn = PopPage("/InfoManage/QueryManage/XSelectX.jsp?SelectedCol="+sSelected+"&InputName="+sInputName+"&ColName="+sColName+"","","dialogWidth=25;dialogheight=26;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;statusbar:no");
		if(typeof(sReturn)!= "undefined" && sReturn.length!=0 && sReturn!=""&&sReturn!="XXXXXXXXXXXXXXX")
		{
			var sTemp1=sReturn.split("@@");
			var s1="",s2="";
