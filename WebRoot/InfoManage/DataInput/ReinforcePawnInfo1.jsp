...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord(sPostEvents)
	{
		...
		//add by hlzhang 2008-08-04 判断评估方式，以此提示用户必输项
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	/*~[Describe=通过“担保本金债权金额”/“认定价值”自动计算抵押率（%）;InputParam=无;OutPutParam=无;]~*/
	function getGuarantyRate() 
	{
		dConfirmValue = getItemValue(0,getRow(),"ConfirmValue");//认定价值
		sGuarantyCurrency = getItemValue(0,getRow(),"GuarantyCurrency");//认定币种
	    dERate = RunMethod("BusinessManage","GetERate",sGuarantyCurrency);
	    dConfirmValue = dConfirmValue*dERate;
		if(dConfirmValue == 0)
		{
			alert("此时无法算出抵押率！");
	    ...
	    {
	        dAboutSum2 = getItemValue(0,getRow(),"AboutSum2");//担保本金债权金额
	        dAboutSum2 = roundOff(dAboutSum2,2);
	        if(parseFloat(dAboutSum2) >= 0)
	        {	       
	            dGuarantyRate = parseFloat(dAboutSum2)/parseFloat(dConfirmValue)*100;
