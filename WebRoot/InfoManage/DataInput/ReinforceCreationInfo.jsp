...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "ReinforceCreationInfo";
	...
	//设置保存时操作关联数据表的动作
	dwTemp.setEvent("AfterInsert","!WorkFlowEngine.ReinforceCLInfo(#SerialNo,#BusinessType,#CustomerID,#CustomerName,#InputUserID,#InputOrgID)");
	//生成HTMLDataWindow
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	{		
		...
		/*
		//检查可循环额度的唯一性
		var sReturn = RunMethod("BusinessManage","CountCreditLineOnly",sCustomerID+","+"<%=CurOrg.OrgID%>"+","+sBusinessType);
		if(sReturn == 0.0 || sBusinessType=='3015' || sBusinessType=='3016' ){
			as_save("myiframe0",sPostEvents);
		}else{
			alert("此客户在所在一级支行里已存在有效的可循环授信额度，不允许再新增！请在客户详情中的额度信息中查看！");
			return;
		}
		*/
	}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{			
		...
		if(sCustomerType == "01")//公司客户
			setObjectValue("SelectApplyCustomer4",sParaString,"@CustomerID@0@CustomerName@1",0,0,"");
		...
		if(sCustomerType == "04" || sCustomerType == "05")
		{
			sParaString = "UserID"+","+"<%=CurUser.UserID%>"+","+"CustomerType"+","+sCustomerType;
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			}else if("<%=sBusinessType%>" == "3060")
			{
				setItemValue(0,0,"BusinessTypeName","信用共同体授信额度");
				//客户类型
				setItemValue(0,0,"CustomerType","05");
				//额度生效
				setItemValue(0,0,"FreezeFlag","1");		
			}else{
				//客户类型
			...
			//申请类型
			setItemValue(0,0,"ApplyType","CreditLineApply");
			//经办机构
			...
			//额度标志
			setItemValue(0,0,"FreezeFlag","1");//是否标志（1：是；2：否）
		}
    }
